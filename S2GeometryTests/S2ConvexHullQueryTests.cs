...
namespace S2Geometry
{
    public class S2ConvexHullQueryTests
    {
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_NoPoints() {
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_OnePoint() {
            S2ConvexHullQuery query = new();
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_TwoPoints() {
            S2ConvexHullQuery query = new();
            S2Point p=new(0, 0, 1);
            S2Point q = new(0, 1, 0);
            query.AddPoint(p);
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_EmptyLoop() {
            S2ConvexHullQuery query=new();
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_FullLoop() {
            S2ConvexHullQuery query=new();
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_EmptyPolygon() {
            S2ConvexHullQuery query=new();
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_NonConvexPoints() {
      ...
      "0:1, 0:9, 1:6, 2:6, 3:10, 4:10, 5:5, 4:0, 3:0, 2:5, 1:5"));
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_LoopsAroundNorthPole() {
        ...
        [Fact]
        public void Test_S2ConvexHullQueryTest_PointsInsideHull() {
            ...
            for (int iter = 0; iter < kIters; ++iter) {
                ...
                S2Cap cap = S2Testing.GetRandomCap(S2Constants.DoubleError, 1.999 * Math.PI);
                S2ConvexHullQuery query = new();
        ...
        private static void TestNorthPoleLoop(S1Angle radius, int num_vertices) {
            ...
            Assert.True(Math.Abs(radius.Radians - S2Constants.M_PI_2) >= S2Constants.DoubleError);

            S2ConvexHullQuery query = new();
            var loop = S2Loop.MakeRegularLoop(new S2Point(0, 0, 1), radius, num_vertices);
            query.AddLoop(loop);
            var result = query.GetConvexHull();
            if (radius > S1Angle.FromRadians(S2Constants.M_PI_2)) {
                Assert.True(result.IsFull);
            } else {
