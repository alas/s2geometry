...
public class S2LatLngRectBounderTests
{
    ...
    private static S2Point PointNearPole() => PerturbATowardsB(RandomPole(), S2Testing.RandomPoint());

    private static S2Point PointNearEquator() =>
        PerturbATowardsB(
            new S2Point(S2Testing.Random.RandDouble(),
            S2Testing.Random.RandDouble(), 0).Normalize(),
            RandomPole());
    ...
    internal void Test_RectBounder_NearlyIdenticalOrAntipodalPoints()
    {
        ...
        for (int iter = 0; iter < kIters; ++iter)
        {
            ...
            if ((a - b).CrossProd(a + b).Norm() <= 6.110 * S2.DoubleEpsilon)
            {
    ...
    internal void Test_RectBounder_ExpandForSubregions()
    {
        ...
        Assert.True(GetSubregionBound(-S2.M_PI_2, -1e-15, S2.M_PI_2 - 7e-16, 0).
                  IsFull());
        Assert.False(GetSubregionBound(-S2.M_PI_2, -1e-15, S2.M_PI_2 - 30e-16, 0).
                    IsFull());
        Assert.True(GetSubregionBound(-S2.M_PI_2 + 4e-16, 0, S2.M_PI_2 - 2e-16, 1e-7).
                    IsFull());
        Assert.False(GetSubregionBound(-S2.M_PI_2 + 30e-16, 0, S2.M_PI_2, 1e-7).
                    IsFull());
        Assert.True(GetSubregionBound(-S2.M_PI_2 + 4e-16, 0, S2.M_PI_2 - 4e-16, S2.M_PI_2).
                    IsFull());
        Assert.False(GetSubregionBound(-S2.M_PI_2, 0, S2.M_PI_2 - 30e-16, S2.M_PI_2).
                    IsFull());
        ...
        Assert.True(GetSubregionBound(-S2.M_PI_2, 0, S2.M_PI_2 - 1e-8, Math.PI - 1e-7).
                  IsFull());
        Assert.False(GetSubregionBound(-S2.M_PI_2, 0, S2.M_PI_2 - 1e-7, Math.PI - 1e-7).
                     IsFull());
        Assert.True(GetSubregionBound(-S2.M_PI_2 + 1e-12, -Math.PI + 1e-4, S2.M_PI_2, 0).
                    IsFull());
        Assert.True(GetSubregionBound(-S2.M_PI_2 + 1e-11, -Math.PI + 1e-4, S2.M_PI_2, 0).
                     IsFull());
        ...
        // apart in latitude.
        Assert.True(GetSubregionBound(1.5, -S2.M_PI_2, 1.5, S2.M_PI_2 - 2e-16).
                    ApproxEquals(new S2LatLngRect(new R1Interval(1.5, 1.5),
                                              S1Interval.Full), kRectError));
        Assert.True(GetSubregionBound(1.5, -S2.M_PI_2, 1.5, S2.M_PI_2 - 7e-16).
                    ApproxEquals(new S2LatLngRect(new R1Interval(1.5, 1.5),
                                              new S1Interval(-S2.M_PI_2, S2.M_PI_2 - 7e-16)),
                                 kRectError));
        ...
        Assert.True(GetSubregionBound(-S2.M_PI_2 + 1e-15, 0, -S2.M_PI_2 + 1e-15, 0).
                    ApproxEquals(new S2LatLngRect(new R1Interval(-S2.M_PI_2, -S2.M_PI_2 + 1e-15),
                                              S1Interval.Full), kRectError));
        Assert.True(GetSubregionBound(S2.M_PI_2 - 1e-15, 0, S2.M_PI_2 - 1e-15, 0).
                    ApproxEquals(new S2LatLngRect(new R1Interval(S2.M_PI_2 - 1e-15, S2.M_PI_2),
                                              S1Interval.Full), kRectError));
    }
    ...
    internal void Test_RectBounder_AccuracyBug()
    {
        ...
        S2Point b=new(7.9020571389665525e-08, -6.6407120842906012e-10,
            0.99999999999999689);
        S2Point c=new(0.9999999999999768, -1.2246467991472876e-16,
            2.1496584824676253e-07);
        S2Point z=new(0, 0, 1);

        // The edge AC is closer to the north pole Z than AB or BC.
        Assert.Equal(S2Pred.Sign(a, b, c), 1);
        Assert.Equal(S2Pred.Sign(a, c, z), 1);
