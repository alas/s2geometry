...
public class S2R2RectTests
{
    ...
    internal void Test_S2R2Rect_ConstructorsAndAccessors()
    {
        ...
        Assert.Equal(d1, d1);
        Assert.NotEqual(d1, S2R2Rect.Empty);
    }
    ...
    internal void Test_S2R2Rect_SimplePredicates()
    {
        ...
        Assert.Equal(new R2Point(0, 0.25), r1.Vertex(0));
        Assert.Equal(new R2Point(0.5, 0.25), r1.Vertex(1));
        Assert.Equal(new R2Point(0.5, 0.75), r1.Vertex(2));
        Assert.Equal(new R2Point(0, 0.75), r1.Vertex(3));
        ...
        for (int k = 0; k < 4; ++k)
        {
            ...
            Assert.True(S2Pred.Sign(S2R2Rect.ToS2Point(r1.Vertex(k - 1)),
                S2R2Rect.ToS2Point(r1.Vertex(k)),
                S2R2Rect.ToS2Point(r1.Vertex(k + 1))) > 0);
        }
    }
    ...
    internal void Test_S2R2Rect_IntervalOperations()
    {
        ...
        R2Point ne1 = new(0.5, 0.75);
        S2R2Rect r1 = new(sw1, ne1);
        S2R2Rect r1_mid = new(new R2Point(0.25, 0.5), new R2Point(0.25, 0.5));
        S2R2Rect r_sw1 = new(sw1, sw1);
        S2R2Rect r_ne1 = new(ne1, ne1);
    ...
    internal void Test_S2R2Rect_CellOperations()
    {
        ...
        S2R2Rect r4 = new(new R2Point(0, 0), new R2Point(0.5, 0.5));
        ...
        S2R2Rect r5 = new(new R2Point(0, 0.45), new R2Point(0.5, 0.55));
    ...
    private static void TestCellOps(S2R2Rect r, S2Cell cell, int level)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (S2.FaceXYZtoUV(0, cell.VertexRaw(i), out var u, out var v))
            {
                if (r.Contains(new R2Point(S2.UVtoST(u), S2.UVtoST(v))))
                    vertex_contained = true;
            }
            if (!r.IsEmpty() && cell.Contains(S2R2Rect.ToS2Point(r.Vertex(i))))
                vertex_contained = true;
        }
