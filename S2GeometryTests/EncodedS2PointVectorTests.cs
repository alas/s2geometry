    ...
    public class EncodedS2PointVectorTests
    {
        ...
        public void Test_EncodedS2PointVectorTest_CellIdWithException()
        {
            ...
            TestEncodedS2PointVector(new S2Point[]
                { S2TextFormat.MakeCellIdOrDie("1/23").ToPoint(), new S2Point(0.1, 0.2, 0.3).Normalized},
        ...
        public void Test_EncodedS2PointVectorTest_PointsAtMultipleLevels()
        {
            ...
            TestEncodedS2PointVector(new S2Point[]
                {
                    S2TextFormat.MakeCellIdOrDie("2/11001310230102").ToPoint(),
                    S2TextFormat.MakeCellIdOrDie("1/23322").ToPoint(),
                    S2TextFormat.MakeCellIdOrDie("3/3").ToPoint(),
                    S2TextFormat.MakeCellIdOrDie("1/23323").ToPoint(),
                    S2TextFormat.MakeCellIdOrDie("2/12101023022012").ToPoint()
                },
        ...
        public void Test_EncodedS2PointVectorTest_SixtyFourBitOffset()
        {
            ...
            const int level = S2Constants.kMaxCellLevel;
            var points = new S2Point[kBlockSize]
        ...
        public void Test_EncodedS2PointVectorTest_AllExceptionsBlock()
        {
            ...
            var points = new S2Point[kBlockSize].Fill(EncodedValueToPoint(0, S2Constants.kMaxCellLevel));
            points = new List<S2Point>(points)
            {
                new S2Point(0.1, 0.2, 0.3).Normalized,
                new S2Point(0.3, 0.2, 0.1).Normalized,
            }.ToArray();
            // Encoding: header (2 bytes), block count (1 byte), block lengths (2 bytes).
        ...
        public void Test_EncodedS2PointVectorTest_MaxFaceSiTiAtAllLevels()
        {
            ...
            {
                _logger.WriteLine("Level = " + level);
                S2CellId id = S2CellId.FromFaceIJ(5, S2Constants.kLimitIJ - 1, S2Constants.kLimitIJ - 1)
                              .Parent(level);
        ...
        private static S2Point EncodedValueToPoint(UInt64 value, int level)
        {
            ...
            return S2Coords.FaceUVtoXYZ(face,
                ...
                S2Coords.STtoUV(S2Coords.SiTitoST(ti))).Normalized;
        }
    }
}
