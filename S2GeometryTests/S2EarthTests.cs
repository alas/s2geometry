...
public class S2EarthTests
{
    ...
    public void Test_S2EarthTest_TestAngleConversion()
    {
        ...
        Assert2.Equal(MetersToAngle(RadiusMeters).Radians, 1);
        Assert2.Equal(MetersToChordAngle(RadiusMeters).Radians(), 1);
        Assert2.Equal(MetersToRadians(RadiansToKm(0.3) * 1000), 0.3);
        Assert2.Equal(ToMeters(S1Angle.FromDegrees(180)), RadiusMeters * S2.M_PI);
        Assert2.Equal(ToMeters(S1ChordAngle.FromDegrees(180)), RadiusMeters * S2.M_PI);
        Assert2.Equal(RadiansToMeters(KmToRadians(2.5)), 2500);

        // Functions that use kilometers:
        Assert2.Equal(KmToAngle(RadiusKm).Radians, 1);
        Assert2.Equal(KmToChordAngle(RadiusKm).Radians(), 1);
        Assert2.Equal(KmToRadians(RadiusMeters / 1000), 1);
        Assert2.Equal(ToKm(S1Angle.FromRadians(0.5)), 0.5 * RadiusKm);
        Assert2.Equal(ToKm(S1ChordAngle.FromRadians(0.5)), 0.5 * RadiusKm);
        Assert2.Equal(RadiansToKm(0.5), 0.5 * RadiusKm);
        ...
        Assert2.Equal(ToAngle(Radius).Radians, 1);
        Assert2.Equal(ToChordAngle(Radius).Radians(), 1);
        Assert2.Equal(ToRadians(Radius), 1);
        Assert2.Equal(ToDistance(S1Angle.FromRadians(2)), 2 * RadiusMeters);
        Assert2.Equal(ToDistance(S1ChordAngle.FromRadians(0.5)), 0.5 * RadiusMeters);
        Assert2.Equal(RadiansToDistance(1.5), 1.5 * RadiusMeters);
    }

    [Fact]
    public void Test_S2EarthTest_TestSolidAngleConversion()
    {
        Assert2.Equal(1, SquareKmToSteradians(Math.Pow(RadiusMeters / 1000, 2)));
        Assert2.Equal(Math.Pow(0.5 * RadiusKm, 2),
            SteradiansToSquareKm(Math.Pow(0.5, 2)));
        Assert2.Equal(Math.Pow(0.3, 2),
    ...
    public void Test_S2EarthTest_TestToLongitudeRadians()
    {
        ...
        Assert2.Equal(1, ToLongitudeRadians(earth_radius, 0));
    ...
    public void Test_S2EarthTest_TestGetDistance()
    {
        ...
        Assert.Equal((Meters)(S2.M_PI * RadiusMeters), (Meters)GetDistance(north, south));
        Assert.Equal(0, GetDistanceKm(west, west));
        Assert.Equal(S2.M_PI_2 * RadiusMeters, GetDistanceMeters(north, west));

        Assert.Equal(GetDistance(west, south), GetDistance(
            S2LatLng.FromDegrees(0, -90),
            S2LatLng.FromDegrees(-90, -38)));

        Assert.Equal(RadiusKm, GetDistanceKm(
            S2LatLng.FromRadians(0, 0.6),
            S2LatLng.FromRadians(0, -0.4)));

        Assert.Equal(1000 * RadiusKm * S2.M_PI_4, GetDistanceMeters(
