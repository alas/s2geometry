    ...
    public class S1ChordAngleTests
    {
        ...
        public void Test_S1ChordAngle_TwoPointConstructor()
        {
            for (int iter = 0; iter < 100; ++iter)
            {
                ...
                Assert2.Near(S2Constants.M_PI_2, new S1ChordAngle(x, z).Radians, 0.03);
                S2Point w = (y + z).Normalized;
                Assert2.Near(S2Constants.M_PI_4, new S1ChordAngle(w, z).Radians);
            }
        }
        ...
        public void Test_S1ChordAngle_Trigonometry()
        {
            ...
            for (int iter = 0; iter <= kIters; ++iter)
            {
                ...
                S1ChordAngle angle = new S1ChordAngle(S1Angle.FromRadians(radians));
                Assert2.Near(Math.Sin(radians), angle.Sin, S2Constants.DoubleError);
                Assert2.Near(Math.Cos(radians), angle.Cos, S2Constants.DoubleError);
                ...
                Assert2.Near(Math.Atan(Math.Tan(radians)), Math.Atan(angle.Tan), S2Constants.DoubleError);
            }
        ...
        public void Test_S1ChordAngle_GetS2PointConstructorMaxError()
        {
            ...
            for (int iter = 0; iter < 100000; ++iter)
            {
                ...
                if (S2Testing.Random.OneIn(10))
                {
                    ...
                    S1Angle r = S1Angle.FromRadians(S2Constants.DoubleError * S2Testing.Random.RandDouble());
