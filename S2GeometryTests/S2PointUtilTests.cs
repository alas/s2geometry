...
namespace S2Geometry
{
    public class S2PointUtilTests
    {
        ...
        public void Test_S2_Frames() {
            ...
            Assert.True(S2.ApproxEquals(S2.ToFrame(m, m.Col(0)), new S2Point(1, 0, 0)));
            Assert.True(S2.ApproxEquals(S2.ToFrame(m, m.Col(1)), new S2Point(0, 1, 0)));
            Assert.True(S2.ApproxEquals(S2.ToFrame(m, m.Col(2)), new S2Point(0, 0, 1)));

            Assert.True(S2.ApproxEquals(S2.FromFrame(m, new S2Point(1, 0, 0)), m.Col(0)));
            Assert.True(S2.ApproxEquals(S2.FromFrame(m, new S2Point(0, 1, 0)), m.Col(1)));
            Assert.True(S2.ApproxEquals(S2.FromFrame(m, new S2Point(0, 0, 1)), m.Col(2)));
        }
