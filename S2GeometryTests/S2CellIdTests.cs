...
namespace S2Geometry
{
    public class S2CellIdTests
    {
        ...
        {
            S2CellId id = S2CellId.FromFacePosLevel(3, 0x12345678, S2Constants.kMaxCellLevel - 4);
            Assert.True(id.IsValid);
            Assert.Equal(3UL, id.Face);
            Assert.Equal(0x12345700UL, id.Pos);
            Assert.Equal(S2Constants.kMaxCellLevel - 4, id.Level);
            ...
            Assert.Equal(id.RangeMin, id.ChildBegin(S2Constants.kMaxCellLevel));
            Assert.Equal(id.RangeMax.Next, id.ChildEnd(S2Constants.kMaxCellLevel));
        ...
        public void Test_S2CellId_CenterSiTi()
        {
            ...
            // Leaf level, 30.
            id.GetCenterSiTi(out var si, out var ti);
            Assert.Equal(1 << 0, si & 1);
            Assert.Equal(1 << 0, ti & 1);

            // Level 29.
            id.Parent(S2Constants.kMaxCellLevel - 1).GetCenterSiTi(out si, out ti);
            Assert.Equal(1 << 1, si & 3);
            Assert.Equal(1 << 1, ti & 3);

            // Level 28.
            id.Parent(S2Constants.kMaxCellLevel - 2).GetCenterSiTi(out si, out ti);
            Assert.Equal(1 << 2, si & 7);
            Assert.Equal(1 << 2, ti & 7);

            // Level 20.
            id.Parent(S2Constants.kMaxCellLevel - 10).GetCenterSiTi(out si, out ti);
            Assert.Equal(1 << 10, si & ((1 << 11) - 1));
            Assert.Equal(1 << 10, ti & ((1 << 11) - 1));

            // Level 10.
            id.Parent(S2Constants.kMaxCellLevel - 20).GetCenterSiTi(out si, out ti);
            Assert.Equal(1 << 20, si & ((1 << 21) - 1));
            Assert.Equal(1 << 20, ti & ((1 << 21) - 1));

            // Level 0.
            id.Parent(0).GetCenterSiTi(out si, out ti);
        ...
        public void Test_S2CellId_Wrapping()
        {
            ...
            Assert.Equal(S2CellId.FromFacePosLevel(
                          5, ~0UL >> S2CellId.kFaceBits, S2Constants.kMaxCellLevel),
                      S2CellId.Begin(S2Constants.kMaxCellLevel).PrevWrap());
            Assert.Equal(S2CellId.FromFacePosLevel(
                          5, ~0UL >> S2CellId.kFaceBits, S2Constants.kMaxCellLevel),
                      S2CellId.Begin(S2Constants.kMaxCellLevel).AdvanceWrap(-1));
            ...
            Assert.Equal(S2CellId.FromFacePosLevel(0, 0, S2Constants.kMaxCellLevel),
                      S2CellId.End(S2Constants.kMaxCellLevel).Prev.NextWrap());
            Assert.Equal(S2CellId.FromFacePosLevel(0, 0, S2Constants.kMaxCellLevel),
                      S2CellId.End(S2Constants.kMaxCellLevel).Advance(-1).AdvanceWrap(1));
        }
        ...
        public void Test_S2CellId_Advance()
        {
            ...
            Assert.Equal(id.Next.ChildBegin(S2Constants.kMaxCellLevel),
                      id.ChildBegin(S2Constants.kMaxCellLevel).Advance(256));
            Assert.Equal(S2CellId.FromFacePosLevel(5, 0, S2Constants.kMaxCellLevel),
                      S2CellId.FromFacePosLevel(1, 0, S2Constants.kMaxCellLevel)
                      .Advance((long)(4UL << (2 * S2Constants.kMaxCellLevel))));
            ...
            Assert.Equal(id.Next.ChildBegin(S2Constants.kMaxCellLevel),
                      id.ChildBegin(S2Constants.kMaxCellLevel).AdvanceWrap(256));
            Assert.Equal(S2CellId.FromFacePosLevel(1, 0, S2Constants.kMaxCellLevel),
                      S2CellId.FromFacePosLevel(5, 0, S2Constants.kMaxCellLevel)
                      .AdvanceWrap((long)(2UL << (2 * S2Constants.kMaxCellLevel))));
        }
        ...
        public void Test_S2CellId_DistanceFromBegin()
        {
            ...
            Assert.Equal(6 * (1L << (2 * S2Constants.kMaxCellLevel)),
                      S2CellId.End(S2Constants.kMaxCellLevel).DistanceFromBegin());

            Assert.Equal(0, S2CellId.Begin(0).DistanceFromBegin());
            Assert.Equal(0, S2CellId.Begin(S2Constants.kMaxCellLevel).DistanceFromBegin());
        ...
        public void Test_S2CellId_GetCommonAncestorLevel()
        {
            ...
            Assert.Equal(30, S2CellId.FromFace(0).ChildBegin(30).GetCommonAncestorLevel(S2CellId.FromFace(0).ChildBegin(30)));

            // One cell id is a descendant of the other.
            Assert.Equal(0, S2CellId.FromFace(0).ChildBegin(30).GetCommonAncestorLevel(S2CellId.FromFace(0)));
            Assert.Equal(0, S2CellId.FromFace(5).GetCommonAncestorLevel(S2CellId.FromFace(5).ChildEnd(30).Prev));

            // Two cells that have no common ancestor.
            Assert.Equal(-1, S2CellId.FromFace(0).GetCommonAncestorLevel(S2CellId.FromFace(5)));
            Assert.Equal(-1, S2CellId.FromFace(2).ChildBegin(30).GetCommonAncestorLevel(S2CellId.FromFace(3).ChildEnd(20)));

            // Two cells that have a common ancestor distinct from both of them.
            Assert.Equal(8, S2CellId.FromFace(5).ChildBegin(9).Next.ChildBegin(15).GetCommonAncestorLevel(
        ...
        public void Test_S2CellId_Inverses()
        {
            ...
            {
                S2CellId id = S2Testing.GetRandomCellId(S2Constants.kMaxCellLevel);
                Assert.True(id.IsLeaf);
                Assert.Equal(S2Constants.kMaxCellLevel, id.Level);
        ...
        public void Test_S2CellId_Tokens()
        {
            ...
            for (int i = 0; i < 10000; ++i)
            {
                ...
                Assert.Equal(id, S2CellId.FromToken(token, token.Length));
            }
            ...
            Assert.Equal(S2CellId.None, S2CellId.FromToken(token2, token2.Length));

            // Sentinel is invalid.
            token2 = S2CellId.Sentinel.ToToken();
            Assert.Equal(S2CellId.FromToken(token2), S2CellId.Sentinel);
            Assert.Equal(S2CellId.FromToken(token2, token2.Length),
                      S2CellId.Sentinel);
        ...
        {
            S2CellId id = new S2CellId(0x7837423);
            Encoder encoder = new Encoder();
            id.Encode(encoder);
            Decoder decoder = new Decoder(encoder.Buffer, 0, encoder.Length);
            var (success, decoded_id) = S2CellId.DecodeStatic(decoder);
            Assert.True(success);
        ...
        public void Test_S2CellId_DecodeFailsWithTruncatedBuffer()
        {
            ...
            Decoder decoder = new Decoder(encoder.Buffer, 0, encoder.Length - 2);
            var (success, _) = S2CellId.DecodeStatic(decoder);
        ...
        public void Test_S2CellId_Continuity()
        {
            ...
            double max_dist = S2Metrics.kMaxEdge.GetValue(kMaxWalkLevel);
            ...
            for (; id != end; id = id.Next)
            {
                ...
                // in (s,t) coordinates.
                S2Coords.XYZtoFaceUV(id.ToPointRaw(), out var u, out var v);
                Assert2.Near(Math.IEEERemainder(S2Coords.UVtoST(u), 0.5 * kCellSize), 0.0, S2Constants.DoubleError);
                Assert2.Near(Math.IEEERemainder(S2Coords.UVtoST(v), 0.5 * kCellSize), 0.0, S2Constants.DoubleError);
            }
        }
        ...
        public void Test_S2CellId_Coverage()
        {
            ...
            double max_dist = 0.5 * S2Metrics.kMaxDiag.GetValue(S2Constants.kMaxCellLevel);
        ...
        public void Test_S2CellId_Neighbors()
        {
            ...
            for (int level = 1; level <= S2Constants.kMaxCellLevel; ++level)
            {
                S2CellId id2 = S2CellId.FromFaceIJ(1, 0, 0).Parent(level);
                var nbrs2 = new S2CellId[4];
                id2.GetEdgeNeighbors(nbrs2);
            ...
            S2CellId id1 = S2CellId.FromFacePosLevel(0, 0, S2Constants.kMaxCellLevel);
            ...
            for (var i = 0; i < 1000; ++i)
            {
                ...
                int max_diff = Math.Min(5, S2Constants.kMaxCellLevel - id2.Level - 1);
        ...
        private static void TestExpandedByDistanceUV(S2CellId id, S1Angle distance)
        {
            ...
            for (int iter = 0; iter < 100; ++iter)
            {
                ...
                S2Point center = S2Coords.FaceUVtoXYZ(face, center_uv).Normalized;
                ...
                if (!S2Coords.FaceXYZtoUV(face, p, out var uv)) continue;
                R2Point closest_uv = ProjectToBoundary(uv, bound);
                S2Point closest = S2Coords.FaceUVtoXYZ(face, closest_uv).Normalized;
                S1Angle actual_dist = new S1Angle(p, closest);
        ...
        private static void ExpandCell(S2CellId parent, List<S2CellId> cells, Dictionary<S2CellId, S2CellId> parent_map)
        {
            ...
            for (int pos = 0; child != parent.ChildEnd(); child = child.Next, ++pos)
            {
                ...
                Assert.Equal(orientation ^ S2Coords.kPosToOrientation[pos], child_orientation);
