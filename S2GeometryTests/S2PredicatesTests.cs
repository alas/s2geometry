...
namespace S2Geometry
{
    public class S2PredicatesTests
    {
        ...
        public void Test_Sign_CollinearPoints()
        {
            ...
            S2Point x1 = new S2Point(0.99999999999999989, 1.4901161193847655e-08, 0);
            S2Point x2 = new S2Point(1, 1.4901161193847656e-08, 0);
            Assert.Equal(x1, x1.Normalized);
            Assert.Equal(x2, x2.Normalized);
            ...
            S2Point x3 = new S2Point(1, 1, 1).Normalized;
            S2Point x4 = 0.99999999999999989 * x3;
            Assert.Equal(x3, x3.Normalized);
            Assert.Equal(x4, x4.Normalized);
            ...
            S2Point y0 = new S2Point(1, 1, 0);
            S2Point y1 = y0.Normalized;
            S2Point y2 = y1.Normalized;
            Assert.NotEqual(y1, y2);
            Assert.Equal(y2, y2.Normalized);
        ...
        public void Test_SignTest_StressTest()
        {
            ...
            const int kNumPointsPerCircle = 20;
            ...
            for (int iter = 0; iter < 3; ++iter)
            {
                ...
                TestGreatCircle(new S2Point(1, 0, 0), new S2Point(0, 1, 0), kNumPointsPerCircle);
                TestGreatCircle(new S2Point(1, 0, 0), new S2Point(0, 0, 1), kNumPointsPerCircle);
                TestGreatCircle(new S2Point(0, -1, 0), new S2Point(0, 0, 1), kNumPointsPerCircle);
                ...
                TestGreatCircle(new S2Point(1 << 25, 1, -8), new S2Point(-4, -(1 << 20), 1), kNumPointsPerCircle);
            }
        }
        ...
        public void Test_Sign_SymbolicPerturbationCodeCoverage()
        {
            ...
            // det(M_1) = b0*c1 - b1*c0
            CheckSymbolicSign(1, new S2Point(-3, -1, 0), new S2Point(-2, 1, 0), new S2Point(1, -2, 0));

            // det(M_2) = b2*c0 - b0*c2
            CheckSymbolicSign(1, new S2Point(-6, 3, 3), new S2Point(-4, 2, -1), new S2Point(-2, 1, 4));

            // det(M_3) = b1*c2 - b2*c1
            CheckSymbolicSign(1, new S2Point(0, -1, -1), new S2Point(0, 1, -2), new S2Point(0, 2, 1));
            // From this point onward, B or C must be zero, or B is proportional to C.

            // det(M_4) = c0*a1 - c1*a0
            CheckSymbolicSign(1, new S2Point(-1, 2, 7), new S2Point(2, 1, -4), new S2Point(4, 2, -8));

            // det(M_5) = c0
            CheckSymbolicSign(1, new S2Point(-4, -2, 7), new S2Point(2, 1, -4), new S2Point(4, 2, -8));

            // det(M_6) = -c1
            CheckSymbolicSign(1, new S2Point(0, -5, 7), new S2Point(0, -4, 8), new S2Point(0, -2, 4));

            // det(M_7) = c2*a0 - c0*a2
            CheckSymbolicSign(1, new S2Point(-5, -2, 7), new S2Point(0, 0, -2), new S2Point(0, 0, -1));

            // det(M_8) = c2
            CheckSymbolicSign(1, new S2Point(0, -2, 7), new S2Point(0, 0, 1), new S2Point(0, 0, 2));
            // From this point onward, C must be zero.

            // det(M_9) = a0*b1 - a1*b0
            CheckSymbolicSign(1, new S2Point(-3, 1, 7), new S2Point(-1, -4, 1), S2Point.Empty);

            // det(M_10) = -b0
            CheckSymbolicSign(1, new S2Point(-6, -4, 7), new S2Point(-3, -2, 1), S2Point.Empty);

            // det(M_11) = b1
            CheckSymbolicSign(-1, new S2Point(0, -4, 7), new S2Point(0, -2, 1), S2Point.Empty);

            // det(M_12) = a0
            CheckSymbolicSign(-1, new S2Point(-1, -4, 5), new S2Point(0, 0, -3), S2Point.Empty);

            // det(M_13) = 1
            CheckSymbolicSign(1, new S2Point(0, -4, 5), new S2Point(0, 0, -5), S2Point.Empty);
        }
        ...
        public void Test_CompareDistances_Coverage()
        {
            ...
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, 1 - 1e-15, 1), new S2Point(1, 1, 1 + 2e-15), -1, Precision.DOUBLE, Sin2Distances);
            TestCompareDistances(new S2Point(1, 1, 0), new S2Point(1, 1 - 1e-15, 1e-21), new S2Point(1, 1 - 1e-15, 0), 1, Precision.DOUBLE, Sin2Distances);
            TestCompareDistances(new S2Point(2, 0, 0), new S2Point(2, -1, 0), new S2Point(2, 1, 1e-8), -1, Precision.LONG_DOUBLE, Sin2Distances);
            TestCompareDistances(new S2Point(2, 0, 0), new S2Point(2, -1, 0), new S2Point(2, 1, 1e-100), -1, Precision.EXACT, Sin2Distances);
            TestCompareDistances(new S2Point(1, 0, 0), new S2Point(1, -1, 0), new S2Point(1, 1, 0), 1, Precision.SYMBOLIC, Sin2Distances);
            TestCompareDistances(new S2Point(1, 0, 0), new S2Point(1, 0, 0), new S2Point(1, 0, 0), 0, Precision.SYMBOLIC, Sin2Distances);

            // Test TriageCompareCosDistances.
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, -1, 0), new S2Point(-1, 1, 3e-15), 1, Precision.DOUBLE, CosDistances);
            TestCompareDistances(new S2Point(1, 0, 0), new S2Point(1, 1e-30, 0), new S2Point(-1, 1e-40, 0), -1, Precision.DOUBLE, CosDistances);
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, -1, 0), new S2Point(-1, 1, 3e-18), 1, Precision.LONG_DOUBLE, CosDistances);
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, -1, 0), new S2Point(-1, 1, 1e-100), 1, Precision.EXACT, CosDistances);
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, -1, 0), new S2Point(-1, 1, 0), -1, Precision.SYMBOLIC, CosDistances);
            TestCompareDistances(new S2Point(1, 1, 1), new S2Point(1, -1, 0), new S2Point(1, -1, 0), 0, Precision.SYMBOLIC, CosDistances);

            // Test TriageCompareSin2Distances using distances greater than 90 degrees.
            TestCompareDistances(new S2Point(1, 1, 0), new S2Point(-1, -1 + 1e-15, 0), new S2Point(-1, -1, 0), -1, Precision.DOUBLE, MinusSin2Distances);
            TestCompareDistances(new S2Point(-1, -1, 0), new S2Point(1, 1 - 1e-15, 0), new S2Point(1, 1 - 1e-15, 1e-21), 1, Precision.DOUBLE, MinusSin2Distances);
            TestCompareDistances(new S2Point(-1, -1, 0), new S2Point(2, 1, 0), new S2Point(2, 1, 1e-8), 1, Precision.LONG_DOUBLE, MinusSin2Distances);
            TestCompareDistances(new S2Point(-1, -1, 0), new S2Point(2, 1, 0), new S2Point(2, 1, 1e-30), 1, Precision.EXACT, MinusSin2Distances);
            TestCompareDistances(new S2Point(-1, -1, 0), new S2Point(2, 1, 0), new S2Point(1, 2, 0), -1, Precision.SYMBOLIC, MinusSin2Distances);
        }
        ...
        public void Test_CompareDistances_Consistency()
        {
            ...
            TestCompareDistancesConsistency(new S2Point(1, 0, 0), new S2Point(0, -1, 0), new S2Point(0, 1, 0), CosDistances);
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                S1Angle r = S1Angle.FromRadians(S2Constants.M_PI_2 * Math.Pow(1e-30, S2Testing.Random.RandDouble()));
                if (S2Testing.Random.OneIn(2)) r = S1Angle.FromRadians(S2Constants.M_PI_2) - r;
                if (S2Testing.Random.OneIn(2)) r = S1Angle.FromRadians(S2Constants.M_PI_2) + r;
                S2Point a = S2EdgeDistances.InterpolateAtDistance(r, x, dir);
                S2Point b = S2EdgeDistances.InterpolateAtDistance(r, x, -dir);
                ...
                if (r.Radians < S2Constants.M_PI_2 - 1e-14)
                {
                ...
                else if (r.Radians > S2Constants.M_PI_2 + 1e-14)
                {
        ...
        public void Test_CompareDistance_Coverage()
        {
            ...
            TestCompareDistance(new S2Point(1, 1, 1), new S2Point(1, 1 - 1e-15, 1),
                S1ChordAngle.FromRadians(1e-15), -1, Precision.DOUBLE, Sin2Distance);
            TestCompareDistance(new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                S1ChordAngle.FromRadians(S2Constants.M_PI_4), -1, Precision.LONG_DOUBLE, Sin2Distance);
            TestCompareDistance(new S2Point(1, 1e-40, 0), new S2Point(1 + S2Constants.DoubleEpsilon, 1e-40, 0),
                S1ChordAngle.FromRadians(0.9 * S2Constants.DoubleEpsilon * 1e-40), 1, Precision.EXACT, Sin2Distance);
            TestCompareDistance(new S2Point(1, 1e-40, 0), new S2Point(1 + S2Constants.DoubleEpsilon, 1e-40, 0),
                S1ChordAngle.FromRadians(1.1 * S2Constants.DoubleEpsilon * 1e-40), -1, Precision.EXACT, Sin2Distance);
            TestCompareDistance(new S2Point(1, 0, 0), new S2Point(1 + S2Constants.DoubleEpsilon, 0, 0),
                S1ChordAngle.Zero, 0, Precision.EXACT, Sin2Distance);

            // Test TriageCompareCosDistance.
            TestCompareDistance(new S2Point(1, 0, 0), new S2Point(1, 1e-8, 0),
            ...
            TestCompareDistance(new S2Point(1, 1, 0), new S2Point(1, -1 - 2 * S2Constants.DoubleEpsilon, 0),
                S1ChordAngle.Right, 1, Precision.DOUBLE, CosDistance);
            TestCompareDistance(new S2Point(1, 1, 0), new S2Point(1, -1 - S2Constants.DoubleEpsilon, 0),
                S1ChordAngle.Right, 1, Precision.LONG_DOUBLE, CosDistance);
            TestCompareDistance(new S2Point(1, 1, 0), new S2Point(1, -1, 1e-30),
                S1ChordAngle.Right, 0, Precision.EXACT, CosDistance);
            // The angle between these two points is exactly 60 degrees.
            TestCompareDistance(new S2Point(1, 1, 0), new S2Point(0, 1, 1),
        ...
        public void Test_CompareDistance_Consistency()
        {
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                S1Angle r = S1Angle.FromRadians(S2Constants.M_PI_2 * Math.Pow(1e-30, S2Testing.Random.RandDouble()));
                if (S2Testing.Random.OneIn(2)) r = S1Angle.FromRadians(S2Constants.M_PI_2) - r;
                if (S2Testing.Random.OneIn(5)) r = S1Angle.FromRadians(S2Constants.M_PI_2) + r;
        ...
        public void Test_CompareEdgeDistance_Coverage()
        {
            ...
            TestCompareEdgeDistance(
                new S2Point(1, 1e-10, 1e-15), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.FromRadians(1e-15 + S2Constants.DoubleEpsilon), -1, Precision.DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1, 1, 1e-15), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.FromRadians(1e-15 + S2Constants.DoubleEpsilon), -1, Precision.LONG_DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1, 1, 1e-40), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.FromRadians(1e-40), -1, Precision.EXACT);
            TestCompareEdgeDistance(
                new S2Point(1, 1, 0), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.Zero, 0, Precision.EXACT);

            // Test TriageCompareLineCos2Distance.
            TestCompareEdgeDistance(
                new S2Point(1e-15, 0, 1), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.FromRadians(S2Constants.M_PI_2 - 1e-15 - 3 * S2Constants.DoubleEpsilon),
                1, Precision.DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1e-15, 0, 1), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.FromRadians(S2Constants.M_PI_2 - 1e-15 - S2Constants.DoubleEpsilon),
                1, Precision.LONG_DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1e-40, 0, 1), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.Right, -1, Precision.EXACT);
            TestCompareEdgeDistance(
                new S2Point(0, 0, 1), new S2Point(1, 0, 0), new S2Point(0, 1, 0),
                S1ChordAngle.Right, 0, Precision.EXACT);

            // Test cases where the closest point is an edge endpoint.
            TestCompareEdgeDistance(
                new S2Point(1e-15, -1, 0), new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                S1ChordAngle.Right, -1, Precision.DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1e-18, -1, 0), new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                S1ChordAngle.Right, -1, Precision.LONG_DOUBLE);
            TestCompareEdgeDistance(
                new S2Point(1e-100, -1, 0), new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                S1ChordAngle.Right, -1, Precision.EXACT);
            TestCompareEdgeDistance(
                new S2Point(0, -1, 0), new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                S1ChordAngle.Right, 0, Precision.EXACT);
        }
        ...
        public void Test_CompareEdgeDistance_Consistency()
        {
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                S2Point a1 = S2EdgeDistances.InterpolateAtDistance(len, a0, ChoosePoint());
            ...
            static Precision TestCompareEdgeDistanceConsistency(S2Point x, S2Point a0, S2Point a1, S1ChordAngle r)
            {
                ...
                int ld_sign = S2Pred.TriageCompareEdgeDistance_Test(ToLD(x), ToLD(a0), ToLD(a1), ToLD(r.Length2));
        ...
        {
            TestCompareEdgeDirections(new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                                      new S2Point(1, -1, 0), new S2Point(1, 0, 0),
                                      1, Precision.DOUBLE);
            TestCompareEdgeDirections(new S2Point(1, 0, 1.5e-15), new S2Point(1, 1, 0),
                                      new S2Point(0, -1, 0), new S2Point(0, 0, 1),
                                      1, Precision.DOUBLE);
            TestCompareEdgeDirections(new S2Point(1, 0, 1e-18), new S2Point(1, 1, 0),
                                      new S2Point(0, -1, 0), new S2Point(0, 0, 1),
                                      1, Precision.LONG_DOUBLE);
            TestCompareEdgeDirections(new S2Point(1, 0, 1e-50), new S2Point(1, 1, 0),
                                      new S2Point(0, -1, 0), new S2Point(0, 0, 1),
                                      1, Precision.EXACT);
            TestCompareEdgeDirections(new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                                      new S2Point(0, -1, 0), new S2Point(0, 0, 1),
        ...
        public void Test_CompareEdgeDirections_Consistency()
        {
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                S2Point a1 = S2EdgeDistances.InterpolateAtDistance(a_len, a0, ChoosePoint());
                S2Point a_norm = S2PointUtil.RobustCrossProd(a0, a1).Normalized;
                S2Point b0 = ChoosePoint();
                S1Angle b_len = S1Angle.FromRadians(Math.PI * Math.Pow(1e-20, S2Testing.Random.RandDouble()));
                S2Point b1 = S2EdgeDistances.InterpolateAtDistance(b_len, b0, a_norm);
            ...
            static Precision TestCompareEdgeDirectionsConsistency(S2Point a0, S2Point a1, S2Point b0, S2Point b1)
            {
                ...
                int ld_sign = S2Pred.TriageCompareEdgeDirections_Test(ToLD(a0), ToLD(a1), ToLD(b0), ToLD(b1));
                int exact_sign = S2Pred.ExactCompareEdgeDirections_Test(ToExact(a0), ToExact(a1), ToExact(b0), ToExact(b1));
        ...
        public void Test_EdgeCircumcenterSign_Coverage()
        {
            TestEdgeCircumcenterSign(
                new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                new S2Point(0, 0, 1), new S2Point(1, 0, 1), new S2Point(0, 1, 1),
                1, Precision.DOUBLE);
            TestEdgeCircumcenterSign(
                new S2Point(1, 0, 0), new S2Point(1, 1, 0),
                new S2Point(0, 0, -1), new S2Point(1, 0, -1), new S2Point(0, 1, -1),
            ...
            TestEdgeCircumcenterSign(
                ...
                new S2Point(1, -1e-5, 1), new S2Point(1, 1e-5, -1), new S2Point(1, 1 - 1e-5, 1e-5),
                -1, Precision.DOUBLE);
            TestEdgeCircumcenterSign(
                new S2Point(1, -1, 0), new S2Point(1, 1, 0),
                new S2Point(1, -1e-5, 1), new S2Point(1, 1e-5, -1), new S2Point(1, 1 - 1e-9, 1e-5),
                -1, Precision.LONG_DOUBLE);
            TestEdgeCircumcenterSign(
                new S2Point(1, -1, 0), new S2Point(1, 1, 0),
                new S2Point(1, -1e-5, 1), new S2Point(1, 1e-5, -1), new S2Point(1, 1 - 1e-15, 1e-5),
                -1, Precision.EXACT);
            TestEdgeCircumcenterSign(
                new S2Point(1, -1, 0), new S2Point(1, 1, 0),
                new S2Point(1, -1e-5, 1), new S2Point(1, 1e-5, -1), new S2Point(1, 1, 1e-5),
                1, Precision.SYMBOLIC);

            // This test falls back to the second symbolic perturbation:
            TestEdgeCircumcenterSign(
                new S2Point(1, -1, 0), new S2Point(1, 1, 0),
                new S2Point(0, -1, 0), new S2Point(0, 0, -1), new S2Point(0, 0, 1),
                -1, Precision.SYMBOLIC);

            // This test falls back to the third symbolic perturbation:
            TestEdgeCircumcenterSign(
                new S2Point(0, -1, 1), new S2Point(0, 1, 1),
                new S2Point(0, 1, 0), new S2Point(0, -1, 0), new S2Point(1, 0, 0),
                -1, Precision.SYMBOLIC);
        }
        ...
        public void Test_EdgeCircumcenterSign_Consistency()
        {
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                S2Point z = (c0 * x0 + c1 * x1).Normalized;
                S1Angle r = S1Angle.FromRadians(Math.PI * Math.Pow(1e-30, S2Testing.Random.RandDouble()));
                S2Point a = S2EdgeDistances.InterpolateAtDistance(r, z, ChoosePoint());
                S2Point b = S2EdgeDistances.InterpolateAtDistance(r, z, ChoosePoint());
                S2Point c = S2EdgeDistances.InterpolateAtDistance(r, z, ChoosePoint());
        ...
        public void Test_VoronoiSiteExclusion_Coverage()
        {
            ...
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-5, 0), new S2Point(1, -2e-5, 0),
                new S2Point(1, 0, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-3),
                S2Pred.Excluded.SECOND, Precision.DOUBLE);

            // Both sites are closest to edge endpoint X1.
            TestVoronoiSiteExclusion(
                new S2Point(1, 1, 1e-30), new S2Point(1, 1, -1e-20),
                new S2Point(1, 0, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-10),
                S2Pred.Excluded.SECOND, Precision.DOUBLE);

            // Test cases where neither site is excluded.
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-10, 1e-5), new S2Point(1, 1e-10, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-4),
                S2Pred.Excluded.NEITHER, Precision.DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-10, 1e-5), new S2Point(1, 1e-10, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-5),
                S2Pred.Excluded.NEITHER, Precision.LONG_DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-17, 1e-5), new S2Point(1, 1e-17, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-4),
                S2Pred.Excluded.NEITHER, Precision.LONG_DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-20, 1e-5), new S2Point(1, 1e-20, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1e-5),
            ...
            TestVoronoiSiteExclusion(
              new S2Point(1, -1e-6, 1.0049999999e-5), new S2Point(1, 0, -1e-5),
              new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1.005e-5),
              S2Pred.Excluded.FIRST, Precision.DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1.00105e-6, 1.0049999999e-5), new S2Point(1, 0, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1.005e-5),
                S2Pred.Excluded.FIRST, Precision.LONG_DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-6, 1.005e-5), new S2Point(1, 0, -1e-5),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1.005e-5),
                S2Pred.Excluded.FIRST, Precision.LONG_DOUBLE);
            TestVoronoiSiteExclusion(
                new S2Point(1, -1e-31, 1.005e-30), new S2Point(1, 0, -1e-30),
                new S2Point(1, -1, 0), new S2Point(1, 1, 0), S1ChordAngle.FromRadians(1.005e-30),
            ...
            TestVoronoiSiteExclusion(
                new S2Point(0, 1, 1), new S2Point(1, 0, 1),
                new S2Point(0, 1, 1), new S2Point(1, 0, -1), S1ChordAngle.FromLength2(1),
                S2Pred.Excluded.NEITHER, Precision.EXACT);
        ...
        public void Test_VoronoiSiteExclusion_Consistency()
        {
            ...
            for (int iter = 0; iter < consistency_iters; ++iter)
            {
                ...
                if (x0 == -x1) continue;  // Not allowed by API.
                double f = Math.Pow(1e-20, S2Testing.Random.RandDouble());
                S2Point p = ((1 - f) * x0 + f * x1).Normalized;
                S1Angle r1 = S1Angle.FromRadians(S2Constants.M_PI_2 * Math.Pow(1e-20, S2Testing.Random.RandDouble()));
                S2Point a = S2EdgeDistances.InterpolateAtDistance(r1, p, ChoosePoint());
                S2Point b = S2EdgeDistances.InterpolateAtDistance(r1, p, ChoosePoint());
                // Check that the other API requirements are met.
                S1ChordAngle r = new S1ChordAngle(r1);
            ...
            static Precision TestVoronoiSiteExclusionConsistency(S2Point a, S2Point b, S2Point x0, S2Point x1, S1ChordAngle r)
            {
                ...
                S2Pred.Excluded ld_result = S2Pred.TriageVoronoiSiteExclusion_Test(ToLD(a), ToLD(b), ToLD(x0), ToLD(x1), ToLD(r.Length2));
        ...
        private static void TestVoronoiSiteExclusion(S2Point a, S2Point b, S2Point x0, S2Point x1, S1ChordAngle r, S2Pred.Excluded expected_result, Precision expected_prec)
        {
            ...
            if (!a.IsUnitLength) a = a.Normalized;
            if (!b.IsUnitLength) b = b.Normalized;
            if (!x0.IsUnitLength) x0 = x0.Normalized;
            if (!x1.IsUnitLength) x1 = x1.Normalized;
        ...
        private static void TestEdgeCircumcenterSign(S2Point x0, S2Point x1, S2Point a, S2Point b, S2Point c, int expected_sign, Precision expected_prec)
        {
            ...
            if (!a.IsUnitLength) a = a.Normalized;
            if (!b.IsUnitLength) b = b.Normalized;
            if (!c.IsUnitLength) c = c.Normalized;
        ...
        private static void TestCompareEdgeDirections(S2Point a0, S2Point a1, S2Point b0, S2Point b1, int expected_sign, Precision expected_prec)
        {
            // Don't normalize the arguments unless necessary (to allow testing points
            // that differ only in magnitude).
            if (!a0.IsUnitLength) a0 = a0.Normalized;
            if (!a1.IsUnitLength) a1 = a1.Normalized;
            if (!b0.IsUnitLength) b0 = b0.Normalized;
            if (!b1.IsUnitLength) b1 = b1.Normalized;
        ...
        private static void TestCompareEdgeDistance(S2Point x, S2Point a0, S2Point a1, S1ChordAngle r, int expected_sign, Precision expected_prec)
        {
            // Don't normalize the arguments unless necessary (to allow testing points
            // that differ only in magnitude).
            if (!x.IsUnitLength) x = x.Normalized;
            if (!a0.IsUnitLength) a0 = a0.Normalized;
            if (!a1.IsUnitLength) a1 = a1.Normalized;
        ...
        private static Precision TestCompareDistanceConsistency(S2Point x, S2Point y, S1ChordAngle r, Triage triage)
        {
            ...
            int ld_sign = triage(ToLD(x), ToLD(y), r);
            int exact_sign = S2Pred.ExactCompareDistance_Test(ToExact(x), ToExact(y), r.Length2);
        ...
        private static void TestCompareDistance(S2Point x, S2Point y, S1ChordAngle r, int expected_sign, Precision expected_prec, Triage triage)
        {
            ...
            // that differ only in magnitude).
            if (!x.IsUnitLength) x = x.Normalized;
            if (!y.IsUnitLength) y = y.Normalized;
        ...
        private static Precision TestCompareDistancesConsistency(S2Point x, S2Point a, S2Point b, TriagePoints triage)
        {
            ...
            int ld_sign = triage(ToLD(x), ToLD(a), ToLD(b));
            int exact_sign = S2Pred.ExactCompareDistances_Test(ToExact(x), ToExact(a), ToExact(b));
            if (dbl_sign != 0) Assert.Equal(ld_sign, dbl_sign);
            if (ld_sign != 0) Assert.Equal(exact_sign, ld_sign);
            if (exact_sign != 0)
            {
        ...
        private static S2Point ToLD(S2Point p) => p;
        private static double ToLD(double p) => p;
        private static S2Point ToExact(S2Point p) => p;
        ...
        private static void TestGreatCircle(S2Point a, S2Point b, int n)
        {
            a = a.Normalized;
            b = b.Normalized;
            var points = new List<S2Point> { a, b };
                ...
                switch (S2Testing.Random.Uniform(8))
                {
                    case 0:
                        ...
                        points.Add((S2Testing.Random.UniformDouble(-1, 1) * a +
                                      S2Testing.Random.UniformDouble(-1, 1) * b).Normalized);
                        break;
                    case 1:
                    ...
                    case 4:
                        {
                            ...
                            S2Point dir = new S2Point(0, 0, 0);
                            dir = dir.SetAxis(coord, S2Testing.Random.OneIn(2) ? 1 : -1);
                            var norm = S2PointUtil.RobustCrossProd(a, b).Normalized;
                            if (norm.Norm2 > 0)
                            {
                                points.Add(S2PointUtil.RobustCrossProd(dir, norm).Normalized);
                            }
                            break;
                        }
                    case 5:
        ...
        {
            S2Point dir = S2PointUtil.RobustCrossProd(a, b).CrossProd(a).Normalized;
        ...
        private static S2Point ChoosePoint()
        {
            ...
            return new S2Point(x).Normalized;
        }
        ...
        public static class StableSignTest
        {
            ...
            public static double GetFailureRate(double km)
            {
                ...
                for (int iter = 0; iter < kIters; ++iter)
                {
                    ...
                    S2Point b = (a - m * x).Normalized;
                    S2Point c = (a + m * x).Normalized;
