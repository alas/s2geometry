    ...
    public class S2CrossingEdgeQueryTests
    {
        ...
        public void Test_GetCrossingCandidates_PerturbedCubeEdges()
        {
            ...
            for (int iter = 0; iter < 10; ++iter)
            {
                ...
                double scale = Math.Pow(S2Constants.DoubleError, S2Testing.Random.RandDouble());
                R2Point uv = new(2 * S2Testing.Random.Uniform(2) - 1, 2 * S2Testing.Random.Uniform(2) - 1);  // vertex
                S2Point a0 = S2Coords.FaceUVtoXYZ(face, scale * uv);
                S2Point b0 = a0 - 2 * S2Coords.GetNorm(face);
        ...
        public void Test_GetCrossingCandidates_PerturbedCubeFaceAxes()
        {
            ...
            for (int iter = 0; iter < 5; ++iter)
            {
                int face = S2Testing.Random.Uniform(6);
                double scale = Math.Pow(S2Constants.DoubleError, S2Testing.Random.RandDouble());
                S2Point axis = S2Coords.GetUVWAxis(face, S2Testing.Random.Uniform(2));
                S2Point a0 = scale * axis + S2Coords.GetNorm(face);
                S2Point b0 = scale * axis - S2Coords.GetNorm(face);
                GetPerturbedSubEdges(a0, b0, 30, edges);
                TestAllCrossings(edges);
            }
        }
        ...
        public void Test_GetCrossingCandidates_CapEdgesNearCubeVertex()
        {
            ...
            List<(S2Point, S2Point)> edges = new();
            GetCapEdges(new S2Cap(new S2Point(-1, -1, 1).Normalized, S1Angle.FromRadians(1e-3)),
        ...
        public void Test_GetCrossingCandidates_DegenerateEdgeOnCellVertexIsItsOwnCandidate()
        {
            for (int i = 0; i < 100; ++i)
            {
                ...
                edges.Add((cell.GetVertex(0), cell.GetVertex(0)));
                TestAllCrossings(edges);
            }
        }
        ...
        public void Test_GetCrossingCandidates_CollinearEdgesOnCellBoundaries()
        {
            ...
            for (int level = 0; level <= S2Constants.kMaxCellLevel; ++level)
            {
                ...
                S2Point p1 = cell.GetVertexRaw(j2);
                S2Point p2 = cell.GetVertexRaw(j2 + 1);
                ...
                for (int i = 0; i <= kNumEdgeIntervals; ++i)
                {
                    for (int j = 0; j < i; ++j)
                    {
                        edges.Add(((p1 + i * delta).Normalized,
                                                       (p1 + j * delta).Normalized));
                    }
                }
                TestAllCrossings(edges);
            }
        ...
        public void Test_VisitCells_QueryEdgeOnFaceBoundary()
        {
            ...
            for (int iter = 0; iter < kIters; ++iter)
            {
                ...
                do
                {
                    ...
                    a = S2Coords.FaceUVtoXYZ(a_face, S2Testing.Random.UniformDouble(-1, 1),
                                        S2Testing.Random.UniformDouble(-1, 1)).Normalized;
                    b_face = S2Coords.GetUVWFace(a_face, 0, 1);  // Towards positive u-axis
                    var uTmp = 1 - S2Testing.Random.Uniform(2) * 0.5 * S2Constants.DoubleEpsilon;
                    b = S2Coords.FaceUVtoXYZ(b_face, uTmp, S2Testing.Random.UniformDouble(-1, 1)).Normalized;
                } while (S2Coords.GetFace(b) != b_face ||
                ...
                S2Point c = S2Coords.FaceUVtoXYZ(b_face, S2Testing.Random.UniformDouble(-1, 1),
                                            S2Testing.Random.UniformDouble(-1, 1)).Normalized;
                MutableS2ShapeIndex index = new();
                index.Add(new S2Polyline.OwningShape(
        ...
        {
            S2Point x = S2EdgeDistances.InterpolateAtDistance(distance, a0, b0);
        ...
        private static void GetPerturbedSubEdges(S2Point a0, S2Point b0, int count, List<(S2Point, S2Point)> edges)
        {
            ...
            {
                S1Angle length = length0 * Math.Pow(S2Constants.DoubleError, S2Testing.Random.RandDouble());
        ...
        private static void GetCapEdges(S2Cap center_cap, S1Angle max_length, int count, List<(S2Point, S2Point)> edges)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                S2Point p2 = (2 * p1.DotProd(center) * center - p1).Normalized;
                edges.Add(new(p1, p2));
            }
        }
        ...
        private void TestAllCrossings(List<(S2Point, S2Point)> edges)
        {
            ...
            foreach (var edge in edges)
            {
                ...
                for (int i = 0; i < shape.NumEdges; ++i)
                {
                    ...
                    S2Point c = edge2.V0;
                    S2Point d = edge2.V1;
                    int sign = S2EdgeCrossings.CrossingSign(a, b, c, d);
                    if (sign >= 0)
                    {
                    ...
                    {
                        double kMaxDist = S2Metrics.kMaxDiag.GetValue(S2Constants.kMaxCellLevel);
        ...
        private static void TestPolylineCrossings(S2ShapeIndex index, S2Point a0, S2Point a1)
        {
            ...
            foreach (var edge in edges)
            {
                Assert.True(S2EdgeCrossings.CrossingSign(a0, a1, edge.V0, edge.V1) >= 0);
            }
