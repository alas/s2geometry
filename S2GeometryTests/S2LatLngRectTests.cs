...
namespace S2Geometry
{
    public class S2LatLngRectTests
    {
        ...
        public void Test_S2LatLngRect_Accessors()
        {
            ...
            Assert2.Near(d1.LatLo.Degrees, -90);
            Assert2.Near(d1.LatHi.Degrees, -45);
            Assert2.Near(d1.LngLo.Degrees, 0);
            Assert2.Near(d1.LngHi.Degrees, 180);
            Assert.Equal(d1.Lat, new R1Interval(-S2Constants.M_PI_2, -S2Constants.M_PI_4));
            Assert.Equal(d1.Lng, new S1Interval(0, Math.PI));
        }
        ...
        {
            S2LatLngRect r1 = new S2LatLngRect(new R1Interval(0, S2Constants.M_PI_2), new S1Interval(-Math.PI, 0));
            Assert.Equal(r1.GetCenter(), S2LatLng.FromRadians(S2Constants.M_PI_4, -S2Constants.M_PI_2));
            Assert.Equal(r1.GetSize(), S2LatLng.FromRadians(S2Constants.M_PI_2, Math.PI));
            Assert.True(S2LatLngRect.Empty.GetSize().LatRadians < 0);
            Assert.True(S2LatLngRect.Empty.GetSize().LngRadians < 0);
        }
        ...
        public void Test_S2LatLngRect_GetVertex()
        {
            ...
            Assert.Equal(r1.GetVertex(2), S2LatLng.FromRadians(S2Constants.M_PI_2, 0));
            Assert.Equal(r1.GetVertex(3), S2LatLng.FromRadians(S2Constants.M_PI_2, Math.PI));

            // Make sure that GetVertex() returns vertices in CCW order.
            for (int i = 0; i < 4; ++i) {
                double lat = S2Constants.M_PI_4 * (i - 2);
                double lng = S2Constants.M_PI_2 * (i - 2) + 0.2;
                S2LatLngRect r = new S2LatLngRect(new R1Interval(lat, lat + S2Constants.M_PI_4), new S1Interval(Math.IEEERemainder(lng, S2Constants.M_2_PI),
                              Math.IEEERemainder(lng + S2Constants.M_PI_2, S2Constants.M_2_PI)));
                for (int k = 0; k < 4; ++k) {
                    Assert.Equal(1, S2Pred.Sign(r.GetVertex(k - 1).ToPoint(),
        ...
        public void Test_S2LatLngRect_Contains()
        {
            ...
            S2LatLng north_pole = S2LatLng.FromRadians(S2Constants.M_PI_2, 0);
        ...
        public void Test_BoundaryIntersects_RectCrossingAntiMeridian() {
            ...
            Assert.True(rect.Contains(S2TextFormat.MakePointOrDie("30:180")));
        ...
        public void Test_S2LatLngRect_AddPoint() {
            ...
            p = p.AddPoint(S2LatLng.FromRadians(0, -S2Constants.M_PI_2));
            Assert.False(p.IsPoint);
            p = p.AddPoint(S2LatLng.FromRadians(S2Constants.M_PI_4, -Math.PI));
        ...
        public void Test_S2LatLngRect_CellOps() {
            ...
            TestCellOps(RectFromDegrees(v0.Lat.Degrees - 1e-8,
                                        ...
                                        v0.Lng.Degrees + 1e-10), cell0tr, 1);
            ...
            TestCellOps(RectFromDegrees(bound202.Lo.Lat.Degrees + 3,
                                        ...
                                        bound202.Hi.Lng.Degrees - 3), cell202, 2);
        }
        ...
        public void Test_S2LatLngRect_EncodeDecode() {
            ...
            var (success, decoded_rect) = S2LatLngRect.DecodeStatic(decoder);
            Assert.True(success);
        ...
        public void Test_S2LatLngRect_Area() {
            ...
            Assert2.Near(S2Constants.M_4_PI, S2LatLngRect.Full.Area());
            Assert2.Near(S2Constants.M_PI_2, RectFromDegrees(0, 0, 90, 90).Area());
        }
        ...
        public void Test_S2LatLngRect_GetCentroid() {
            ...
            for (int i = 0; i < 100; ++i) {
                double lat1 = S2Testing.Random.UniformDouble(-S2Constants.M_PI_2, S2Constants.M_PI_2);
                double lat2 = S2Testing.Random.UniformDouble(-S2Constants.M_PI_2, S2Constants.M_PI_2);
                S2LatLngRect r = new S2LatLngRect(R1Interval.FromPointPair(lat1, lat2), S1Interval.Full);
                S2Point centroid = r.Centroid;
                Assert2.Near(0.5 * (Math.Sin(lat1) + Math.Sin(lat2)) * r.Area(), centroid.Z, S2Constants.DoubleError);
                Assert.True(new R2Point(centroid.X, centroid.Y).Norm <= 1e-15);
            }
            ...
            for (int i = 0; i < 100; ++i) {
                ...
                S2LatLngRect r = new S2LatLngRect(S2LatLngRect.FullLat,
                               S1Interval.FromPointPair(lng1, lng2));
                S2Point centroid = r.Centroid;
                Assert.True(Math.Abs(centroid.Z) <= 1e-15);
                Assert2.Near(r.Lng.Center, new S2LatLng(centroid).LngRadians, S2Constants.DoubleError);
                double alpha = 0.5 * r.Lng.Length;
                // TODO(Alas): the next Assert fails sometimes
                Assert2.Near(0.25 * Math.PI * Math.Sin(alpha) / alpha * r.Area(),
                            new R2Point(centroid.X, centroid.Y).Norm, S2Constants.DoubleError);
            }
        ...
        public void Test_S2LatLngRect_GetDistanceRandomPairs() {
            ...
            for (int i = 0; i < 10000; ++i) {
                ...
                S2LatLng c = new S2LatLng(S2Testing.RandomPoint());
        ...
        private static void VerifyGetDirectedHausdorffDistance(S2LatLngRect a, S2LatLngRect b)
        {
            ...
            int sample_size_on_lat =
                (int)(a.Lat.Length / kResolution) + 1;
            int sample_size_on_lng =
                (int)(a.Lng.Length / kResolution) + 1;
            double delta_on_lat = a.Lat.Length / sample_size_on_lat;
            double delta_on_lng = a.Lng.Length / sample_size_on_lng;
        ...
        private static S1Angle BruteForceDistance(S2LatLngRect a, S2LatLngRect b) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                for (int j = 0; j < 2; ++j) {
                    ...
                    S1Angle a_to_lng = S2EdgeDistances.GetDistance(current_a.ToPoint(), lng_edge_b[j][0], lng_edge_b[j][1]);
                    S1Angle b_to_lng = S2EdgeDistances.GetDistance(current_b.ToPoint(), lng_edge_a[j][0], lng_edge_a[j][1]);
        ...
        private static S1Angle BruteForceRectPointDistance(S2LatLngRect a, S2LatLng b) {
            ...
            S1Angle b_to_lo_lng = S2EdgeDistances.GetDistance(b.ToPoint(), new S2LatLng(a.LatLo, a.LngLo).ToPoint(), new S2LatLng(a.LatHi, a.LngLo).ToPoint());
            S1Angle b_to_hi_lng = S2EdgeDistances.GetDistance(b.ToPoint(), new S2LatLng(a.LatLo, a.LngHi).ToPoint(), new S2LatLng(a.LatHi, a.LngHi).ToPoint());
