...
namespace S2Geometry
{
    public class S2LoopTests
    {
        ...
        private readonly List<S2Loop> all_loops = new List<S2Loop>();
        ...
        public S2LoopTests(ITestOutputHelper logger)
        {
            ...
            var snapped_loop_a_vertices = new S2Point[]{
                new S2CellId(S2TextFormat.MakePointOrDie("0:178")).ToPoint(),
                new S2CellId(S2TextFormat.MakePointOrDie("-1:180")).ToPoint(),
                new S2CellId(S2TextFormat.MakePointOrDie("0:-179")).ToPoint(),
                new S2CellId(S2TextFormat.MakePointOrDie("1:-180")).ToPoint()};
        ...
        public void Test_S2LoopTestBase_GetRectBound() {
            ...
            Assert.True(candy_cane_.GetRectBound().LatLo.Degrees < -20);
            Assert.True(candy_cane_.GetRectBound().LatHi.Degrees > 10);
            ...
            var arctic_80_inv = (S2Loop)arctic_80_.Clone();
            ...
            Assert.True(south_hemi_.GetRectBound().Lat.ApproxEquals(
                new R1Interval(-S2Constants.M_PI_2, 0), kRectError.LatRadians));
        }
        ...
        public void Test_S2LoopTestBase_AreaConsistentWithCurvature() {
            ...
            foreach (var loop in all_loops) {
                ...
                var gauss_area = S2Constants.M_2_PI - loop.GetCurvature();
                ...
                do {
                    ...
                    for (int i2 = 0; i2 < num_vertices; ++i2) {
                        ...
                        vertices.Add(S2LatLng.FromRadians(0,
                            S2Testing.Random.RandDouble() * S2Constants.M_PI_2).ToPoint());
                    }
                    loop = new S2Loop(vertices, false);
                } while (!loop.IsValid);
                bool ccw = loop.IsNormalized();
                _logger.WriteLine("loop: " + loop.ToDebugString());
                Assert2.Near(ccw ? 0 : S2Constants.M_4_PI, loop.GetArea(), S2Constants.DoubleError);
        ...
        public void Test_S2LoopTestBase_GetAreaAndCentroid() {
            Assert.Equal(0.0, empty_.GetArea());
            Assert.Equal(S2Constants.M_4_PI, full_.GetArea());
            Assert.Equal(S2Point.Empty, empty_.GetCentroid());
            Assert.Equal(S2Point.Empty, full_.GetCentroid());

            Assert2.Near(north_hemi_.GetArea(), S2Constants.M_2_PI);
            Assert2.Near(east_hemi_.GetArea(), S2Constants.M_2_PI, 1e-15);
        ...
        public void Test_S2LoopTestBase_GetCurvature() {
            Assert.Equal(S2Constants.M_2_PI, empty_.GetCurvature());
            Assert.Equal(-S2Constants.M_2_PI, full_.GetCurvature());

            Assert2.Near(0, north_hemi3_.GetCurvature(), 1e-15);
            CheckCurvatureInvariants(north_hemi3_);

            Assert2.Near(0, west_hemi_.GetCurvature(), 1e-15);
            ...
            Assert2.Near(S2Constants.M_2_PI, line_triangle_.GetCurvature());
            CheckCurvatureInvariants(line_triangle_);

            Assert2.Near(S2Constants.M_2_PI, skinny_chevron_.GetCurvature());
            ...
            for (int i = 0; i < kArmPoints; ++i) {
                ...
                vertices[kArmPoints - i - 1] = new S2Point(r1 * x, r1 * y, 1).Normalized;
                vertices[kArmPoints + i] = new S2Point(r2 * x, r2 * y, 1).Normalized;
            }
            // This is a pathological loop that contains many long parallel edges, and
            // takes tens of seconds to validate in debug mode.
            S2Loop spiral = new S2Loop(vertices, false);
            ...
            Assert2.Near(S2Constants.M_2_PI - spiral.GetArea(), spiral.GetCurvature(),
        ...
        public void Test_S2LoopTestBase_Contains() {
            ...
            var north_copy = (S2Loop)north_hemi_.Clone();
            var south_copy = (S2Loop)south_hemi_.Clone();
            var west_copy = (S2Loop)west_hemi_.Clone();
            var east_copy = (S2Loop)east_hemi_.Clone();
            for (int i = 0; i < 4; ++i) {
        ...
        public void Test_S2Loop_ContainsMatchesCrossingSign() {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                points[i] = children[i].GetCenter().Normalized;
            }
            ...
            // |               |               |
            // +---------------+---------------+
            S2Cell grandchild_cell = new S2Cell(cell_id.Child(0).Child(2));
            S2Point a0 = grandchild_cell.GetVertex(0);
            ...
            Assert.Equal(-1, S2EdgeCrossings.CrossingSign(a0, S2PointUtil.Origin, loop.Vertex(0), loop.Vertex(1)));
            Assert.Equal(1, S2EdgeCrossings.CrossingSign(a0, S2PointUtil.Origin, loop.Vertex(1), loop.Vertex(2)));
            Assert.Equal(-1, S2EdgeCrossings.CrossingSign(a0, S2PointUtil.Origin, loop.Vertex(2), loop.Vertex(3)));
            Assert.Equal(-1, S2EdgeCrossings.CrossingSign(a0, S2PointUtil.Origin, loop.Vertex(3), loop.Vertex(4)));

            // Contains should return false for the origin, and true for a0.
            Assert.False(loop.Contains(S2PointUtil.Origin));
        ...
        public void Test_S2Loop_LoopRelations2() {
            ...
            for (int iter = 0; iter < 1000; ++iter) {
                S2CellId begin = new S2CellId(S2Testing.Random.Rand64() | 1);
                if (!begin.IsValid) continue;
                begin = begin.Parent(S2Testing.Random.Uniform(S2Constants.kMaxCellLevel));
        ...
        public void Test_S2Loop_BoundsForLoopContainment() {
            ...
            for (int iter = 0; iter < 1000; ++iter) {
                ...
                var b = (S2Testing.RandomPoint() + new S2Point(0, 0, 1)).Normalized;
                var v = b.CrossProd(new S2Point(0, 0, 1)).Normalized;
                var a = S2EdgeDistances.Interpolate(S2Testing.Random.RandDouble(), -v, b);
                var c = S2EdgeDistances.Interpolate(S2Testing.Random.RandDouble(), b, v);
                if (S2Pred.Sign(a, b, c) < 0) {
                    --iter; continue;
                }
                // Now construct another point D directly below B, and create two loops
                // ABCD and ACD.
                S2Point d = new S2Point(b.X, b.Y, 0).Normalized;
                var vertices = new S2Point[] { c, d, a, b };  // Reordered for convenience
                S2Loop outer = new S2Loop(vertices.Take(4));
                S2Loop inner = new S2Loop(vertices.Take(3));
        ...
        public void Test_S2Loop_EncodeDecode() {
            var l = S2TextFormat.MakeLoopOrDie("30:20, 40:20, 39:43, 33:35");
            l.Depth = 3;
            TestEncodeDecode(l);
            ...
            S2Loop uninitialized = new S2Loop(Array.Empty<S2Point>());
        ...
        public void Test_S2Loop_EncodeDecodeWithinScope() {
            S2Loop l = S2TextFormat.MakeLoopOrDie("30:20, 40:20, 39:43, 33:35");
            l.Depth = 3;
            Encoder encoder = new Encoder();
            l.Encode(encoder);
            Decoder decoder1 = new Decoder(encoder.Buffer, 0, encoder.Length);

            // Initialize the loop using DecodeWithinScope and check that it is the
            // same as the original loop.
            var (success1, loop1) = S2Loop.DecodeStatic(decoder1);
            ...
            Decoder decoder2 = new Decoder(encoder.Buffer, 0, encoder.Length);
            var (success2, loop2) = S2Loop.DecodeStatic(decoder2);
            ...
            var l2 = S2TextFormat.MakeLoopOrDie("30:40, 40:75, 39:43, 80:35");
            l2.Depth = 2;
            Encoder encoder2 = new Encoder();
            l2.Encode(encoder2);
            Decoder decoder3 = new Decoder(encoder2.Buffer, 0, encoder2.Length);
            var (success3, loop3) = S2Loop.DecodeStatic(decoder3);
            Assert.True(success3);
            Decoder decoder4 = new Decoder(encoder.Buffer, 0, encoder.Length);
            var (success4, loop4) = S2Loop.DecodeStatic(decoder4);
        ...
        public void Test_S2LoopTestBase_FourVertexCompressedLoopRequires36Bytes() {
            Encoder encoder = new Encoder();
            TestEncodeCompressed(snapped_loop_a_, S2Constants.kMaxCellLevel, encoder);
        ...
        public void Test_S2LoopTestBase_CompressedEncodedLoopDecodesApproxEqual() {
            var loop = (S2Loop)snapped_loop_a_.Clone();
            loop.Depth = 3;

            Encoder encoder = new Encoder();
            TestEncodeCompressed(loop, S2Constants.kMaxCellLevel, encoder);
            TestDecodeCompressed(encoder, S2Constants.kMaxCellLevel, out var decoded_loop);
        ...
        public void Test_S2Loop_S2CellConstructorAndContains() {
            S2Cell cell = new S2Cell(new S2CellId(S2LatLng.FromE6(40565459, -74645276)));
            S2Loop cell_as_loop = new S2Loop(cell);

            var vertices = new List<S2Point>();
            for (int i = 0; i < cell_as_loop.NumVertices; ++i) {
                vertices.Add(cell_as_loop.Vertex(i));
            }
            S2Loop loop_copy = new S2Loop(vertices);
        ...
        public void Test_S2LoopTestBase_DistanceMethods() {
            ...
            var square = S2TextFormat.MakeLoopOrDie("-1:-1, -1:1, 1:1, 1:-1");
        ...
        public void Test_S2LoopTestBase_MakeRegularLoop() {
            ...
            Assert2.Near(20.0, new S2LatLng(center).GetDistance(new S2LatLng(p0)).Degrees);
            Assert2.Near(20.0, new S2LatLng(center).GetDistance(new S2LatLng(p1)).Degrees);
            Assert2.Near(20.0, new S2LatLng(center).GetDistance(new S2LatLng(p2)).Degrees);
            Assert2.Near(20.0, new S2LatLng(center).GetDistance(new S2LatLng(p3)).Degrees);
            // Make sure that all angles of the polygon are the same.
            Assert2.Near(S2Constants.M_PI_2, (p1 - p0).Angle(p3 - p0));
            Assert2.Near(S2Constants.M_PI_2, (p2 - p1).Angle(p0 - p1));
            Assert2.Near(S2Constants.M_PI_2, (p3 - p2).Angle(p1 - p2));
            Assert2.Near(S2Constants.M_PI_2, (p0 - p3).Angle(p2 - p3));
            // Make sure that all edges of the polygon have the same length.
            Assert2.Near(27.990890717782829, new S2LatLng(p0).GetDistance(new S2LatLng(p1)).Degrees);
            Assert2.Near(27.990890717782829, new S2LatLng(p1).GetDistance(new S2LatLng(p2)).Degrees);
            Assert2.Near(27.990890717782829, new S2LatLng(p2).GetDistance(new S2LatLng(p3)).Degrees);
            Assert2.Near(27.990890717782829, new S2LatLng(p3).GetDistance(new S2LatLng(p0)).Degrees);

            // Check actual coordinates. This may change if we switch the algorithm
            // intentionally.
            Assert2.Near(62.162880741097204, new S2LatLng(p0).Lat.Degrees);
            Assert2.Near(103.11051028343407, new S2LatLng(p0).Lng.Degrees);
            Assert2.Near(61.955157772928345, new S2LatLng(p1).Lat.Degrees);
            Assert2.Near(165.25681963683536, new S2LatLng(p1).Lng.Degrees);
            Assert2.Near(75.139812547718478, new S2LatLng(p2).Lat.Degrees);
            Assert2.Near(-119.13042521187423, new S2LatLng(p2).Lng.Degrees);
            Assert2.Near(75.524190079054392, new S2LatLng(p3).Lat.Degrees);
            Assert2.Near(26.392175948257943, new S2LatLng(p3).Lng.Degrees);
        }

        [Fact]
        public void Test_S2LoopShape_Basic() {
            var loop = S2TextFormat.MakeLoopOrDie("0:0, 0:1, 1:0");
            S2Loop.Shape shape = new S2Loop.Shape(loop);
        ...
        private S2Loop AddLoop(string str) {
            return AddLoop(S2TextFormat.MakeLoopOrDie(str));
        }
        ...
        {
            var expected = loop.GetCurvature();
            var loop_copy = (S2Loop)loop.Clone();
            for (int i = 0; i < loop.NumVertices; ++i)
            {
                loop_copy.Invert();
                Assert.Equal(-expected, loop_copy.GetCurvature());
                loop_copy.Invert();
...
#pragma warning disable IDE0051 // Quitar miembros privados no utilizados
        private void DebugDumpCrossings(S2Loop loop)
#pragma warning restore IDE0051 // Quitar miembros privados no utilizados
        {
            ...
            for (int i = 1; i <= loop.NumVertices; ++i)
            {
                ...
                S2Point a = S2PointUtil.Ortho(o);
                S2Point b = loop.Vertex(i - 1);
                S2Point c = loop.Vertex(i + 1);
                _logger.WriteLine($"Vertex {i}: [{o.X:17g}, {o.Y:17g}, {o.Z:17g}], " +
                    $"{i - 1}{i}R={S2Pred.Sign(b, o, a)}, " +
                    $"{i + 1}{i}{i - 1}={S2Pred.Sign(c, o, b)}, " +
                    $"R{i}{i + 1}={S2Pred.Sign(a, o, c)}, inside: {S2Pred.OrderedCCW(a, b, c, o)}");
            }
            for (int i = 0; i < loop.NumVertices + 2; ++i)
            {
                S2Point orig = S2PointUtil.Origin;
                S2Point dest;
                if (i < loop.NumVertices)
                {
                    dest = loop.Vertex(i);
                    _logger.WriteLine($"Origin.{i} crosses:");
                }
                else
                {
                    dest = new S2Point(0, 0, 1);
                    if (i == loop.NumVertices + 1) orig = loop.Vertex(1);
                    _logger.WriteLine($"Case {i}:");
                }
                var sb = new StringBuilder();
                for (int j = 0; j < loop.NumVertices; ++j)
                {
                    sb.Append($" {S2EdgeCrossings.EdgeOrVertexCrossing(orig, dest, loop.Vertex(j), loop.Vertex(j + 1))}");
                }
                _logger.WriteLine(sb.ToString());
            }
            for (int i = 0; i <= 2; i += 2)
            {
                _logger.WriteLine("Origin.v1 crossing v%d.v1: ", i);
                S2Point a = S2PointUtil.Ortho(loop.Vertex(1));
                S2Point b = loop.Vertex(i);
                S2Point c = S2PointUtil.Origin;
                S2Point o = loop.Vertex(1);
                _logger.WriteLine("%d1R=%d, M1%d=%d, R1M=%d, crosses: %d\n",
                       i, S2Pred.Sign(b, o, a),
                       i, S2Pred.Sign(c, o, b),
                       S2Pred.Sign(a, o, c),
        ...
        {
            var a = S2TextFormat.MakeLoopOrDie(a_str);
            var b = S2TextFormat.MakeLoopOrDie(b_str);
        ...
        private static void CheckIdentical(S2Loop loop, S2Loop loop2)
        {
            ...
            Assert.Equal(loop.Contains(S2PointUtil.Origin), loop2.Contains(S2PointUtil.Origin));
        ...
        private static void TestEncodeDecode(S2Loop loop)
        {
            ...
            loop.Encode(encoder);
            Decoder decoder = new Decoder(encoder.Buffer, 0, encoder.Length);
            var (success, loop2) = S2Loop.DecodeStatic(decoder);
            Assert.True(success);
        ...
        private static void TestEmptyFullSnapped(S2Loop loop, int level)
        {
            ...
            S2Loop loop2 = new S2Loop(vertices);
        ...
        {
            TestEmptyFullSnapped(loop, S2Constants.kMaxCellLevel);
        ...
        {
            var loop = S2TextFormat.MakeLoopOrDie(str, false);
        ...
        {
            S2Loop l = new S2Loop(points, false);
        ...
        private static void TestDistanceMethods(S2Loop loop, S2Point x, S2Point boundary_x)
        {
            ...
            else
            {
                ...
                Assert2.Near(new S1Angle(x, boundary_x).Degrees, loop.GetDistanceToBoundary(x).Degrees, kMaxError.Degrees);
            }
            if (loop.Contains(x))
            {
