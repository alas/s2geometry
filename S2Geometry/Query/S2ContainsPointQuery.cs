...
using Options = S2Geometry.S2ContainsPointQueryOptions;
...
namespace S2Geometry
{
    ...
    public class S2ContainsPointQuery<TIndex> where TIndex : S2ShapeIndex
    {
        ...
        //
        // Note that the API allows non-access to the visited shapes.
        ...
        public bool ShapeContains(S2CellId id, S2ClippedShape clipped, S2Point p)
        {
            ...
            if (num_edges > 0)
            {
                ...
                var crosser = new S2CopyingEdgeCrosser(id.ToPoint(), p);
                for (int i = 0; i < num_edges; ++i)
                {
                    var edge = shape.GetEdge(clipped.Edge(i));
                    ...
                    if (sign == 0)
                    {
                        ...
                        sign = S2EdgeCrossings.VertexCrossing(crosser.A, crosser.B, edge.V0, edge.V1) ? 1 : 0;
                    }
    ...
    public enum S2VertexModel { OPEN, SEMI_OPEN, CLOSED }
}
