...
public class S2ConvexHullQuery
{
    ...
    public S2Loop GetConvexHull()
    {
        ...
        MyDebug.Assert(S2Pred.Sign(origin, points_.First(), points_.Last()) >= 0);
        ...
        MyDebug.Assert(lower.First() == upper.Last());
        MyDebug.Assert(lower.Last() == upper.First());
    ...
    private S2Loop GetSinglePointLoop(S2Point p)
    {
        ...
        vertices[1] = (p + kOffset * d0).Normalize();
    ...
    private S2Loop GetSingleEdgeLoop(S2Point a, S2Point b)
    {
        ...
        vertices[0] = a;
        vertices[1] = b;
        vertices[2] = S2.Interpolate(a, b, 0.5);
