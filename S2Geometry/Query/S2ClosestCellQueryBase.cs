...
using System.Linq;
using static S2Geometry.S2CellIndex;
...
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        {
            // The defaultructor yields an empty result, with a distance() of
            // Infinity() and invalid cell_id() and label() values.
            public Result()
            {
                Distance = Infinity;
                CellId = S2CellId.None;
                Label = -1;
            }

            // Constructs a Result object for the given (cell_id, label) pair.
            public Result(Distance distance, S2CellId cell_id, Int32 label)
            {
                Distance = distance; CellId = cell_id; Label = label;
            }
            ...
            {
                return HashCode.Combine(Distance, CellId, Label);
            }
            ...
            public static bool operator <(Result x, Result y)
            {
                if (x.Distance.CompareTo(y.Distance) < 0) return true;
                if (y.Distance.CompareTo(x.Distance) < 0) return false;
        ...
        private void InitCovering()
        {
            ...
            if (it.Current.StartId != last.Current.StartId)
            {
                ...
                int level = it.Current.StartId.GetCommonAncestorLevel(index_last_id) + 1;
        ...
        private bool ProcessOrEnqueue(S2CellId id, NonEmptyRangeEnumerator iter, bool seek)
        {
            ...
            var max_it = (NonEmptyRangeEnumerator)iter.Clone();
        ...
        private readonly SortedSet<Result> result_set_ = new SortedSet<Result>(); // gtl.btree_set<Result>
        ...
        private readonly Dictionary<int, int> tested_cells_ = new Dictionary<int, int>();
        ...
        private readonly SortedSet<ReverseKeyData<IDistance, S2CellId>> queue_ 
                   = new SortedSet<ReverseKeyData<IDistance, S2CellId>>();
