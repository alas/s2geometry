...
using System.Linq;
using Delta = S2Geometry.S1ChordAngle;
...
namespace S2Geometry
{
    ...
    public class S2ClosestPointQueryBase<Distance, Data> where Distance : IDistance where Data : IComparable<Data>
    {
        ...
        private readonly List<S2CellId> intersection_with_region_ = new List<S2CellId>();
        private readonly List<S2CellId> intersection_with_max_distance_ = new List<S2CellId>();
        ...
        private readonly SortedSet<Result> result_set_ = new SortedSet<Result>();
        ...
        private void FindClosestPointsBruteForce()
        {
            for (var pos = 0; pos < Index.NumPoints(); pos++)
            {
                ...
                MaybeAddResult(it.Value.Item2, it.Value.Item3);
            }
        }
        ...
        private void InitQueue()
        {
            ...
            if (Options_.MaxResults== 1)
            {
                ...
                {
                    var it_ = Index.Get(i);
                    MaybeAddResult(it_.Value.Item2, it_.Value.Item3);
                }
                var it = Index.Get(--i);
                if (it != null)
                {
            ...
            for (int i = 0; i < initial_cells.Count && pos < Index.NumPoints(); ++i)
            {
                ...
                ProcessOrEnqueue(id, id.RangeMin > Index.Get(pos)?.Item1 /*seek*/, ref pos);
            }
        }
        private void InitCovering()
        {
            ...
            var pos = 0;
            var index_last_id = Index.Get(Index.NumPoints() - 1)?.Item1;
            var index_first_id = Index.Get(0)?.Item1;
            if (index_first_id != index_last_id)
            {
                ...
                int level = index_first_id.GetCommonAncestorLevel(index_last_id) + 1;
                ...
                {
                    var current = Index.Get(pos);

                    // Skip any covering cells that don't contain any index cells.
                    if (id.RangeMax < current?.Item1) continue;
                    ...
                    var cell_first_id = current?.Item1;
                    pos = Index.Seek(id.RangeMax.Next) - 1;
                    current = Index.Get(pos);
                    S2CellId cell_last_id = current?.Item1;
                    pos++;
                    AddInitialRange(cell_first_id, cell_last_id);
                }
            }
            AddInitialRange(Index.Get(pos)?.Item1, index_last_id);
        }
        ...
        private bool ProcessOrEnqueue(S2CellId id, bool seek, ref int pos)
        {
            ...
            if (id.IsLeaf)
            {
                KeyData2<S2CellId, S2Point, Data>? cur;
                // Leaf cells can't be subdivided.
                for (; pos < Index.NumPoints() && (cur = Index.Get(pos))?.Item1 == id; pos++)
                {
                    MaybeAddResult(cur.Value.Item2, cur.Value.Item3);
                }
                return false;  // No need to seek to next child.
            }
            S2CellId last = id.RangeMax;
            int num_points = 0;
            KeyData2<S2CellId, S2Point, Data>? item;
            for (; pos < Index.NumPoints() && (item = Index.Get(pos))?.Item1 <= last; pos++)
            {
                ...
                tmp_point_data_[num_points++] = (item.Value.Item2, item.Value.Item3);
            }
        ...
        public class Result : IEquatable<Result>, IComparable<Result>
        {
            ...
            // The defaultructor creates an "empty" result, with a distance() of
