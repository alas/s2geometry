...
namespace S2Geometry
{
    ...
    public class S2ClosestEdgeQueryBase<Distance> where Distance : IDistance
    {
        ...
        private readonly List<S2CellId> index_covering_ = new List<S2CellId>();
        private readonly Array6<S2ShapeIndexCell> index_cells_ = new Array6<S2ShapeIndexCell>(() => new S2ShapeIndexCell());
        ...
        private readonly List<Result> result_vector_ = new List<Result>();
        private readonly SortedSet<Result> result_set_ = new SortedSet<Result>(); // gtl.btree_set
        ...
        private readonly Dictionary<int, int> tested_edges_ = new Dictionary<int, int>(); // gtl.dense_hash_set

        private readonly SortedSet<QueueEntry> queue_ = new SortedSet<QueueEntry>();

        // Temporaries, defined here to avoid multiple allocations / initializations.

        private readonly List<S2CellId> max_distance_covering_ = new List<S2CellId>();
        private readonly List<S2CellId> initial_cells_ = new List<S2CellId>();
        ...
        private void FindClosestEdgesInternal(S2DistanceTarget<Distance> target, Options options)
        {
            ...
            {
                var shape_ids = new SortedSet<Int32>(); // gtl.btree_set
        ...
        private void InitCovering()
        {
            ...
            var nextPos = 0;
            var next = Index.GetCellId(nextPos);
            var lastPos = Index.GetEnumerableCount() - 1;
            var last = Index.GetCellId(lastPos);
            if (next != last)
            {
                ...
                int level = next.GetCommonAncestorLevel(last) + 1;
                ...
                for (var id = next.Parent(level); id != last_id; id = id.Next)
                {
                    ...
                    nextPos = pos;
                    next = Index.GetCellId(nextPos);
                    var cell_lastPos = nextPos - 1;
                    var cell_last = Index.GetCellId(cell_lastPos);
        ...
        private readonly struct QueueEntry : IEquatable<QueueEntry>, IComparable<QueueEntry>
        {
            ...
            public S2ShapeIndexCell IndexCell { get; }

            public QueueEntry(Distance _distance, S2CellId _id, S2ShapeIndexCell _index_cell)
            {
                Distance = _distance; Id = _id; IndexCell = _index_cell;
            }
            ...
            {
                return Equals(Distance, other.Distance);
            }
            public override bool Equals(object obj)
            {
                return obj is QueueEntry qe && Equals(qe);
            }
            public static bool operator ==(QueueEntry x, QueueEntry y)
            {
                return Equals(x, y);
            }
            public static bool operator !=(QueueEntry x, QueueEntry y)
            ...
            public static bool operator <(QueueEntry a, QueueEntry b)
            {
                ...
                return b.Distance.IsLessThan(a.Distance);
            }
            public static bool operator >(QueueEntry a, QueueEntry b)
            {
                // The priority queue returns the largest elements first, so we want the
                // "largest" entry to have the smallest distance.
                return a.Distance.IsLessThan(b.Distance);
            }
...
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        public class Result : IEquatable<Result>, IComparable<Result>
        {
            ...
            public Result() { Distance = Infinity; ShapeId = -1; EdgeId = -1; }

            // Constructs a Result object for the given arguments.
            public Result(Distance distance, Int32 shape_id, Int32 edge_id)
            { Distance = distance; ShapeId = shape_id; EdgeId = edge_id; }
            ...
            public override int GetHashCode()
            {
                return HashCode.Combine(Distance, ShapeId, EdgeId);
            }
            ...
            public static bool operator <(Result x, Result y)
            {
                if (x.Distance.IsLessThan(y.Distance)) return true;
                if (y.Distance.IsLessThan(x.Distance)) return false;
                if (x.ShapeId < y.ShapeId) return true;
                if (y.ShapeId < x.ShapeId) return false;
                return x.EdgeId < y.EdgeId;
            }

            public static bool operator >(Result x, Result y)
            {
                if (y.Distance.IsLessThan(x.Distance)) return true;
                if (x.Distance.IsLessThan(y.Distance)) return false;
                if (y.ShapeId < x.ShapeId) return true;
                if (x.ShapeId < y.ShapeId) return false;
                return y.EdgeId < x.EdgeId;
            }
