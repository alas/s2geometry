...
public static class S2Pred
{
    ...
    {
        System.Diagnostics.Debug.Assert(a != o && b != o && c != o);
    ...
    public static int CompareEdgeDistance(S2Point x, S2Point a0, S2Point a1, S1ChordAngle r)
    {
        ...
        System.Diagnostics.Debug.Assert(a0 != -a1);
    ...
    public static int CompareEdgeDirections(S2Point a0, S2Point a1, S2Point b0, S2Point b1)
    {
        ...
        System.Diagnostics.Debug.Assert(a0 != -a1);
        System.Diagnostics.Debug.Assert(b0 != -b1);
    ...
    {
        System.Diagnostics.Debug.Assert(r < S1ChordAngle.Right);
        System.Diagnostics.Debug.Assert(CompareDistances(x0, a, b) < 0);  // (implies a != b)
        System.Diagnostics.Debug.Assert(CompareEdgeDistance(a, x0, x1, r) <= 0);
        System.Diagnostics.Debug.Assert(CompareEdgeDistance(b, x0, x1, r) <= 0);
        // Check that the edge does not consist of antipodal points.  (This catches
    ...
    public static int TriageSign(S2Point a, S2Point b, S2Point c, S2Point a_cross_b)
    {
        ...
        System.Diagnostics.Debug.Assert(a_cross_b == a.CrossProd(b));
        double det = a_cross_b.DotProd(c);

#if s2debug
        // Double-check borderline cases in debug mode.
        System.Diagnostics.Debug.Assert(Math.Abs(det) <= kMaxDetError || Math.Abs(det) >= 100 * kMaxDetError || det * ExpensiveSign(a, b, c) > 0);
#endif
    ...
    private static int SymbolicallyPerturbedSign(S2Point a, S2Point b, S2Point c, S2Point b_cross_c)
    {
        ...
        System.Diagnostics.Debug.Assert(a < b && b < c);
        ...
        System.Diagnostics.Debug.Assert(0 == (c[1] * a[2] - c[2] * a[1]).Sgn());  // db[0]
    ...
    {
        System.Diagnostics.Debug.Assert(a != b && b != c && c != a);
        ...
        System.Diagnostics.Debug.Assert(!double.IsNaN(det));
        //System.Diagnostics.Debug.Assert(det.prec() < det.max_prec());
        ...
        if (det_sign == 0 && perturb)
        {
            ...
            System.Diagnostics.Debug.Assert(0 != det_sign);
        }
...
#pragma warning restore IDE0060 // Remove unused parameter
    ...
    {
        System.Diagnostics.Debug.Assert(r2 < 2.0);  // Only valid for distance limits < 90 degrees.
...
#pragma warning restore IDE0060 // Quitar el parámetro no utilizado
    ...
    {
        System.Diagnostics.Debug.Assert(!ArePointsAntipodal(a0, a1));
        System.Diagnostics.Debug.Assert(!ArePointsAntipodal(b0, b1));
    ...
    private static int ExactEdgeCircumcenterSign(S2Point x0, S2Point x1, S2Point a, S2Point b, S2Point c, int abc_sign)
    {
        ...
        {
            System.Diagnostics.Debug.Assert(x0.DotProd(x1) > 0);  // Antipodal edges not allowed.
            return 0;
        }
    ...
    private static Excluded TriageVoronoiSiteExclusion(S2Point a, S2Point b, S2Point x0, S2Point x1, double r2)
    {
        ...
        if (sin_d < -sin_d_error)
        {
            ...
            System.Diagnostics.Debug.Assert(ca <= 0 || cb <= 0);
        ...
        System.Diagnostics.Debug.Assert(abs_lhs3 > lhs3_error);
    ...
    {
        System.Diagnostics.Debug.Assert(!ArePointsAntipodal(x0, x1));
        ...
        if (rhs2_sgn < 0)
        {
            ...
            if (ca < 0 && cb < 0) return Excluded.NEITHER;
            System.Diagnostics.Debug.Assert(ca != 0 && cb != 0);  // This is guaranteed since d < 0.
            System.Diagnostics.Debug.Assert(ca < 0 || cb < 0);    // At least one site must be kept.
            return (ca > 0) ? Excluded.FIRST : Excluded.SECOND;
        }
        ...
        if (lhs2_sgn == 0)
        {
            ...
            System.Diagnostics.Debug.Assert(rhs2_sgn > 0);
