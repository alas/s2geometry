...
public static class S2Pred
{
    ...
    public static int TriageSign(S2Point a, S2Point b, S2Point c, S2Point a_cross_b)
    {
        ...
        Assert.Equal(a_cross_b, a.CrossProd(b));
...
#pragma warning restore IDE0060 // Quitar el parámetro no utilizado
    ...
    private static Excluded ExactVoronoiSiteExclusion(S2Point a, S2Point b, S2Point x0, S2Point x1, double r2)
    {
        ...
        if (rhs2_sgn < 0)
        {
            ...
            if (ca < 0 && cb < 0) return Excluded.NEITHER;
            Assert.True(ca != 0 && cb != 0);  // This is guaranteed since d < 0.
            Assert.True(ca < 0 || cb < 0);    // At least one site must be kept.
            return (ca > 0) ? Excluded.FIRST : Excluded.SECOND;
        }
