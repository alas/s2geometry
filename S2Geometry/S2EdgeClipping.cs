...
using S2PointUVW = S2Geometry.S2Point;
...
namespace S2Geometry
{
    ...
    public static class S2EdgeClipping
    {
        ...
        public static void GetFaceSegments(S2Point a, S2Point b, FaceSegmentVector segments)
        {
            ...
            int a_face = S2Coords.XYZtoFaceUV(a, out var segmentA);
            int b_face = S2Coords.XYZtoFaceUV(b, out var segmentB);
            if (a_face == b_face)
            {
            ...
            for (int face = a_face; face != b_face;)
            {
                ...
                // current face.
                var n = S2Coords.FaceXYZtoUVW(face, ab);
                ...
                S2Point exit_xyz = S2Coords.FaceUVtoXYZ(face, segmentB);
                face = GetNextFace(face, segmentB, exit_axis, n, b_face);
                S2PointUVW exit_uvw = S2Coords.FaceXYZtoUVW(face, exit_xyz);
                segmentFace = face;
        ...
        public static bool ClipToPaddedFace(S2Point a, S2Point b, int face, double padding, out R2Point a_uv, out R2Point b_uv)
        {
            ...
            if (S2Coords.GetFace(a) == face && S2Coords.GetFace(b) == face)
            {
                S2Coords.ValidFaceXYZtoUV(face, a, out a_uv);
                S2Coords.ValidFaceXYZtoUV(face, b, out b_uv);
                return true;
            }
            ...
            S2PointUVW n = S2Coords.FaceXYZtoUVW(face, S2PointUtil.RobustCrossProd(a, b));
            S2PointUVW a_2 = S2Coords.FaceXYZtoUVW(face, a);
            S2PointUVW b_2 = S2Coords.FaceXYZtoUVW(face, b);
            ...
            // that it underflows.
            if (Math.Max(Math.Abs(n[0]), Math.Max(Math.Abs(n[1]), Math.Abs(n[2]))) < MathUtils.Ldexp(1, -511))
            {
                n *= MathUtils.Ldexp(1, 563);
            }  // END OF HACK
            n = n.Normalized;
        ...
        public const double kFaceClipErrorRadians = 3 * S2Constants.DoubleEpsilon;
        //private const double kFaceClipErrorUVDist = 9 * S2Constants.DoubleEpsilon;
        public static readonly double kFaceClipErrorUVCoord = 9 * S2Constants.M_SQRT1_2 * S2Constants.DoubleEpsilon;
        ...
        public static readonly double kIntersectsRectErrorUVDist = 3 * S2Constants.M_SQRT2 * S2Constants.DoubleEpsilon;
        ...
        public const double kEdgeClipErrorUVCoord = 2.25 * S2Constants.DoubleEpsilon;
        public const double kEdgeClipErrorUVDist = 2.25 * S2Constants.DoubleEpsilon;
        ...
        private static int MoveOriginToValidFace(int face, S2Point a, S2Point ab, ref R2Point a_uv)
        {
            ...
            S2PointUVW n = S2Coords.FaceXYZtoUVW(face, ab);
            if (IntersectsFace(n))
            {
                ...
                S2Point exit = S2Coords.FaceUVtoXYZ(face, GetExitPoint(n, GetExitAxis(n)));
                S2Point a_tangent = ab.Normalized.CrossProd(a);
            ...
            {
                face = S2Coords.GetUVWFace(face, 0 /*U axis*/, a_uv[0] > 0);
            }
            else
            {
                face = S2Coords.GetUVWFace(face, 1 /*V axis*/, a_uv[1] > 0);
            }
            Assert.True(IntersectsFace(S2Coords.FaceXYZtoUVW(face, ab)));
            S2Coords.ValidFaceXYZtoUV(face, a, out a_uv);
        ...
        private static int GetNextFace(int face, R2Point exit, int axis, S2PointUVW n, int target_face)
        {
            ...
            if (Math.Abs(exit[1 - axis]) == 1 &&
                S2Coords.GetUVWFace(face, 1 - axis, exit[1 - axis] > 0) == target_face &&
            ...
            return S2Coords.GetUVWFace(face, axis, exit[axis] > 0);
        }
        ...
        {
            Assert.True(IntersectsFace(scaled_n));
