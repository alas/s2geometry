...
public class S2MaxDistancePointTarget : S2MaxDistanceTargets
{
    ...
    // S2MaxDistance.Zero().)
    public sealed override S2Cap GetCapBound() => new(-point_, S1ChordAngle.Zero);
    public sealed override bool UpdateMinDistance(S2Point p, ref S2MaxDistance min_dist) => S2MaxDistance.UpdateMin(new S2MaxDistance(new S1ChordAngle(p, point_)), ref min_dist);
    public sealed override bool UpdateMinDistance(S2Point v0, S2Point v1, ref S2MaxDistance min_dist)
    {
        var dist = min_dist.Distance;
        if (S2.UpdateMaxDistance(point_, v0, v1, ref dist))
        {
    ...
    public sealed override bool UpdateMinDistance(S2Cell cell, ref S2MaxDistance min_dist) => S2MaxDistance.UpdateMin(new S2MaxDistance(cell.MaxDistance(point_)), ref min_dist);
...
public class S2MaxDistanceEdgeTarget : S2MaxDistanceTargets
{
    ...
    {
        var dist = min_dist.Distance;
        if (S2.UpdateMaxDistance(p, a_, b_, ref dist))
        {
            S2MaxDistance.UpdateMin(new S2MaxDistance(dist), ref min_dist);
            return true;
        }
    ...
    {
        var dist = min_dist.Distance;
        if (S2.UpdateEdgePairMaxDistance(a_, b_, v0, v1, ref dist))
        {
            S2MaxDistance.UpdateMin(new S2MaxDistance(dist), ref min_dist);
            return true;
        }
    ...
    {
        return S2MaxDistance.UpdateMin(new S2MaxDistance(cell.MaxDistance(a_, b_)), ref min_dist);
    }
    public sealed override bool VisitContainingShapes(S2ShapeIndex index, ShapeVisitor visitor)
    {
        ...
        var target = new S2MaxDistancePointTarget((a_ + b_).Normalize());
...
public class S2MaxDistanceCellTarget : S2MaxDistanceTargets
{
    ...
    public sealed override bool UpdateMinDistance(S2Point p, ref S2MaxDistance min_dist)
    {
        return S2MaxDistance.UpdateMin(new S2MaxDistance(cell_.MaxDistance(p)), ref min_dist);
    }
    public sealed override bool UpdateMinDistance(S2Point v0, S2Point v1, ref S2MaxDistance min_dist)
    {
        return S2MaxDistance.UpdateMin(new S2MaxDistance(cell_.MaxDistance(v0, v1)), ref min_dist);
    }
    public sealed override bool UpdateMinDistance(S2Cell cell, ref S2MaxDistance min_dist)
    {
        return S2MaxDistance.UpdateMin(new S2MaxDistance(cell_.MaxDistance(cell)), ref min_dist);
    }
    public sealed override bool VisitContainingShapes(S2ShapeIndex index, ShapeVisitor visitor)
    {
        // We only need to check one point here - cell center is simplest.
        // See comment at S2MaxDistanceEdgeTarget.VisitContainingShapes.
        var target = new S2MaxDistancePointTarget(cell_.Center());
        return target.VisitContainingShapes(index, visitor);
    }
...
public class S2MaxDistanceShapeIndexTarget : S2MaxDistanceTargets
{
    ...
    //
    // DEFAULT: true
    public bool IncludeInteriors
    {
        get => query_.Options_.IncludeInteriors;
        set => query_.Options_.IncludeInteriors = value;
    }
    ...
    {
        get => query_.Options_.UseBruteForce;
        set => query_.Options_.UseBruteForce = value;
    }

    internal override S1ChordAngle MaxError 
    {
        get => query_.Options_.MaxError;
        set => query_.Options_.MaxError = value;
    }
