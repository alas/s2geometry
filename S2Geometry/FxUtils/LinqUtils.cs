...
public static class LinqUtils
{
    ...
    public static int GetUpperBound<T>(this T[] arr, T value, int first, int last) where T : IComparable<T>
    {
        var i = Array.BinarySearch(arr, first, last - first, value);
        if (i < 0) return ~i;
