...
namespace S2Geometry
{
    ...
    public static class S2ShapeMeasures
    {
        ...
        public static S1Angle GetPerimeter(S2Shape shape)
        {
            ...
            {
                GetChainVertices(shape, chain_id, out var vertices);
                perimeter += S2LoopMeasures.GetPerimeter(vertices);
            }
            return perimeter;
        }
        ...
        public static double GetArea(S2Shape shape)
        {
            ...
            //
            // So instead we use S2LoopMeasures.GetSignedArea() to ensure that all loops have areas
            ...
            {
                GetChainVertices(shape, chain_id, out var vertices);
                area += S2LoopMeasures.GetSignedArea(vertices);
            }
            // Note that S2LoopMeasures.GetSignedArea() guarantees that the full loop (containing
            // all points on the sphere) has a very small negative area.
            Assert.True(Math.Abs(area) <= S2Constants.M_4_PI);
        ...
        public static double GetApproxArea(S2Shape shape)
        {
            ...
            {
                GetChainVertices(shape, chain_id, out var vertices);
                area += S2LoopMeasures.GetApproxArea(vertices); //todo
            }
            // Special case to ensure that full polygons are handled correctly.
            if (area <= S2Constants.M_4_PI) return area;
            return area % S2Constants.M_4_PI;
        }
        ...
        public static S2Point GetCentroid(S2Shape shape)
        {
            ...
            for (int chain_id = 0; chain_id < num_chains; ++chain_id)
            {
                switch (dimension)
                {
                    ...
                    default:
                        GetChainVertices(shape, chain_id, out vertices);
                        centroid += S2LoopMeasures.GetCentroid(vertices);
                        break;
                }
            }
            return centroid;
