...
namespace S2Geometry
{
    ...
    // as vector.  That is, methods may be called concurrently from
    // multiple threads, and non-methods require exclusive access to the
    // S2ShapeIndex.
    public abstract class S2ShapeIndex
    {
        ...
        public (CellRelation cellRelation, int pos) LocateCell(S2CellId target)
        {
            ...
            if (id != null)
            {
                if (id >= target && id.RangeMin <= target) return (CellRelation.INDEXED, pos);
        ...
        public virtual (int pos, bool found) LocatePoint(S2Point target_point)
        {
            ...
            if (cell1 != null && cell1.RangeMin <= target) return (pos, true);
            var cell2 = GetCellId(pos - 1);
            if (cell2 != null && cell2.RangeMax >= target) return (pos - 1, true);
            return (pos - 1, false);
        }
    ...
    public class S2ShapeIndexCell
    {
        ...
        private readonly List<S2ClippedShape> shapes_ = new List<S2ClippedShape>();
    }
