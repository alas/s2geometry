...
public class S2WindingOperation
{
    ...
    private class WindingOracle
    {
        ...
        {
            g_ = g;
            MyDebug.Assert(g_.Options.EdgeType_ == EdgeType.DIRECTED);
            MyDebug.Assert(g_.Options.DegenerateEdges_ == DegenerateEdges.KEEP);
            MyDebug.Assert(g_.Options.DuplicateEdges_ == DuplicateEdges.KEEP);
            MyDebug.Assert(g_.Options.SiblingPairs_ == SiblingPairs.KEEP);
    ...
    private class WindingLayer(S2WindingOperation op, S2Builder.Layer result_layer) : S2Builder.Layer
    {
                                   ...
                                   List<EdgeId> left_turn_map, List<EdgeId> sibling_map)
        {
            ...
            MyDebug.Assert(g.GetEdge(start).EdgeId == v);
            ...
            for (; ; )
            {
                ...
                MyDebug.Assert(g.GetEdge(e0).EdgeId == v);
                MyDebug.Assert(g.GetEdge(e1).ShapeId == v);
