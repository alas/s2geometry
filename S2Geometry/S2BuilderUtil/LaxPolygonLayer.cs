...
namespace S2Geometry.S2BuilderUtil
{
    ...
    public class LaxPolygonLayer : S2Builder.Layer
    {
        public class Options
        {
            // Constructor that uses the default options (listed below).
            public Options() : this(S2Builder.EdgeType.DIRECTED) { }
        ...
        private static void AppendPolygonLoops(Graph g, List<EdgeLoop> edge_loops, List<List<S2Point>> loops)
        {
            foreach (var edge_loop in edge_loops)
            {
                ...
                {
                    vertices.Add(g.Vertex(g.GetEdge(edge_id).Item1));
                }
                loops.Add(vertices);
            }
        }
        ...
        {
            Assert.True(edges_to_discard.IsSorted());
            new_edges.Clear();
            new_input_edge_id_set_ids.Clear();
            ...
            Assert.True(it == edges_to_discard.Count);
        }
    ...
    public class IndexedLaxPolygonLayer : S2Builder.Layer
    {
