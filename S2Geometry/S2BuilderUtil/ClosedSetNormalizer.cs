...
namespace S2Geometry.S2BuilderUtil
{
    ...
    public class ClosedSetNormalizer
    {
            ...
            if (Options_.SuppressLowerDimensions)
            {
                ...
                for (var dim = 1; dim <= 2; ++dim)
                {
                    for (var e = 0; e < g[dim].NumEdges; ++e)
                    {
                        ...
                        {
                            is_suppressed_[edge.Item1] = true;
                            is_suppressed_[edge.Item2] = true;
                        }
                    }
                }
            }
            ...
            for (; ; )
            {
                if (edge2 <= edge1 && edge2 <= edge0)
                {
                    ...
                    else if (!degeneracies[degeneracy++].IsHole)
                    {
                        ...
                        else
                        {
                            ...
                            if (!IsSuppressed(edge2.Item1)) AddEdge(0, g[2], e2);
                        }
                    }
                ...
                else if (edge1 <= edge0)
                {
                    ...
                    else
                    {
                        ...
                        if (!IsSuppressed(edge1.Item1)) AddEdge(0, g[1], e1);
                ...
                {
                    // Input point.
                    if (!IsSuppressed(edge0.Item1)) AddEdge(0, g[0], e0);
        ...
        private readonly List<bool> is_suppressed_ = new List<bool>();
        ...
        // Output data.
        private readonly List<Graph> new_graphs_ = new List<Graph>();
    ...
    public class NormalizeClosedSetImpl
    {
        ...
        private class DimensionLayer : S2Builder.Layer
        {
