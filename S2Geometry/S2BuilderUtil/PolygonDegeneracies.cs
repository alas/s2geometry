...
namespace S2Geometry.S2BuilderUtil
{
    ...
    public struct PolygonDegeneracy : IEquatable<PolygonDegeneracy>, IComparable<PolygonDegeneracy>
    {
        public static readonly PolygonDegeneracy Zero = new PolygonDegeneracy(0, false);
        public readonly uint EdgeId;
        public readonly bool IsHole;

        public PolygonDegeneracy(Int32 _edge_id, bool _is_hole)
        {
            EdgeId = (uint)_edge_id; IsHole = _is_hole;
        }
        public override int GetHashCode()
        {
            return HashCode.Combine(EdgeId, IsHole);
        }
        public int CompareTo([AllowNull] PolygonDegeneracy other)
        {
            if (EdgeId.CompareTo(other.EdgeId) != 0)
                return EdgeId.CompareTo(other.EdgeId);

            return IsHole.CompareTo(other.IsHole);
        }
        public bool Equals(PolygonDegeneracy other)
        {
            return EdgeId == other.EdgeId && IsHole == other.IsHole;
        }
        public override bool Equals(object obj)
        {
            return obj is PolygonDegeneracy pd && Equals(pd);
        }
        public static bool operator ==(PolygonDegeneracy x, PolygonDegeneracy y)
        {
            return Equals(x, y);
        }
        public static bool operator !=(PolygonDegeneracy x, PolygonDegeneracy y)
        ...
        {
            Assert.True(g.Options.EdgeType_== EdgeType.DIRECTED);
            Assert.True(g.Options.DegenerateEdges_== DegenerateEdges.DISCARD ||
                   g.Options.DegenerateEdges_== DegenerateEdges.DISCARD_EXCESS);
            Assert.True(g.Options.SiblingPairs_== SiblingPairs.DISCARD ||
    ...
    public class DegeneracyFinder
    {
        ...
        public List<PolygonDegeneracy> Run(out S2Error error)
        {
            ...
            if (num_unknown_signs > 0)
            {
                ...
                int kMaxUnindexedContainsCalls = 20;  // Tuned using benchmarks.
        ...
        private int ComputeDegeneracies()
        {
            ...
            for (int in_ = 0, out_ = 0; out_ < n; ++out_)
            {
                ...
                if (out_edge.Item1 == out_edge.Item2)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        is_vertex_unbalanced_[out_edge.Item1] = true;
                    }
                }
            }
        ...
        private bool CrossingParity(Int32 v0, Int32 v1, bool include_same)
        {
            ...
            S2Point p0_ref = S2PointUtil.Ortho(p0);
            foreach (var edge in out_.Edges(v0))
            {
                ...
                else if (S2Pred.OrderedCCW(p0_ref, g_.Vertex(edge.Item2), p1, p0))
                {
                    ++crossings;
                }
            }
            foreach (Int32 e in in_.EdgeIds(v0))
            {
                ...
                else if (S2Pred.OrderedCCW(p0_ref, g_.Vertex(edge.Item1), p1, p0))
                {
                    ++crossings;
                }
            }
        ...
        private int ContainsVertexSign(Int32 v0)
        {
            ...
            foreach (var edge in out_.Edges(v0))
            {
                query.AddEdge(g_.Vertex(edge.Item2), 1);
            }
            foreach (Int32 e in in_.EdgeIds(v0))
            {
                query.AddEdge(g_.Vertex(g_.GetEdge(e).Item1), -1);
            }
        ...
        private void ComputeUnknownSignsBruteForce(Int32 known_vertex, int known_vertex_sign, List<Component> components)
        {
            ...
            for (var i = 0; i < components.Count; i++)
            {
                ...
                for (Int32 e = 0; e < g_.NumEdges; ++e)
                {
                    ...
                    inside ^= crosser.EdgeOrVertexCrossing(g_.Vertex(edge.Item1), g_.Vertex(edge.Item2));
                }
        ...
        private void ComputeUnknownSignsIndexed(Int32 known_vertex, int known_vertex_sign, List<Component> components)
        {
            ...
            var crossing_edges = new List<S2ShapeUtil.Edge>();
            var crosser = new S2EdgeCrosser();
            for (var i = 0; i < components.Count; i++)
            {
                var component = components[i];
                ...
                foreach (var (_, edgeId) in crossing_edges)
                {
                    int e = edgeId;
                    if (is_edge_degeneracy_[e]) continue;
                    inside ^= crosser.EdgeOrVertexCrossing(g_.Vertex(g_.GetEdge(e).Item1),
                                                           g_.Vertex(g_.GetEdge(e).Item2));
                }
                components[i] = new Component
                {
                    Degeneracies = component.Degeneracies,
        ...
        private static List<PolygonDegeneracy> MergeDegeneracies(List<Component> components)
        {
            ...
            {
                Assert.True(component.RootSign != 0);
