...
namespace S2Geometry.S2BuilderUtil
{
    ...
    {
        // The defaultructor uses a snap_radius of zero (i.e., no snapping).
        ...
        public IdentitySnapFunction(S1Angle snap_radius)
        {
        ...
        public override S1Angle SnapRadius
        {
            ...
            set
            {
                Assert.True(value <= SnapFunction.kMaxSnapRadius);
                snap_radius_ = value;
            }
        }
        ...
        {
            return new IdentitySnapFunction(snap_radius_);
        }
    }
    ...
    {
        // The defaultructor snaps to S2Constants.kMaxCellLevel (i.e., the centers
        // of leaf cells), and uses the minimum allowable snap radius at that level.
        public S2CellIdSnapFunction()
        {
            Level = (S2Constants.kMaxCellLevel);
        }
        ...
        public int Level
        {
            ...
            {
                Assert.True(value >= 0);
                Assert.True(value <= S2Constants.kMaxCellLevel);
                level_ = value;
        ...
        public static S1Angle MinSnapRadiusForLevel(int level)
        {
            ...
            return S1Angle.FromRadians(0.5 * S2Metrics.kMaxDiag.GetValue(level) + 4 * S2Constants.DoubleEpsilon);
        }
        ...
        public static int LevelForMaxSnapRadius(S1Angle snap_radius)
        {
            ...
            return S2Metrics.kMaxDiag.GetLevelForMaxValue(
                2 * (snap_radius.Radians - 4 * S2Constants.DoubleEpsilon));
        }
        ...
        public override S1Angle MinVertexSeparation()
        {
            ...
            //    when snapped.
            S1Angle min_edge = S1Angle.FromRadians(S2Metrics.kMinEdge.GetValue(level_));
            S1Angle max_diag = S1Angle.FromRadians(S2Metrics.kMaxDiag.GetValue(level_));
        ...
        public override S1Angle MinEdgeVertexSeparation()
        {
            ...
            S1Angle min_diag = S1Angle.FromRadians(S2Metrics.kMinDiag.GetValue(level_));
        ...
        public override S2Point SnapPoint(S2Point point)
        {
            return new S2CellId(point).Parent(level_).ToPoint();
        }

        public override object Clone()
        {
            return new S2CellIdSnapFunction(level_, snap_radius_);
        }
    }
...
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        // The defaultructor yields an invalid snap function.  You must set
        ...
        // REQUIRES: snap_radius <= SnapFunction.kMaxSnapRadius()
        public override S1Angle SnapRadius { get => snap_radius_; set
        {
        ...
        public static S1Angle MinSnapRadiusForExponent(int exponent)
        {
            ...
            return (S1Angle.FromDegrees(S2Constants.M_SQRT1_2 / power) +
                    S1Angle.FromRadians((9 * S2Constants.M_SQRT2 + 1.5) * S2Constants.DoubleEpsilon));
        }
        ...
        public static int ExponentForMaxSnapRadius(S1Angle snap_radius)
        {
            ...
            snap_radius -= S1Angle.FromRadians((9 * S2Constants.M_SQRT2 + 1.5) * S2Constants.DoubleEpsilon);
            snap_radius = S1Angle.Max(snap_radius, S1Angle.FromRadians(1e-30));
            double exponent = Math.Log10(S2Constants.M_SQRT1_2 / snap_radius.Degrees);
            ...
            return Math.Max(kMinExponent,
                       Math.Min(kMaxExponent,
                           (int)(Math.Ceiling(exponent - 2 * S2Constants.DoubleEpsilon))));
        }
        ...
        public override S1Angle MinVertexSeparation()
        {
            ...
            return S1Angle.Max(0.471 * snap_radius_,        // Math.Sqrt(2) / 3 in the plane
                snap_radius_ - S1Angle.FromDegrees(S2Constants.M_SQRT1_2 * to_degrees_));
        }
    ...
    //    crosses an edge.
    public abstract class SnapFunction : ICloneable
    {
...
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        public S1Angle MaxEdgeDeviation()
        {
            ...
            Assert.True(SnapRadius<= kMaxSnapRadius);
