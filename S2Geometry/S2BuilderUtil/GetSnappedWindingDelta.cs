...
public class SnappedWindingDelta
{
        ...
        Graph g, out S2Error error)
    {
        ...
        MyDebug.Assert(!builder.Options_.SimplifyEdgeChains);
        MyDebug.Assert(g.Options.EdgeType_ == S2Builder.EdgeType.DIRECTED);
        MyDebug.Assert(g.Options.DegenerateEdges_ == GraphOptions.DegenerateEdges.KEEP);
        MyDebug.Assert(
            new[]
            {
        ...
        while (input_vertex_edge_map.Count!=0)
        {
            ...
            else
            {
                ...
                MyDebug.Assert(chain_out.Count == 3);
                MyDebug.Assert(chain_out[1] == ref_out);
    ...
    public static VertexId FindFirstVertexId(InputEdgeId input_edge_id, Graph g)
    {
        ...
        MyDebug.Assert(excess_degree_map.Count == 1);
