...
public class SnappedWindingDelta
{
        ...
        Graph g, out S2Error error)
    {
        ...
        Assert.True(!builder.Options_.SimplifyEdgeChains);
        Assert.True(g.Options.EdgeType_ == S2Builder.EdgeType.DIRECTED);
        Assert.True(g.Options.DegenerateEdges_ == GraphOptions.DegenerateEdges.KEEP);
        Assert.True(
            new[]
            {
        ...
        while (input_vertex_edge_map.Any())
        {
            ...
            if (chain_out.Count == 1)
            {
                ...
                Assert.True(chain_out[0] == ref_out);         // Snaps to R'.
                Assert.True(chain_in[0] == chain_in.Last());  // Chain is a loop.
            ...
            else
            {
                ...
                Assert.True(chain_out.Count == 3);
                Assert.True(chain_out[1] == ref_out);
    ...
    {
        error = S2Error.OK;
        Assert.True(!chain_in.Any());
        Assert.True(!chain_out.Any());
    ...
    public static VertexId FindFirstVertexId(InputEdgeId input_edge_id, Graph g)
    {
        ...
        Assert.Equal(excess_degree_map.Count, 1);
