...
/// </summary>
public readonly record struct R2Rect
{
    #region Fields, Constants

    public readonly R1Interval X { get; init; }
    public readonly R1Interval Y { get; init; }

    /// <summary>
    /// The canonical empty rectangle.  Use IsEmpty to test for empty
    /// rectangles, since they have more than one representation.
    /// </summary>
    public static readonly R2Rect Empty = new(R1Interval.Empty, R1Interval.Empty);
    ...
    /// </summary>
    public R2Rect()
        : this(R1Interval.Empty, R1Interval.Empty)
    {
    }
    ...
    /// </summary>
    public static R2Rect FromPoint(R2Point p) => new(p, p);

    #endregion

    #region R2Rect

    public R2Point Lo() => new(X.Lo, Y.Lo);

    public R2Point Hi() => new(X.Hi, Y.Hi);
    ...
    /// </summary>
    public bool InteriorContains(R2Rect other) =>
        X.InteriorContains(other.X) && Y.InteriorContains(other.Y);
    ...
    /// </summary>
    public bool InteriorIntersects(R2Rect other) =>
        X.InteriorIntersects(other.X) && Y.InteriorIntersects(other.Y);

    /// <summary>
    /// Expand the rectangle to include the given point.  The rectangle is
    /// expanded by the minimum amount possible.
    /// </summary>
    public R2Rect AddPoint(R2Point p) =>
        new(R1Interval.AddPoint(X, p.X), R1Interval.AddPoint(Y, p.Y));
    ...
    /// </summary>
    public R2Rect AddRect(R2Rect other) =>
        new(R1Interval.AddInterval(X, other.X), R1Interval.AddInterval(Y, other.Y));

    /// <summary>
    /// Return the closest point in the rectangle to the given point "p".
    /// The rectangle must be non-empty.
    /// </summary>
    public R2Point Project(R2Point p) => new(X.Project(p.X), Y.Project(p.Y));
    ...
    /// the given rectangle.
    /// </summary>
    public R2Rect Union(R2Rect other) => new(X.Union(other.X), Y.Union(other.Y));
    ...
    /// </summary>
    public bool ApproxEquals(R2Rect other, double max_error = S2.DoubleError) =>
