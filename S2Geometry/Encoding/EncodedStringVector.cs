...
namespace S2Geometry
{
	...
	public class StringVectorEncoder
	{
		...
		public void Encode(Encoder encoder)
		{
			...
			encoder.PutN(data_.Buffer, data_.Length);
		}
		...
		{
			var string_vector = new StringVectorEncoder();
		...
		// the last string.
		private readonly List<UInt64> offsets_ = new List<ulong>();
	...
	public class EncodedStringVector
	{
		...
		public static bool Init(Decoder decoder, out EncodedStringVector output)
		{
			output = null;
			var res = new EncodedStringVector();
			if (!res.offsets_.Init(decoder)) return false;

			res.data_ = decoder.Buffer;
			if (res.offsets_.Count > 0)
			{
				var length = (int)res.offsets_[^1];
