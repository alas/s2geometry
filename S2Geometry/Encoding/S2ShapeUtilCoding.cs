...
public static class S2ShapeUtilCoding
{
    ...
    public static bool FastEncodeShape(S2Shape shape, Encoder encoder)
    {
        ...
        {
            MyDebug.WriteLine("(DFATAL) Unsupported S2Shape type: " + tag);
            return false;
        }
        // Update the following constant when adding new S2Shape encodings.
        MyDebug.Assert(tag < S2Shape.TypeTag.kNextAvailableTypeTag);
    ...
    public static S2Shape? LazyDecodeShape(S2Shape.TypeTag tag, Decoder decoder)
    {
        switch (tag)
        {
            case S2Shape.TypeTag.S2PointVectorShape:
                {
                    var (success, shape) = EncodedS2PointVectorShape.Init(decoder);
                    // Some platforms (e.g. NaCl) require the following conversion.
                    return !success ? null : (S2Shape?)shape;  // Converts to S2Shape.
                }
            case S2Shape.TypeTag.S2LaxPolylineShape:
                {
                    var (success, shape) = EncodedS2LaxPolylineShape.Init(decoder);
                    return !success ? null : (S2Shape?)shape;  // Converts to S2Shape.
                }
            case S2Shape.TypeTag.S2LaxPolygonShape:
                {
                    var (success, shape) = EncodedS2LaxPolygonShape.Init(decoder);
                    return !success ? null : (S2Shape?)shape;  // Converts to S2Shape.
                }
            default:
    ...
    public static bool FastEncodeTaggedShapes(S2ShapeIndex index, Encoder encoder) =>
        EncodeTaggedShapes(index, FastEncodeShape, encoder);
    ...
    public static bool CompactEncodeTaggedShapes(S2ShapeIndex index, Encoder encoder) =>
        EncodeTaggedShapes(index, CompactEncodeShape, encoder);
    ...
    public class TaggedShapeFactory : S2ShapeIndex.ShapeFactory
    {
        ...
        public TaggedShapeFactory(ShapeDecoder shape_decoder, Decoder decoder)
        {
            ...
            {
                //shape.Clear();
                encoded_shapes_ = null;
            }
        }

        public override int Count => encoded_shapes_?.Count() ?? 0;
