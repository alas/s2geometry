...
public class EncodedS2PointVector
{
    ...
    public static void EncodeS2PointVectorCompact(S2Point[] points, Encoder encoder)
    {
        ...
        System.Diagnostics.Debug.Assert(last_block_count >= 0);
        System.Diagnostics.Debug.Assert(last_block_count <= kBlockSize);
        System.Diagnostics.Debug.Assert(base_bytes <= 7);
        System.Diagnostics.Debug.Assert(level <= 30);
        encoder.Put8((byte)(
        ...
        for (int i = 0; i < values.Length; i += kBlockSize)
        {
            ...
            System.Diagnostics.Debug.Assert(delta_nibbles <= 16);
            ...
            for (int j = 0; j < block_size; ++j)
            {
                ...
                {
                    System.Diagnostics.Debug.Assert(values[i + j] >= base_);
            ...
            for (int j = 0; j < block_size; ++j)
            {
                ...
                {
                    System.Diagnostics.Debug.Assert(values[i + j] >= offset + base_);
                    delta = values[i + j] - (offset + base_);
                    if (have_exceptions)
                    {
                        System.Diagnostics.Debug.Assert(delta <= (~0UL - kBlockSize));
                        delta += kBlockSize;
                    }
                }
    ...
    private bool InitCellIdsFormat(Decoder decoder)
    {
        ...
        System.Diagnostics.Debug.Assert((header1 & 7) == (byte)Format.CELL_IDS);
    ...
    private static UInt64[] ConvertCellsToValues(CellPoint[] cell_points, int level, out bool have_exceptions)
    {
        ...
        for (var i = 0; i < cell_points.Length; i++)
        {
            ...
            else
            {
                ...
                System.Diagnostics.Debug.Assert(v <= BitMask(MaxBitsForLevel(level)));
                values[i] = v;
            }
        }
        return values;
    ...
    private static BlockCode GetBlockCode(ArraySegment<UInt64> values, UInt64 base_, bool have_exceptions)
    {
        ...
        if (!CanEncode(b_min, b_max, delta_bits, 0, have_exceptions))
        {
            ...
            {
                System.Diagnostics.Debug.Assert(delta_bits <= 60);
                delta_bits += 4;
                if (!CanEncode(b_min, b_max, delta_bits, 0, have_exceptions))
                {
                    System.Diagnostics.Debug.Assert(CanEncode(b_min, b_max, delta_bits, 4, have_exceptions));
                    overlap_bits = 4;
                }
            }
        }
        ...
        {
            System.Diagnostics.Debug.Assert(delta_bits == 4 && overlap_bits == 0);
            delta_bits = 8;
        }
        ...
        if (b_max > max_delta)
        {
            ...
            System.Diagnostics.Debug.Assert(min_offset > 0);
