...
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        public static void EncodeS2PointVectorCompact(S2Point[] points, Encoder encoder)
        {
            ...
            Assert.True(last_block_count >= 0);
            Assert.True(last_block_count <= kBlockSize);
            Assert.True(base_bytes <= 7);
            Assert.True(level <= 30);
            encoder.Put8((byte)(
            ...
            for (int i = 0; i < values.Length; i += kBlockSize)
            {
                ...
                Assert.True(offset_bytes - overlap_nibbles <= 7);
                Assert.True(overlap_nibbles <= 1);
                Assert.True(delta_nibbles <= 16);
                ...
                Assert.True((offset == 0) == (offset_bytes == 0));
                if (offset > 0)
                {
                ...
                for (int j = 0; j < block_size; ++j)
                {
                    ...
                    {
                        Assert.True(values[i + j] >= offset + base_);
                        delta = values[i + j] - (offset + base_);
                        if (have_exceptions)
                        {
                            Assert.True(delta <= (~0UL - kBlockSize));
                            delta += kBlockSize;
                        }
                    }
                    Assert.True(delta <= BitMask(code.DeltaBits_));
        ...
        private bool InitCellIdsFormat(Decoder decoder)
        {
            ...
            Assert.True((header1 & 7) == (byte)Format.CELL_IDS);
            ...
            if (!EncodedStringVector.Init(decoder, out blocks)) return false;
        ...
        private S2Point DecodeCellIdsFormat(int i)
        {
            ...
            int shift = S2Constants.kMaxCellLevel - level;
            ...
            return S2Coords.FaceUVtoXYZ(face, S2Coords.STtoUV(S2Coords.SiTitoST((uint)si)),
                                   S2Coords.STtoUV(S2Coords.SiTitoST((uint)ti))).Normalized;
        }
        ...
        private static UInt64 BitMask(int n)
        {
            return (n == 0) ? 0 : (~0UL >> (64 - n));
        }
        ...
        private static int BaseShift(int level, int base_bits)
        {
            return Math.Max(0, MaxBitsForLevel(level) - base_bits);
        }
        ...
        private static int ChooseBestLevel(S2Point[] points, out CellPoint[] cell_points)
        {
            ...
            var level_counts = new int[S2Constants.kMaxCellLevel + 1].Fill(0);
            for (var i = 0; i < points.Length; i++)
            {
                int level = S2Coords.XYZtoFaceSiTi(points[i], out int face, out uint si, out uint ti);
        ...
        private static UInt64[] ConvertCellsToValues(CellPoint[] cell_points, int level, out bool have_exceptions)
        {
            ...
            for (var i = 0; i < cell_points.Length; i++)
            {
                ...
                else
                {
                    ...
                    Assert.True(v <= BitMask(MaxBitsForLevel(level)));
                    values[i] = v;
                }
            }
            return values;
        ...
        private static BlockCode GetBlockCode(ArraySegment<UInt64> values, UInt64 base_, bool have_exceptions)
        {
            ...
            if (!CanEncode(b_min, b_max, delta_bits, 0, have_exceptions))
            {
                ...
                {
                    Assert.True(delta_bits <= 60);
                    delta_bits += 4;
                    if (!CanEncode(b_min, b_max, delta_bits, 0, have_exceptions))
                    {
                        Assert.True(CanEncode(b_min, b_max, delta_bits, 4, have_exceptions));
                        overlap_bits = 4;
                    }
                }
            }
            ...
            if (values.Count == 1)
            {
                Assert.True(delta_bits == 4 && overlap_bits == 0);
                delta_bits = 8;
            }
