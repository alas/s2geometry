...
namespace S2Geometry
{
    ...
    public readonly struct S1Interval : IEquatable<S1Interval>
    {
        ...
        public static readonly S1Interval Empty = new S1Interval(Math.PI, -Math.PI);
        public static readonly S1Interval Full = new S1Interval(-Math.PI, Math.PI);
        ...
        {
            Assert.True(Math.Abs(p1) <= Math.PI);
            Assert.True(Math.Abs(p2) <= Math.PI);
        ...
        public bool IsValid
        {
            get
            {
                return Math.Abs(Lo) <= Math.PI && Math.Abs(Hi) <= Math.PI &&
        ...
        public bool Contains(double p)
        {
            ...
            Assert.True(Math.Abs(p) <= Math.PI);
            if (p == -Math.PI) p = Math.PI;
        ...
        public S1Interval Expanded(double margin)
        {
            if (margin >= 0)
            {
                ...
                if (Length + 2 * margin + 2 * S2Constants.DoubleEpsilon >= S2Constants.M_2_PI) return Full;
            }
            else
            {
                ...
                // for a 1-bit rounding error when computing each endpoint.
                if (Length + 2 * margin - 2 * S2Constants.DoubleEpsilon <= 0) return Empty;
            }
            var lo = Math.IEEERemainder(Lo - margin, S2Constants.M_2_PI);
            var hi = Math.IEEERemainder(Hi + margin, S2Constants.M_2_PI);
        ...
        public S1Interval Intersection(S1Interval y)
        {
            ...
            if (FastContains(y.Lo))
            {
                if (FastContains(y.Hi))
                {
                    ...
                    if (y.Length < Length) return y;  // IsFull code path
                    return this;
                }
                return new S1Interval(y.Lo, Hi);
            }
        ...
        public bool ApproxEquals(S1Interval y, double maxError = S2Constants.DoubleError)
        {
            ...
            return (
                Math.Abs(Math.IEEERemainder(y.Lo - Lo, S2Constants.M_2_PI)) <= maxError &&
                Math.Abs(Math.IEEERemainder(y.Hi - Hi, S2Constants.M_2_PI)) <= maxError &&
                Math.Abs(Length - y.Length) <= 2 * maxError);
        }
        ...
        public bool Equals(S1Interval other) => Lo == other.Lo && Hi == other.Hi;

        public override bool Equals(object obj) => obj is S1Interval interval && Equals(interval);

        public override int GetHashCode() => HashCode.Combine(Math.Abs(Lo), Math.Abs(Hi));

        public static bool operator ==(S1Interval x, S1Interval y) => Equals(x, y);
