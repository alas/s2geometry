...
namespace S2Geometry
{
    ...
    public sealed class S2Polygon : S2Region<S2Polygon>, ICoder, IDisposable
    {
        ...
        private readonly List<S2Loop> loops_ = new List<S2Loop>();
        ...
        private Int32 unindexed_contains_calls_;
        ...
        // The defaultructor creates an empty polygon.  It can be made
        ...
        public S2Polygon(List<S2Loop> loops, bool initOriented = false)
        {
            loops_ = loops;
        ...
        private S2Polygon(List<S2Loop> loops, S2LatLngRect bound, S2LatLngRect subregion_bound) : this(loops)
        {
            loops_ = loops;
            bound_ = bound;
        ...
        public void InitOriented()
        {
            ...
            //
            // 1. Remember which of the given loops contain S2PointUtil.Origin.
            //
            ...
            //    and only if they contain S2PointUtil.Origin.  (In theory this step is only
            ...
            //    S2PointUtil.Origin (where "adjacent" means that there exists a path
            //    connecting S2PointUtil.Origin to some vertex of L such that the path does
            ...
            var contained_origin = new SortedSet<S2Loop>();
            foreach (var loop in loops_)
            {
                ...
                var angle = loop.GetCurvature();
                if (Math.Abs(angle) > loop.GetCurvatureMaxError())
                {
                    // Normalize the loop.
            ...
            foreach (var loop in loops_)
            {
                if (contained_origin.Contains(loop) != loop.ContainsOrigin != loop.IsHole)
                {
                    ...
                    Assert.True(IsValid);  // Always fails.
#endif
                }
            }
        }
        ...
        public void Copy(S2Polygon src)
        {
            ...
            {
                loops_.Add((S2Loop)src.Loop(i).Clone());
            }
        ...
        public int GetSnapLevel()
        {
            ...
            foreach (var child in loops_)
            {
                ...
                {
                    var level = S2Coords.XYZtoFaceSiTi(child.Vertex(j), out var face, out var si, out var ti);
        ...
        {
            InitToApproxIntersection(a, b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            InitToApproxUnion(a, b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            InitToApproxDifference(a, b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            InitToApproxSymmetricDifference(a, b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        public void InitToSnapped(S2Polygon a, int snap_level = S2Constants.kMaxCellLevel)
        {
            var builder = new S2Builder { Options_ = new S2Builder.Options(new S2CellIdSnapFunction(snap_level)) };
        ...
        {
            InitToSimplifiedInCell(a, cell, snap_radius, S1Angle.FromRadians(S2Constants.DoubleError));
        }
        ...
        public void InitToSimplifiedInCell(S2Polygon a, S2Cell cell, S1Angle snap_radius, S1Angle boundary_tolerance)
        {
            ...
            var options = new S2Builder.Options(new IdentitySnapFunction(S2EdgeCrossings.kIntersectionErrorS1Angle))
            {
            ...
            if (NumLoops() == 0)
            {
                if (a.bound_.Area() > S2Constants.M_2_PI && a.GetArea() > S2Constants.M_2_PI) Invert();
            }
        }
        ...
        public void InitToCellUnionBorder(S2CellUnion cells)
        {
            ...
            // different edges.
            var snap_radius = 0.5 * S2Metrics.kMinWidth.GetValue(S2Constants.kMaxCellLevel);
            ...
            if (NumLoops() == 0)
            {
                ...
                Assert.True(6UL << (2 * S2Constants.kMaxCellLevel) == cells.LeafCellsCovered());
                Invert();
            }
        }
        ...
        public void Invert()
        {
            ...
            else
            {
                ...
                Assert.True(new_loops.Count == NumLoops());
            }
            ClearIndex();
            InitLoopProperties();
        }
        ...
        {
            return ApproxContains(b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            return !ApproxDisjoint(b, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            return ApproxIntersectWithPolyline(input, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            return ApproxSubtractFromPolyline(input, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            return DestructiveApproxUnion(polygons, S2EdgeCrossings.kIntersectionMergeRadiusS1Angle);
        }
        ...
        {
            return BoundaryApproxEquals(b, S1Angle.FromRadians(S2Constants.DoubleError));
        }
        ...
        {
            return BoundaryNear(b, S1Angle.FromRadians(S2Constants.DoubleError));
        }
        ...
        {
            Assert.True(Index.NumShapeIds() == 0);
            Index.Add(new Shape(this));
#if FLAGS_s2polygon_not_lazy_indexing
            index_.ForceBuild();
#endif
#if s2debug
        ...
        private List<S2Polyline> OperationWithPolyline(S2BooleanOperation.OpType op_type, SnapFunction snap_function, S2Polyline a)
        {
            ...
            var layer_options = new S2PolylineVectorLayer.Options();
        ...
        {
            byte mask = 0;
            if (S2Coords.FaceXYZtoUV(cell.Face, p, out var uv))
            {
                var bound = cell.BoundUV;
#if s2debug
                Assert.True(bound.Expanded(tolerance_uv).Contains(uv));
#endif
        ...
        public override S2Cap GetCapBound()  // Cap surrounding rect bound.
        {
            return bound_.GetCapBound();
        }
        public override S2LatLngRect GetRectBound() { return bound_; }
        ...
        public void Encode(Encoder encoder)
        {
            if (NumVertices == 0)
            {
                EncodeCompressed(encoder, null, S2Constants.kMaxCellLevel);
                return;
            }
            ...
            var histogram = new int[S2Constants.kMaxCellLevel + 2].Fill(0);
        ...
        public static (bool success, S2Polygon result) DecodeStatic(Decoder decoder)
        {
        ...
        public static (bool success, S2Polygon result) DecodeWithinScope(Decoder decoder)
        {
            if (decoder.Avail() < sizeof(byte)) return (false, null);
            var version = decoder.Get8();
            return version switch
        ...
        private static (bool success, S2Polygon result) DecodeUncompressed(Decoder decoder)
        {
            ...
            {
                var (success, newLoop) = S2Loop.DecodeStatic(decoder);
                if (!success) return (false, null);
                loops.Add(newLoop);
            }
            var (success2, bound) = S2LatLngRect.DecodeStatic(decoder);
        ...
        private static (bool success, S2Polygon result) DecodeCompressed(Decoder decoder)
        {
            ...
            if (snap_level > S2Constants.kMaxCellLevel) return (false, null);
            // Polygons with no loops are explicitly allowed here: a newly created
            // polygon has zero loops and such polygons encode and decode properly.
        ...
        public override bool Equals(object other) => other is S2Polygon pol && Equals(pol);
        public override bool Equals(S2Polygon b) => loops_.SequenceEqual(b.loops_);
        public override int GetHashCode() => LinqUtils.GetSequenceHashCode(loops_);
        public static bool operator ==(S2Polygon left, S2Polygon right) => Equals(left, right);
        public static bool operator !=(S2Polygon left, S2Polygon right) => !Equals(left, right); 
        ...
        public class Shape : S2Shape
        {
            ...
            public void Init(S2Polygon polygon)
            {
                ...
                if (!polygon.IsFull)
                {
                    ...
                    for (var i = 0; i < num_loops; ++i)
                    {
                        if (cumulative_edges_ != null) cumulative_edges_[i] = num_edges_;
                        num_edges_ += polygon.Loop(i).NumVertices;
                    }
                }
            }
            ...
            // polygon_.NumVertices except in one case (polygon_.IsFull).  On
            ...
            public sealed override int NumEdges => num_edges_;
            public sealed override Edge GetEdge(int e)
            {
                Assert.True(e < NumEdges);
                var p = Polygon;
                int i;
                if (cumulative_edges_ != null)
                {
                    // "upper_bound" finds the loop just beyond the one we want.
                    i = LinqUtils.GetUpperBound(cumulative_edges_, e) - 1;
                    e -= cumulative_edges_[i];
                }
                else
                {
                ...
                return new Edge(p.Loop(i).OrientedVertex(e), p.Loop(i).OrientedVertex(e + 1));
            }
            ...
            public sealed override ReferencePoint GetReferencePoint()
            {
                ...
                for (var i = 0; i < p.NumLoops(); ++i)
                {
                    contains_origin ^= p.Loop(i).ContainsOrigin;
                }
                return new ReferencePoint(S2PointUtil.Origin, contains_origin);
            }
            public sealed override int NumChains() => Polygon.NumLoops();
            public sealed override Chain GetChain(int i)
            {
                Assert.True(i < NumChains());
                if (cumulative_edges_ != null)
                {
                    return new Chain(cumulative_edges_[i], Polygon.Loop(i).NumVertices);
                }
                else
                {
                    var e = 0;
            ...
            {
                Assert.True(i < NumChains());
                Assert.True(j < Polygon.Loop(i).NumVertices);
                return new Edge(Polygon.Loop(i).OrientedVertex(j),
                            Polygon.Loop(i).OrientedVertex(j + 1));
            }
        ...
        public class OwningShape : Shape, IEncodeInit
        {
            ...
            public OwningShape(S2Polygon polygon)
              : base(polygon) { Polygon = polygon; }

            public bool Init(Decoder decoder)
            {
                var (success, polygon) = DecodeStatic(decoder);
                if (!success) return false;
                Init(polygon);
                return true;
            }
