...
namespace S2Geometry
{
    ...
    public sealed class S2Loop : S2Region<S2Loop>, IEquatable<S2Loop>, IComparable<S2Loop>, ICoder
    {
        ...
        // Returns true if this loop contains S2PointUtil.Origin.
        ...
        private S2Point[] _vertices = null;
        ...
        private MutableS2ShapeIndex _index = new MutableS2ShapeIndex();
        ...
        private static readonly S2Point kEmptyVertex = new S2Point(0, 0, 1);

        // The single vertex in the "full loop" vertex chain.
        private static readonly S2Point kFullVertex = new S2Point(0, 0, -1);
...
#pragma warning disable IDE1006 // Estilos de nombres
        public static S2Loop kEmpty => new S2Loop(new S2Point[] { kEmptyVertex });
...
#pragma warning disable IDE1006 // Estilos de nombres
        public static S2Loop kFull => new S2Loop(new S2Point[] { kFullVertex });
#pragma warning restore IDE1006 // Estilos de nombres

        // Aux key for Dictionaries
        public static S2Loop NullLoop => new S2Loop(new S2Point[] { S2Point.Empty }, false);
        ...
        public S2Loop(IEnumerable<S2Point> vertices, bool assertIsValid = true)
        {
            ClearIndex();
            NumVertices = vertices.Count();
            _vertices = vertices.ToArray();
            InitOriginAndBound(assertIsValid);
        ...
        public S2Loop(S2Cell cell, bool assertIsValid = true)
        {
            Depth = 0;
            NumVertices = 4;
            _vertices = new S2Point[NumVertices];
        ...
        public S2Point[] CloneVertices()
        {
            ...
            Array.Copy(_vertices, v, NumVertices);
            return v;
        }
        ...
        public bool IsNormalized()
        {
            ...
            if (_bound.Lng.Length < Math.PI) return true;
        ...
        public void Invert()
        {
            ...
            if (IsEmptyOrFull)
            {
                _vertices[0] = IsFull ? kEmptyVertex : kFullVertex;
            }
            else
            {
                Array.Reverse(_vertices, 0, NumVertices);
            }
            // origin_inside_ must be set correctly before building the S2ShapeIndex.
            ContainsOrigin ^= true;
            if (_bound.Lat.Lo > -S2Constants.M_PI_2 && _bound.Lat.Hi < S2Constants.M_PI_2)
            {
        ...
        public double GetArea()
        {
            ...
            if (IsEmptyOrFull)
            {
                return ContainsOrigin ? S2Constants.M_4_PI : 0;
            }
        ...
        public S2Point GetCentroid()
        {
            ...
            return S2LoopMeasures.GetCentroid(VerticesSpan);
        }
        ...
        public double GetCurvature()
        {
            ...
            if (IsEmptyOrFull)
            {
                return ContainsOrigin ? (-S2Constants.M_2_PI) : S2Constants.M_2_PI;
            }
        ...
        public double GetCurvatureMaxError()
        {
            return S2LoopMeasures.GetCurvatureMaxError(VerticesSpan);
        }
        ...
        public bool BoundaryApproxEquals(S2Loop b, S1Angle max_error)
        {
            ...
            for (int offset = 0; offset < NumVertices; ++offset)
            {
                if (S2PointUtil.ApproxEquals(Vertex(offset), b.Vertex(0), max_error))
                {
                    bool success = true;
                    for (int i = 0; i < NumVertices; ++i)
                    {
                        if (!S2PointUtil.ApproxEquals(Vertex(i + offset), b.Vertex(i), max_error))
                        {
                            success = false;
                            break;
                        }
        ...
        //
        // Also requires that the defaultructor for T must initialize the
        ...
        {
            return S2LoopMeasures.GetSurfaceIntegral(VerticesSpan, f_tri);
        }
        ...
        {
            var m = S2PointUtil.GetFrame(center);
        ...
        public static S2Loop MakeRegularLoop(S2PointVector3 frame, S1Angle radius, int num_vertices)
        {
            ...
            double radian_step = S2Constants.M_2_PI / num_vertices;
            var vertices = new List<S2Point>();
            for (int i = 0; i < num_vertices; ++i)
            {
                double angle = i * radian_step;
                var p = new S2Point(r * Math.Cos(angle), r * Math.Sin(angle), z);
                vertices.Add(S2PointUtil.FromFrame(frame, p).Normalized);
            }
        ...
        // Returns an S2Point[] containing the loop vertices, for use with the
        // functions defined in s2loop_measures.h.
        public S2Point[] VerticesSpan => _vertices.ToArray();

        private void InitOriginAndBound(bool assertIsValid = true)
        {
            if (NumVertices < 3)
            {
            ...
            else
            {
                ...
                // at a fixed point on the sphere (S2PointUtil.Origin).  Historically this was
                ...
                // the fixed vector R = S2PointUtil.Ortho(B) is contained by the wedge ABC.  The
                ...
                // with S2.VertexCrossing.  (Note that we can't use S2PointUtil.Origin
                // as the fixed vector because of the possibility that B == S2PointUtil.Origin.)
                //
                // TODO(ericv): Investigate using vertex(0) as the reference point.

                ContainsOrigin = false;  // Initialize before calling Contains().
                bool v1_inside = S2Pred.OrderedCCW(S2PointUtil.Ortho(Vertex(1)), Vertex(0),
                                                    Vertex(2), Vertex(1));
        ...
        private void InitBound()
        {
            ...
            {
                b = new S2LatLngRect(new R1Interval(b.Lat.Lo, S2Constants.M_PI_2), S1Interval.Full);
            }
            ...
            {
                b = new S2LatLngRect(new R1Interval(-S2Constants.M_PI_2, b.Lat.Hi), b.Lng);
            }
            _bound = b;
            _subregionBound = S2LatLngRectBounder.ExpandForSubregions(_bound);
        }

        private void InitIndex(bool assertIsValid = true)
        {
        ...
        public bool BruteForceContains(S2Point p)
        {
            ...
            var crosser = new S2EdgeCrosser(S2PointUtil.Origin, p, Vertex(0));
        ...
        public void GetXYZFaceSiTiVertices(S2PointCompression.S2XYZFaceSiTi[] vertices, int offset)
        {
            ...
            {
                var xyz = Vertex(i);
                var cellLevel = S2Coords.XYZtoFaceSiTi(xyz, out int face, out uint si, out uint ti);
        ...
        public static bool DecodeCompressed(Decoder decoder, int snap_level, out S2Loop output)
        {
            ...
            S2LatLngRect subregionBound = null;
            if (BitsUtils.IsBitSet(properties_UInt32, (int)CompressedLoopProperty.kBoundEncoded))
            {
                var (success_bound, bound2) = S2LatLngRect.DecodeStatic(decoder);
                if (!success_bound)
                {
                    return false;
                }
        ...
        public S2LoopMeasures.LoopOrder GetCanonicalLoopOrder()
        {
            return S2LoopMeasures.GetCanonicalLoopOrder(VerticesSpan);
        }
        ...
        private static bool HasCrossingRelation(S2Loop a, S2Loop b, LoopRelation relation)
        {
            ...
            var ai = new S2ShapeUtil.RangeEnumerator(a._index);
            var bi = new S2ShapeUtil.RangeEnumerator(b._index);
        ...
        private static bool MatchBoundaries(S2Loop a, S2Loop b, int a_offset, S1Angle max_error)
        {
            ...
            while (pending.Any())
            {
                ...
                if (i < a.NumVertices && !done.Contains((i + 1, j)) &&
                    S2EdgeDistances.GetDistance(a.Vertex(io + 1), b.Vertex(j),
                                            b.Vertex(j + 1)) <= max_error)
                {
                    pending.Add((i + 1, j));
                }
                if (j < b.NumVertices && !done.Contains((i, j + 1)) &&
                    S2EdgeDistances.GetDistance(b.Vertex(j + 1), a.Vertex(io),
        ...
        public override object Clone()
        {
            return new S2Loop
            {
                ...
                _vertices = (S2Point[])_vertices.Clone(),
                _index = _index,
        ...
        public override bool Contains(S2Cell cell)
        {
            ...
            return Contains(ic, cell.GetCenter());
        }
        public override bool MayIntersect(S2Cell cell)
        {
            ...
            // Otherwise check if the loop contains the center of "target".
            return Contains(icell, target.GetCenter());
        }
        ...
        public static (bool success, S2Loop result) DecodeStatic(Decoder decoder)
        {
            ...
            var (success_bound, bound) = S2LatLngRect.DecodeStatic(decoder);
        ...
        {
            return obj is S2Loop loop && Equals(loop);
        }
        public override bool Equals(S2Loop b)
        {
            return _vertices.SequenceEqual(b._vertices);
        }
        public static bool operator ==(S2Loop left, S2Loop right)
        {
        ...
        {
            return LinqUtils.GetSequenceHashCode(_vertices);
        }
        ...
        {
            var sb = new StringBuilder("S2Loop, ");

            if (_vertices != null)
            {
                sb.Append(_vertices.Length).Append(" points. [");
        ...
        public class Shape : S2Shape
        {
            ...
            // S2Shape interface:
            public override int NumEdges => Loop.IsEmptyOrFull ? 0 : Loop.NumVertices;
            ...
            {
                return new ReferencePoint(S2PointUtil.Origin, Loop.ContainsOrigin);
            }
        ...
        internal class LoopCrosser
        {
            ...
            public bool HasCrossingRelation(S2ShapeUtil.RangeEnumerator ai, S2ShapeUtil.RangeEnumerator bi)
            {
            ...
            private bool HasCrossing(S2ShapeUtil.RangeEnumerator ai, S2ShapeUtil.RangeEnumerator bi)
            {
                Assert.True(ai.Id.Contains(bi.Id));
