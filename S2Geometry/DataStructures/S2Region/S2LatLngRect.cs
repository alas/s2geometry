...
namespace S2Geometry
{
    ...
    public sealed class S2LatLngRect : S2Region<S2LatLngRect>, IEquatable<S2LatLngRect>, ICoder
    {
        #region Fields, Constants

        public R1Interval Lat { get; }
        public S1Interval Lng { get; }
        ...
        public static readonly R1Interval FullLat = new R1Interval(-S2Constants.M_PI_2, S2Constants.M_PI_2);
        ...
        public S2LatLngRect(S2LatLng lo, S2LatLng hi)
        {
            ...
            if (!IsValid) System.Diagnostics.Debug.WriteLine($"Invalid rect: lo={lo}, hi={hi}");
        }
        ...
        public S2LatLngRect(R1Interval lat, S1Interval lng)
        {
            Lat = lat;
            Lng = lng;
            if (!IsValid) System.Diagnostics.Debug.WriteLine($"Invalid rect: lat={lat}, lng={lng}");
        }
        ...
        {
            if (!p.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng: {p}");
        ...
        {
            if (!p1.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng 1: {p1}");
            if (!p2.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng 2: {p2}");
        ...
        public S2LatLng Lo => new S2LatLng(LatLo, LngLo);
        public S2LatLng Hi => new S2LatLng(LatHi, LngHi);
        ...
        public bool IsValid
        {
            get
            {
                ...
                return Math.Abs(Lat.Lo) <= S2Constants.M_PI_2 &&
                       Math.Abs(Lat.Hi) <= S2Constants.M_PI_2 &&
                       Lng.IsValid &&
        ...
        public double Area()
        {
            ...
            // the longitude ratio.
            return Lng.Length * (Math.Sin(LatHi.Radians) - Math.Sin(LatLo.Radians));
        }
        ...
        public S2Point Centroid
        {
            get
            {
                ...
                var alpha = 0.5 * Lng.Length;
                var r = Math.Sin(alpha) * (r2 * z2 - r1 * z1 + Lat.Length);
                var lng = Lng.Center;
        ...
        {
            if (!ll.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng in S2LatLngRect.Contains: {ll}");
        ...
        {
            if (!ll.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng in S2LatLngRect.InteriorContains: {ll}");
        ...
        public bool BoundaryIntersects(S2Point v0, S2Point v1)
        {
            ...
            if (Lat.Lo != -S2Constants.M_PI_2 && IntersectsLatEdge(v0, v1, Lat.Lo, Lng))
            {
                return true;
            }
            if (Lat.Hi != S2Constants.M_PI_2 && IntersectsLatEdge(v0, v1, Lat.Hi, Lng))
            {
                return true;
            }
            return false;
        ...
        {
            if (!ll.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng in S2LatLngRect.AddPoint: {ll}");
        ...
        {
            if (Lat.Lo == -S2Constants.M_PI_2 || Lat.Hi == S2Constants.M_PI_2)
            {
        ...
        public S2LatLngRect ExpandedByDistance(S1Angle distance)
        {
            if (distance >= S1Angle.Zero)
            {
                ...
                for (int k = 0; k < 4; ++k)
                {
                    r = r.Union(new S2Cap(GetVertex(k).ToPoint(), radius).GetRectBound());
                }
                return r;
            }
            else
            {
                ...
                double max_lng_margin = sin_a < sin_c ? Math.Asin(sin_a / sin_c) : S2Constants.M_PI_2;
        ...
        public S1Angle GetDistance(S2LatLngRect other)
        {
            ...
            return new[]
            {
                S2EdgeDistances.GetDistance(a_lo, b_lo, b_hi),
                S2EdgeDistances.GetDistance(a_hi, b_lo, b_hi),
                S2EdgeDistances.GetDistance(b_lo, a_lo, a_hi),
                S2EdgeDistances.GetDistance(b_hi, a_lo, a_hi),
            }.Min();
        }
        ...
        public S1Angle GetDistance(S2LatLng p)
        {
            ...
            S2LatLngRect a = this;
            if (a.IsEmpty) System.Diagnostics.Debug.WriteLine($"Empty S2LatLngRect in GetDistance: {a}");
            if (!p.IsValid) System.Diagnostics.Debug.WriteLine($"Invalid S2LatLng in S2LatLngRect.GetDistance: {p}");
        ...
        {
            return ApproxEquals(other, S1Angle.FromRadians(S2Constants.DoubleError));
        }
        ...
        public static bool IntersectsLngEdge(S2Point a, S2Point b, R1Interval lat, double lng)
        {
            ...
            return S2EdgeCrossings.CrossingSign(a, b, p1, p2) > 0;
        }
        ...
        public static bool IntersectsLatEdge(S2Point a, S2Point b, double lat, S1Interval lng)
        {
            ...
            var z = S2PointUtil.RobustCrossProd(a, b).Normalized;
            if (z[2] < 0) z = -z;

            // Extend this to an orthonormal frame (x,y,z) where x is the direction
            // where the great circle through AB achieves its maximium latitude.
            var y = S2PointUtil.RobustCrossProd(z, new S2Point(0, 0, 1)).Normalized;
            var x = y.CrossProd(z);
        ...
        private static S1Angle GetDirectedHausdorffDistance(double lng_diff, R1Interval a, R1Interval b)
        {
            ...
            var max_distance = S2EdgeDistances.GetDistance(a_lo, b_lo, b_hi);
            max_distance = S1Angle.Max(max_distance, S2EdgeDistances.GetDistance(a_hi, b_lo, b_hi));

            if (lng_diff <= S2Constants.M_PI_2)
            {
                // Case A2.
        ...
        private static S1Angle GetInteriorMaxDistance(R1Interval a_lat, S2Point b)
        {
            ...
            // in a_lat.
            var intersection_point = new S2Point(-b.X, 0, -b.Z).Normalized;
        ...
        {
            lng = Math.Abs(lng);
            double lat_center = lat.Center;
            // A vector orthogonal to the bisector of the given longitudinal edge.
            S2LatLng ortho_bisector;
            if (lat_center >= 0)
            {
                ortho_bisector = S2LatLng.FromRadians(lat_center - S2Constants.M_PI_2, lng);
            }
            else
            {
                ortho_bisector = S2LatLng.FromRadians(-lat_center - S2Constants.M_PI_2, lng - Math.PI);
            }
            // A vector orthogonal to longitude 0.
            var ortho_lng = new S2Point(0, -1, 0);
            return S2PointUtil.RobustCrossProd(ortho_lng, ortho_bisector.ToPoint());
        } 
        ...
        public override object Clone() { return new S2LatLngRect(Lat, Lng); }
        public override S2Cap GetCapBound()
        {
            ...
            if (Lat.Lo + Lat.Hi < 0)
            {
                ...
                pole_z = -1;
                pole_angle = S2Constants.M_PI_2 + Lat.Hi;
            }
            else
            {
                pole_z = 1;
                pole_angle = S2Constants.M_PI_2 - Lat.Lo;
            }
            var pole_cap = new S2Cap(new S2Point(0, 0, pole_z), S1Angle.FromRadians(pole_angle));

            // For bounding rectangles that span 180 degrees or less in longitude, the
            // maximum cap size is achieved at one of the rectangle vertices.  For
            // rectangles that are larger than 180 degrees, we punt and always return a
            // bounding cap centered at one of the two poles.
            double lng_span = Lng.Hi - Lng.Lo;
            if (Math.IEEERemainder(lng_span, S2Constants.M_2_PI) >= 0 && lng_span < S2Constants.M_2_PI)
            {
                var mid_cap = new S2Cap(GetCenter().ToPoint(), S1Angle.FromRadians(0));
                for (int k = 0; k < 4; ++k)
                {
                    mid_cap = mid_cap.AddPoint(GetVertex(k).ToPoint());
                }
        ...
        public static (bool success, S2LatLngRect result) DecodeStatic(Decoder decoder)
        {
        ...
        public override bool Equals(S2LatLngRect other)
        {
