...
namespace S2Geometry
{
    ...
    public sealed class S2Cell : S2Region<S2Cell>, IEquatable<S2Cell>, IComparable<S2Cell>, ICoder
    {
        ...
        public readonly R2Rect BoundUV;
        ...
        private static readonly double kPoleMinLat = Math.Asin(Math.Sqrt(1.0 / 3)) - 0.5 * S2Constants.DoubleEpsilon;
        ...
        public bool IsLeaf => Level == S2Constants.kMaxCellLevel;
        ...
        public int SizeIJ => S2CellId.GetSizeIJ(Level);
        public double SizeST => S2CellId.GetSizeST(Level);
        ...
        public S2Point GetVertex(int k) => GetVertexRaw(k).Normalized;
        public S2Point GetVertexRaw(int k) => S2Coords.FaceUVtoXYZ(Face, BoundUV.GetVertex(k));
        ...
        public S2Point GetEdge(int k) => GetEdgeRaw(k).Normalized;
        public S2Point GetEdgeRaw(int k) => (k & 3) switch
        {
            0 => S2Coords.GetVNorm(Face, BoundUV[1][0]), // Bottom
            1 => S2Coords.GetUNorm(Face, BoundUV[0][1]), // Right
            2 => -S2Coords.GetVNorm(Face, BoundUV[1][1]),// Top
            _ => -S2Coords.GetUNorm(Face, BoundUV[0][0]),// Left
        };
        ...
        public bool Subdivide(S2Cell[] children)
        {
            ...
            for (int pos = 0; pos < 4; ++pos, id = id.Next)
            {
                ...
                int ij = S2Coords.kPosToIJ[Orientation][pos];
                int i = ij >> 1;
                int j = ij & 1;
                ...
                children[pos] = new S2Cell(id, Face, Level + 1, Orientation ^ S2Coords.kPosToOrientation[pos], uv);
            }
            return true;
        }
        ...
        // unit length.
        public S2Point GetCenter() => GetCenterRaw().Normalized;
        public S2Point GetCenterRaw() => Id.ToPointRaw();

        // Returns the average area for cells at the given level.
        public static double AverageArea(int level) => S2Metrics.kAvgArea.GetValue(level);
        ...
        public double ApproxArea()
        {
            ...
            return flat_area * 2 / (1 + Math.Sqrt(1 - Math.Min(S2Constants.M_1_PI * flat_area, 1.0)));
        }
        ...
        public S1ChordAngle GetMaxDistance(S2Point target)
        {
            ...
            S2Point target_uvw = S2Coords.FaceXYZtoUVW(Face, target);
        ...
        public S1ChordAngle GetDistance(S2Point a, S2Point b)
        {
            ...
            var min_dist = S1ChordAngle.Min(GetDistance(a), GetDistance(b));
            ...
            for (int i = 0; i < 4; ++i)
            {
                S2EdgeDistances.UpdateMinDistance(v[i], a, b, ref min_dist);
            }
            return min_dist;
        }
        ...
        public S1ChordAngle GetMaxDistance(S2Point a, S2Point b)
        {
            ...
            var max_dist = S1ChordAngle.Max(GetMaxDistance(a), GetMaxDistance(b));
            if (max_dist <= S1ChordAngle.Right)
            {
                return max_dist;
            }
        ...
        public S1ChordAngle GetDistance(S2Cell target)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                for (int j = 0; j < 4; ++j)
                {
                    S2EdgeDistances.UpdateMinDistance(va[i], vb[j], vb[(j + 1) & 3], ref min_dist);
                    S2EdgeDistances.UpdateMinDistance(vb[i], va[j], va[(j + 1) & 3], ref min_dist);
                }
            }
            return min_dist;
        }
        ...
        public S1ChordAngle GetMaxDistance(S2Cell target)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                for (int j = 0; j < 4; ++j)
                {
                    S2EdgeDistances.UpdateMaxDistance(va[i], vb[j], vb[(j + 1) & 3], ref max_dist);
                    S2EdgeDistances.UpdateMaxDistance(vb[i], va[j], va[(j + 1) & 3], ref max_dist);
                }
            }
            return max_dist;
        }
        ...
        {
            var vertex = new S2Point(BoundUV[0][i], BoundUV[1][j], 1).Normalized;
        ...
        private S1ChordAngle GetDistanceInternal(S2Point target_xyz, bool to_interior)
        {
            // All calculations are done in the (u,v,w) coordinates of this cell's face.
            S2Point target = S2Coords.FaceXYZtoUVW(Face, target_xyz);
        ...
        public override S2Cap GetCapBound()
        {
            ...
            var center = S2Coords.FaceUVtoXYZ(Face, BoundUV.Center).Normalized;
        ...
        public override S2LatLngRect GetRectBound()
        {
            if (Level > 0)
            {
                ...
                int i = (S2Coords.GetUAxis(Face)[2] == 0 ? (u < 0) : (u > 0)) ? 1 : 0;
                int j = (S2Coords.GetVAxis(Face)[2] == 0 ? (v < 0) : (v > 0)) ? 1 : 0;
                var lat = R1Interval.FromPointPair(GetLatitude(i, j),
                                                           GetLatitude(1 - i, 1 - j));
                var lng = S1Interval.FromPointPair(GetLongitude(i, 1 - j),
                                                           GetLongitude(1 - i, j));
                ...
                return new S2LatLngRect(lat, lng)
                    .Expanded(S2LatLng.FromRadians(2 * S2Constants.DoubleEpsilon, 2 * S2Constants.DoubleEpsilon))
                    .PolarClosure();
            }

            // The face centers are the +X, +Y, +Z, -X, -Y, -Z axes in that order.
            Assert.True(((Face < 3) ? 1 : -1) == S2Coords.GetNorm(Face)[Face % 3]);
            var bound = Face switch
            {
                0 => new S2LatLngRect(new R1Interval(-S2Constants.M_PI_4, S2Constants.M_PI_4),
                                      new S1Interval(-S2Constants.M_PI_4, S2Constants.M_PI_4)),
                1 => new S2LatLngRect(new R1Interval(-S2Constants.M_PI_4, S2Constants.M_PI_4),
                                      new S1Interval(S2Constants.M_PI_4, 3 * S2Constants.M_PI_4)),
                2 => new S2LatLngRect(new R1Interval(kPoleMinLat, S2Constants.M_PI_2), S1Interval.Full),
                3 => new S2LatLngRect(new R1Interval(-S2Constants.M_PI_4, S2Constants.M_PI_4),
                                      new S1Interval(3 * S2Constants.M_PI_4, -3 * S2Constants.M_PI_4)),
                4 => new S2LatLngRect(new R1Interval(-S2Constants.M_PI_4, S2Constants.M_PI_4),
                                      new S1Interval(-3 * S2Constants.M_PI_4, -S2Constants.M_PI_4)),
                _ => new S2LatLngRect(new R1Interval(-S2Constants.M_PI_2, -kPoleMinLat), S1Interval.Full),
            };
            ...
            return bound.Expanded(S2LatLng.FromRadians(S2Constants.DoubleEpsilon, 0));
        }
        ...
        public override bool Contains(S2Point p)
        {
            ...
            if (!S2Coords.FaceXYZtoUV(Face, p, out R2Point uv)) return false;
            ...
            return BoundUV.Expanded(S2Constants.DoubleEpsilon).Contains(uv);
        }
        ...
        public static (bool success, S2Cell result) DecodeStatic(Decoder decoder)
        {
            var (success, id) = S2CellId.DecodeStatic(decoder);
            if (!success)
                return (false, null);
        ...
        public override bool Equals(S2Cell cell)
        {
            return Equals(Id, cell.Id);
        }
        ...
        public static bool operator <(S2Cell x, S2Cell y)
        {
            return x.Id < y.Id;
        }
