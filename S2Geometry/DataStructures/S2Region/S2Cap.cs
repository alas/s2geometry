...
namespace S2Geometry
{
    ...
    public sealed class S2Cap : S2Region<S2Cap>, ICoder
    {
        #region Fields, Constants

        public readonly S2Point Center;
        public readonly S1ChordAngle Radius;

        // an empty cap, i.e. a cap that contains no points.
        public static readonly S2Cap Empty = new S2Cap(new S2Point(1, 0, 0), S1ChordAngle.Negative);

        // Return a full cap, i.e. a cap that contains all points.
        public static readonly S2Cap Full = new S2Cap(new S2Point(1, 0, 0), S1ChordAngle.Straight);
        ...
        public double Area => S2Constants.M_2_PI * Math.Max(0.0, Height);
        ...
        public S2Cap AddCap(S2Cap other)
        {
            ...
            else if (!other.IsEmpty)
            {
                ...
                var newRadius = S1ChordAngle.Max(Radius, dist.PlusError(S2Constants.DoubleEpsilon * dist.Length2));
        ...
        {
            Assert.True(distance.Radians >= 0);
        ...
        public S2Cap Union(S2Cap other)
        {
            ...
            var distance = new S1Angle(Center, other.Center);
            ...
            var result_center = S2EdgeDistances.InterpolateAtDistance(
        ...
        public bool ApproxEquals(S2Cap other, S1Angle max_error_angle)
        {
            ...
            return (S2PointUtil.ApproxEquals(Center, other.Center, max_error_angle) &&
        ...
        public override S2LatLngRect GetRectBound()
        {
            ...
            if (lat[0] <= -S2Constants.M_PI_2)
            {
                lat[0] = -S2Constants.M_PI_2;
                all_longitudes = true;
            }
            // Check whether cap includes the north pole.
            lat[1] = center_ll.LatRadians + cap_angle;
            if (lat[1] >= S2Constants.M_PI_2)
            {
                lat[1] = S2Constants.M_PI_2;
                all_longitudes = true;
            }
            if (!all_longitudes)
            {
                ...
                if (sin_a <= sin_c)
                {
                    ...
                    lng[0] = Math.IEEERemainder(center_ll.LngRadians - angle_A, S2Constants.M_2_PI);
                    lng[1] = Math.IEEERemainder(center_ll.LngRadians + angle_A, S2Constants.M_2_PI);
                }
            }
        ...
        public override void GetCellUnionBound(List<S2CellId> cellsId)
        {
            ...
            int level = S2Metrics.kMinWidth.GetLevelForMinValue(RadiusAngle.Radians) - 1;
        ...
        /// </summary>
        public override bool Equals(S2Cap other)
        {
        ...
        public static (bool success, S2Cap result) DecodeStatic(Decoder d)
        {
...
#if s2debug
            Assert.True(result.IsValid); // Invalid S2Cap
#endif
        ...
        {
            return $"[Center={Center}, Radius={RadiusAngle}]";
        }
