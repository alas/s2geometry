...
namespace S2Geometry
{
    ...
    public sealed class S2CellUnion : S2Region<S2CellUnion>, IEquatable<S2CellUnion>, IEnumerable<S2CellId>, ICoder
    {
        #region Fields, Constants

        public List<S2CellId> CellIds { get; private set; }

        // Constructs a cell union for the whole sphere.
        public static readonly S2CellUnion WholeSphere = new S2CellUnion(new int[] { 0, 1, 2, 3, 4, 5 }
        ...
        public S2CellUnion() { CellIds = new List<S2CellId>(); }
        ...
        public S2CellId this[int i] => CellIds[i];
        ...
        public int NumCells => CellIds.Count;
...
#pragma warning restore IDE0060 // Quitar el parámetro no utilizado
        ...
        public bool Contains(S2CellId id)
        {
            ...
            var i = CellIds.GetLowerBound(id);
            if (i != CellIds.Count && CellIds[i].RangeMin <= id) return true;
            return i != 0 && CellIds[--i].RangeMax >= id;
        }
        ...
        public bool Intersects(S2CellId id)
        {
            ...
            var i = CellIds.GetLowerBound(id);
            if (i != CellIds.Count && CellIds[i].RangeMin <= id.RangeMax) return true;
            return i != 0 && CellIds[--i].RangeMax >= id.RangeMin;
        }
        ...
        public S2CellUnion Expand(S1Angle min_radius, int max_level_diff)
        {
            ...
            int radius_level = S2Metrics.kMinWidth.GetLevelForMinValue(min_radius.Radians);
            if (radius_level == 0 && min_radius.Radians > S2Metrics.kMinWidth.GetValue(0))
            {
        ...
        public UInt64 LeafCellsCovered()
        {
            ...
            foreach (S2CellId id in this)
            {
                int inverted_level = S2Constants.kMaxCellLevel - id.Level;
        ...
        {
            return S2Cell.AverageArea(S2Constants.kMaxCellLevel) * LeafCellsCovered();
        }
        ...
        public static bool Normalize(List<S2CellId> ids)
        {
            ...
            if (ids.Count == output) return false;
        ...
        public static void Denormalize(List<S2CellId> input, int min_level, int level_mod, List<S2CellId> output)
        {
            ...
            Assert.True(min_level <= S2Constants.kMaxCellLevel);
            ...
            foreach (var id in input)
            {
                ...
                if (level_mod > 1)
                {
                    ...
                    new_level += (S2Constants.kMaxCellLevel - (new_level - min_level)) % level_mod;
                    new_level = Math.Min(S2Constants.kMaxCellLevel, new_level);
                }
                if (new_level == level)
                {
                    output.Add(id);
            ...
            while (i != x.Count && j != y.Count)
            {
                ...
                if (imin > jmin)
                {
                    ...
                    else
                    {
                        ...
                        j = y.GetLowerBound(imin, j + 1, y.Count);
                        // The previous cell *(j-1) may now contain *i.
                        if (x[i] <= y[j - 1].RangeMax) --j;
                    }
                }
                else if (jmin > imin)
                {
                    ...
                    {
                        i = x.GetLowerBound(jmin, i + 1, x.Count);
        ...
        {
            return new S2CellUnion(CellIds, false);
        }
        ...
        public static (bool success, S2CellUnion result) DecodeStatic(Decoder decoder)
        {
            ...
            var temp_cell_ids = new S2CellId[num_cells];
            ...
            var result = new S2CellUnion(temp_cell_ids.ToList());
            return (true, result);
        }
        ...
        public override bool Equals(S2CellUnion other)
        {
            return CellIds.SequenceEqual(other.CellIds);
        }

        // Return true if two cell unions are identical.
        public static bool operator ==(S2CellUnion x, S2CellUnion y)
        {
            return Equals(x, y);
        }
