...
namespace S2Geometry
{
    ...
    public class S2CellId : IEquatable<S2CellId>, IComparable<S2CellId>, ICoder
    {
        ...
        public static readonly S2CellId None = new S2CellId(0);
        ...
        public const int kPosBits = 2 * S2Constants.kMaxCellLevel + 1;
        public const int kMaxSize = 1 << S2Constants.kMaxCellLevel;
        private const int kLookupBits = 4;
        private const double kScale = 1.0 / kMaxSize;
        private const double kLimit = 1.0 + S2Constants.DoubleEpsilon;
        ...
        public S2CellId(UInt64 id = 0) { Id = id; }
        ...
        public S2CellId(S2Point p)
        {
            int face = S2Coords.XYZtoFaceUV(p, out double u, out double v);
            int i = S2Coords.STtoIJ(S2Coords.UVtoST(u));
            int j = S2Coords.STtoIJ(S2Coords.UVtoST(v));
            Id = FromFaceIJ(face, i, j).Id;
        }
        ...
        public static S2CellId FromDebugString(string str)
        {
            ...
            if (level < 0 || level > S2Constants.kMaxCellLevel) return None;
            int face = str[0] - '0';
        ...
        public S2Point ToPoint() => ToPointRaw().Normalized;
        public S2Point ToPointRaw()
        {
            int face = GetCenterSiTi(out int si, out int ti);
            return S2Coords.FaceSiTitoXYZ(face, (uint)si, (uint)ti);
        }

        // Return the center of the cell in (s,t) coordinates (see s2coords.h).
        public R2Point GetCenterST()
        {
            GetCenterSiTi(out int si, out int ti);
            return new R2Point(S2Coords.SiTitoST((uint)si), S2Coords.SiTitoST((uint)ti));
        }

        // Return the edge length of this cell in (s,t)-space.
        public double GetSizeST() => GetSizeST(Level);

        // Return the edge length in (s,t)-space of cells at the given level.
        public static double GetSizeST(int level) => S2Coords.IJtoSTMin(GetSizeIJ(level));

        // Return the bounds of this cell in (s,t)-space.
        public R2Rect GetBoundST()
        {
            double size = GetSizeST();
            return R2Rect.FromCenterSize(GetCenterST(), new R2Point(size, size));
        }
        ...
        public R2Point GetCenterUV()
        {
            GetCenterSiTi(out int si, out int ti);
            return new R2Point(
                S2Coords.STtoUV(S2Coords.SiTitoST((uint)si)),
                S2Coords.STtoUV(S2Coords.SiTitoST((uint)ti)));
        }
        ...
        public int GetCenterSiTi(out int psi, out int pti)
        {
        ...
        public bool IsValid
        {
            get
            {
                return Face < kNumFaces && (LowestOnBit & 0x1555555555555555UL) != 0;
            }
        }

        // Which cube face this cell belongs to, in the range 0..5.
        public UInt64 Face => Id >> kPosBits;

        // The position of the cell center along the Hilbert curve over this face,
        // in the range 0..(2**kPosBits-1).
        public UInt64 Pos => Id & (~0UL >> kFaceBits);
        ...
        public int Level
        {
            get
            {
                ...
                if (IsLeaf)
                {
                    return S2Constants.kMaxCellLevel;
                }
                var x = (uint)Id;
                var level = -1;
                if (x != 0)
        ...
        public int ChildPosition(int level)
        {
            ...
            return (int)(Id >> (2 * (S2Constants.kMaxCellLevel - level) + 1)) & 3;
        }
        ...
        public S2CellId RangeMin => new S2CellId(Id - (LowestOnBit - 1));

        public S2CellId RangeMax => new S2CellId(Id + (LowestOnBit - 1));
        ...
        public S2CellId Next => new S2CellId(Id + (LowestOnBit << 1));

        public S2CellId Prev => new S2CellId(Id - (LowestOnBit << 1));
        ...
        public S2CellId Advance(Int64 steps)
        {
            ...
            int step_shift = 2 * (S2Constants.kMaxCellLevel - Level) + 1;
            if (steps < 0)
            {
        ...
        public int GetCommonAncestorLevel(S2CellId other)
        {
            ...
            Assert.True(bits != 0);  // Because LowestOnBit is non-zero.
        ...
        public static S2CellId FromToken(string token, int length)
        {
            ...
            token = (token + "0000000000000000").Substring(0, 16);
        ...
        public static S2CellId FromFaceIJ(int face, int i, int j)
        {
            ...
            // coordinate system.
            UInt64 bits = (UInt64)(face & S2Constants.kSwapMask);
            ...
            for (var k = 7; k >= 0; k--)
            {
                ...
                bits &= (S2Constants.kSwapMask | S2Constants.kInvertMask);
            }
        ...
        public int ToFaceIJOrientation(out int pi, out int pj, out int orientation, bool setOrientation)
        {
            ...
            int i = 0, j = 0;
            var face = (int)Face;
            int bits = face & S2Constants.kSwapMask;
            ...
            {
                int nbits = (k == 7)
                    ? (S2Constants.kMaxCellLevel - 7 * kLookupBits)
                    : kLookupBits;
            ...
            if (setOrientation)
            {
                ...
                // the kSwapMask bit.
                Assert.True(0 == S2Coords.kPosToOrientation[2]);
                Assert.True(S2Constants.kSwapMask == S2Coords.kPosToOrientation[0]);
                if ((LowestOnBit & 0x1111111111111110UL) != 0)
                {
                    bits ^= S2Constants.kSwapMask;
                }
                orientation = bits;
            }
            else
        ...
        public UInt64 LowestOnBit => Id & (~Id + 1);

        // Return the lowest-numbered bit that is on for cells at the given level.
        public static UInt64 LowestOnBitForLevel(int level)
        {
            return 1UL << (2 * (S2Constants.kMaxCellLevel - level));
        }
        ...
        public static R2Rect IJLevelToBoundUV(int[] ij, int level)
        {
            ...
            for (int d = 0; d < 2; ++d)
            {
                ...
                r[d] = new R1Interval(
                    S2Coords.STtoUV(S2Coords.IJtoSTMin(ij_lo)),
                    S2Coords.STtoUV(S2Coords.IJtoSTMin(ij_hi)));
            }
        ...
        private static S2CellId FromFaceIJWrap(int face, int i, int j)
        {
            ...
            face = S2Coords.XYZtoFaceUV(S2Coords.FaceUVtoXYZ(face, u, v), out u, out v);
        ...
        private static S2CellId FromFaceIJSame(int face, int i, int j, bool same_face)
        {
            if (same_face)
        ...
        private static void InitLookupCell(int level, int i, int j, int orig_orientation, int pos, int orientation)
        {
            ...
            else
            {
                ...
                j <<= 1;
                pos <<= 2;
                var r = S2Coords.kPosToIJ[orientation];
                InitLookupCell(level, i + (r[0] >> 1), j + (r[0] & 1), orig_orientation,
                               pos, orientation ^ S2Coords.kPosToOrientation[0]);
                InitLookupCell(level, i + (r[1] >> 1), j + (r[1] & 1), orig_orientation,
                               pos + 1, orientation ^ S2Coords.kPosToOrientation[1]);
                InitLookupCell(level, i + (r[2] >> 1), j + (r[2] & 1), orig_orientation,
                               pos + 2, orientation ^ S2Coords.kPosToOrientation[2]);
                InitLookupCell(level, i + (r[3] >> 1), j + (r[3] & 1), orig_orientation,
                               pos + 3, orientation ^ S2Coords.kPosToOrientation[3]);
            }
        }
        ...
        private static void MaybeInit()
        {
            if (!callOnceFlag)
            {
                ...
                InitLookupCell(0, 0, 0, S2Constants.kSwapMask, 0, S2Constants.kSwapMask);
                InitLookupCell(0, 0, 0, S2Constants.kInvertMask, 0, S2Constants.kInvertMask);
                InitLookupCell(0, 0, 0, S2Constants.kSwapMask | S2Constants.kInvertMask, 0, S2Constants.kSwapMask | S2Constants.kInvertMask);
                callOnceFlag = true;
            }
        } 
        ...
        public static (bool success, S2CellId result) DecodeStatic(Decoder decoder)
        {
        ...
        {
            return obj is S2CellId cell && Equals(cell);
        }
        ...
        //typedef true_type goog_btree_prefer_linear_node_search;
        ...
        public override string ToString()
        {
            ...
            for (int current_level = 1; current_level <= Level; ++current_level)
            {
                ...
                // to_string.
                output += "0123"[ChildPosition(current_level)];
            }
            return output;
        }
