	...
	public class S2PaddedCell
	{
		...
		public R2Rect Middle
		{
			get
			{
				...
				if (!middle_.HasValue)
				{
					int ij_size = S2CellId.GetSizeIJ(Level);
					double u = S2Coords.STtoUV(S2Coords.SiTitoST((uint)(2 * ij_lo_[0] + ij_size)));
					double v = S2Coords.STtoUV(S2Coords.SiTitoST((uint)(2 * ij_lo_[1] + ij_size)));
        ...
        public S2PaddedCell(S2PaddedCell parent, int i, int j)
        {
            ...
            int pos = S2Coords.kIJtoPos[parent.orientation_][2 * i + j];
            Id = parent.Id.Child(pos);
            int ij_size = S2CellId.GetSizeIJ(Level);
            ij_lo_[0] = parent.ij_lo_[0] + i * ij_size;
            ij_lo_[1] = parent.ij_lo_[1] + j * ij_size;
            orientation_ = parent.orientation_ ^ S2Coords.kPosToOrientation[pos];
		...
		{
			int ij = S2Coords.kPosToIJ[orientation_][pos];
			i = ij >> 1;
			j = ij & 1;
		}
		...
		public S2CellId ShrinkToFit(R2Rect rect)
		{
			...
			{
				if (rect[0].Contains(S2Coords.STtoUV(S2Coords.SiTitoST((uint)(2 * ij_lo_[0] + ij_size)))) ||
					rect[1].Contains(S2Coords.STtoUV(S2Coords.SiTitoST((uint)(2 * ij_lo_[1] + ij_size)))))
				{
					return Id;
				}
			}
			...
			R2Rect padded = rect.Expanded(Padding+ 1.5 * S2Constants.DoubleEpsilon);
			...
			for (int d = 0; d < 2; ++d)
			{
				ij_min[d] = Math.Max(ij_lo_[d], S2Coords.STtoIJ(S2Coords.UVtoST(padded[d][0])));
				int ij_max = Math.Min(ij_lo_[d] + ij_size - 1,
								 S2Coords.STtoIJ(S2Coords.UVtoST(padded[d][1])));
			...
			var level = S2Constants.kMaxCellLevel - BitsUtils.FindMSBSetNonZero((uint)level_msb);
		...
		{
			var ij_size = S2CellId.GetSizeIJ(Level);
			var si = (uint)(2 * ij_lo_[0] + ij_size);
			var ti = (uint)(2 * ij_lo_[1] + ij_size);
			return S2Coords.FaceSiTitoXYZ((int)Id.Face, si, ti).Normalized;
		}
		...
		public S2Point GetEntryVertex()
		{
			...
			if ((orientation_ & S2Constants.kInvertMask) != 0)
			{
				int ij_size = S2CellId.GetSizeIJ(Level);
				i = (uint)(i + ij_size);
				j = (uint)(j + ij_size);
			}
			return S2Coords.FaceSiTitoXYZ((int)Id.Face, 2 * i, 2 * j).Normalized;
		}
		...
		public S2Point GetExitVertex()
		{
			...
			uint j = (uint)ij_lo_[1];
			int ij_size = S2CellId.GetSizeIJ(Level);
			if (orientation_ == 0 || orientation_ == S2Constants.kSwapMask + S2Constants.kInvertMask)
			{
				i = (uint)(i + ij_size);
			}
			else
