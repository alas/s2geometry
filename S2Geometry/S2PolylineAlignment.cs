...
using System.Linq;
using System.Text;
using WarpPath = System.Collections.Generic.List<(int start, int end)>;
...
namespace S2Geometry
{
    ...
    public static class S2PolylineAlignment
    {
        ...
        public static VertexAlignment GetExactVertexAlignment(S2Polyline a, S2Polyline b)
        {
            ...
            Assert.True(a_n > 0); // A is empty polyline.
            Assert.True(b_n > 0); // B is empty polyline.
        ...
        public static VertexAlignment GetApproxVertexAlignment(S2Polyline a, S2Polyline b, int radius)
        {
            ...
            Assert.True(a_n > 0); // A is empty polyline.
            Assert.True(b_n > 0); // B is empty polyline.
            Assert.True(radius >= 0); // Radius is negative.
        ...
        public static S2Polyline GetConsensusPolyline(S2Polyline[] polylines, ConsensusOptions options)
        {
            ...
            var consensus = new S2Polyline((S2Polyline)polylines[seed_index].Clone());
            int num_consensus_vertices = consensus.NumVertices;
            Assert.True(num_consensus_vertices > 1);
        ...
        public class Window
        {
            ...
            {
                Assert.True(strides.Any()); // Cannotruct empty window.
                Assert.True(strides[0].start == 0); // First element of start_cols is non-zero.
                strides_ = strides;
                rows_ = strides.Length;
                cols_ = strides.Last().end;
                Assert.True(IsValid); // Constructor validity check fail.
            }

            // Construct a Window from a non-empty sequence of warp path index pairs.
            public Window(WarpPath warp_path)
            {
                Assert.True(warp_path.Any()); // Cannot construct window from empty warp path.
                Assert.True(warp_path.First() == (0, 0)); // Must start at (0, 0).
                rows_ = warp_path.Last().start + 1;
                Assert.True(rows_ > 0); // Must have at least one row.
                cols_ = warp_path.Last().end + 1;
                Assert.True(cols_ > 0); // Must have at least one column.
            ...
            {
                Assert.True(new_rows >= rows_); // Upsampling: New_rows < current_rows
                Assert.True(new_cols >= cols_); // Upsampling: New_cols < current_cols
            ...
            public Window Dilate(int radius)
            {
                ...
                Assert.True(radius >= 0); // Negative dilation radius.
