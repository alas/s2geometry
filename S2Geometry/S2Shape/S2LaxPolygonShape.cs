...
public class S2LaxPolygonShape : S2Shape, IInitEncoder<S2LaxPolygonShape>
{
    ...
    public void Init(List<Loop> loops)
    {
        ...
        {
            loop_starts_ = new uint[NumLoops + 1];
            NumVertices = 0;
    ...
    public static (bool, S2LaxPolygonShape?) Init(Decoder decoder)
    {
        ...
        if (numLoops != 0)
        {
            ...
            if (numLoops > 1)
            {
                var (success2, loop_starts) = EncodedUIntVector<uint>.Init(decoder);
                if (!success2) return (false, null);
                loop_starts_ = new uint[loop_starts!.Count];
...
public class EncodedS2LaxPolygonShape : S2Shape, IInitEncoder<EncodedS2LaxPolygonShape>
{
    ...
    public EncodedUIntVector<uint>? LoopStarts { private get; init; }
    ...
    public static (bool, EncodedS2LaxPolygonShape?) Init(Decoder decoder)
    {
        ...
        EncodedUIntVector<uint>? loop_starts_ = null;
        if (num_loops > 1)
        {
            var (success, ls) = EncodedUIntVector<uint>.Init(decoder);
            if (!success) return (false, null);
            loop_starts_ = ls;
        }
