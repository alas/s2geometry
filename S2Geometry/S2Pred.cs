...
namespace S2Geometry
{
    ...
    /// 
    /// int CrossingSign(S2Point a0, S2Point a1,
    ///                  S2Point b0, S2Point b1);
    /// 
    /// bool EdgeOrVertexCrossing(S2Point a0, S2Point a1,
    ///                           S2Point b0, S2Point b1);
    /// </summary>
    public static class S2Pred
    {
        ...
        public static int CompareDistances(S2Point x, S2Point a, S2Point b)
        {
            ...
            if (cos_ax > S2Constants.M_SQRT1_2)
            {
                // Angles < 45 degrees.
                sign = CompareSin2Distances(x, a, b);
            }
            else if (cos_ax < -S2Constants.M_SQRT1_2)
            {
        ...
        public static int CompareDistance(S2Point x, S2Point y, S1ChordAngle r)
        {
            ...
            /*else
            {
                sign = TriageCompareCosDistance(x, y, r.Length2);
            }*/
            if (sign != 0) return sign;
            return ExactCompareDistance(x, y, r.Length2);
        }
        ...
        public static int CompareEdgeDistance(S2Point x, S2Point a0, S2Point a1, S1ChordAngle r)
        {
            ...
            sign = TriageCompareEdgeDistance(x, a0, a1, r.Length2);
            if (sign != 0) return sign;
        ...
        public static int CompareEdgeDirections(S2Point a0, S2Point a1, S2Point b0, S2Point b1)
        {
            ...
            sign = TriageCompareEdgeDirections(a0, a1, b0, b1);
            if (sign != 0) return sign;
        ...
        public static int EdgeCircumcenterSign(S2Point x0, S2Point x1, S2Point a, S2Point b, S2Point c)
        {
            ...
            sign = TriageEdgeCircumcenterSign(x0, x1, a, b, c, abc_sign);
            if (sign != 0) return sign;
        ...
        {
            Assert.True(r < S1ChordAngle.Right);
            Assert.True(CompareDistances(x0, a, b) < 0);  // (implies a != b)
            Assert.True(CompareEdgeDistance(a, x0, x1, r) <= 0);
            Assert.True(CompareEdgeDistance(b, x0, x1, r) <= 0);
            // Check that the edge does not consist of antipodal points.  (This catches
            ...
            result = TriageVoronoiSiteExclusion(a, b, x0, x1, r.Length2);
            if (result != Excluded.UNCERTAIN) return result;
        ...
        public static int TriageSign(S2Point a, S2Point b, S2Point c, S2Point a_cross_b)
        {
            ...
            double kMaxDetError = 1.8274 * S2Constants.DoubleEpsilon;
            ...
            Assert.True(Math.Abs(det) <= kMaxDetError || Math.Abs(det) >= 100 * kMaxDetError || det * ExpensiveSign(a, b, c) > 0);
#endif
        ...
        public const double TT_ERR = S2Constants.DoubleEpsilon;
        public const double DBL_ERR = S2Constants.DoubleEpsilon;
        public const double LD_ERR = S2Constants.DoubleEpsilon;

        // A predefined S1ChordAngle representing (approximately) 45 degrees.
#pragma warning disable IDE1006 // Estilos de nombres
        private static S1ChordAngle k45Degrees { get; } = S1ChordAngle.FromLength2(2 - S2Constants.M_SQRT2);
#pragma warning restore IDE1006 // Estilos de nombres
        ...
        private static int StableSign(S2Point a, S2Point b, S2Point c)
        {
            var ab = b - a;
            var bc = c - b;
            var ca = a - c;
            ...
            var kDetErrorMultiplier = 3.2321 * S2Constants.DoubleEpsilon;  // see above
        ...
        private static int SymbolicallyPerturbedSign(S2Point a, S2Point b, S2Point c, S2Point b_cross_c)
        {
            ...
            Assert.True(0 == (c[1] * a[2] - c[2] * a[1]).Sgn());  // db[0]
        ...
        /// </summary>
        private static int ExactSign(S2Point a, S2Point b, S2Point c, bool perturb)
        {
...
#pragma warning restore IDE0060 // Quitar el parámetro no utilizado
        ...
        {
            Assert.True(r2 < 2.0);  // Only valid for distance limits < 90 degrees.
...
#pragma warning restore IDE0060 // Quitar el parámetro no utilizado
        ...
        private static int TriageCompareEdgeDistance(S2Point x, S2Point a0, S2Point a1, double r2)
        {
            ...
            if (Math.Abs(a0_sign) < a0_sign_error || Math.Abs(a1_sign) < a1_sign_error)
            {
        ...
        {
            Assert.True(!ArePointsAntipodal(a0, a1));
            Assert.True(!ArePointsAntipodal(b0, b1));
        ...
        private static int ExactEdgeCircumcenterSign(S2Point x0, S2Point x1, S2Point a, S2Point b, S2Point c, int abc_sign)
        {
            ...
            {
                Assert.True(x0.DotProd(x1) > 0);  // Antipodal edges not allowed.
                return 0;
            }
        ...
        private static Excluded TriageVoronoiSiteExclusion(S2Point a, S2Point b, S2Point x0, S2Point x1, double r2)
        {
            ...
            Assert.True(abs_lhs3 > lhs3_error);
        ...
        {
            Assert.True(!ArePointsAntipodal(x0, x1));
