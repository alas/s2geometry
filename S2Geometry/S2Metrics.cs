...
namespace S2Geometry
{
	...
	public static class S2Metrics
	{
		public abstract class Metric
		{
			...
			{
				return GetLevelForMaxValue((Dim == 1 ? S2Constants.M_SQRT2 : 2) * value);
			}

			// Return the minimum level such that the metric is at most the given
			// value, or S2Constants.kMaxCellLevel if there is no such level.  For example,
			...
			{
				if (value <= 0) return S2Constants.kMaxCellLevel;

				// This code is equivalent to computing a floating-point "level"
				// value and rounding up.  Math.ILogB() returns the exponent corresponding to a
				// fraction in the range [1,2).
				int level = Math.ILogB(value / Deriv);
				level = Math.Max(0, Math.Min(S2Constants.kMaxCellLevel, -(level >> (Dim - 1))));
				Assert.True(level == S2Constants.kMaxCellLevel || GetValue(level) <= value);
				Assert.True(level == 0 || GetValue(level - 1) > value);
				return level;
			}

			// Return the maximum level such that the metric is at least the given
			// value, or zero if there is no such level.  For example,
			// S2Metrics.kMinWidth.GetLevelForMinValue(0.1) returns the maximum level such
			...
			public int GetLevelForMinValue(double value)
			{
				...
				level = Math.Max(0, Math.Min(S2Constants.kMaxCellLevel, level >> (Dim - 1)));
				Assert.True(level == 0 || GetValue(level) >= value);
				Assert.True(level == S2Constants.kMaxCellLevel || GetValue(level + 1) < value);
				return level;
			}
		}
...
#pragma warning disable IDE1006 // Estilos de nombres
        public static readonly LengthMetric kMinAngleSpan = new LengthMetric(
		...
		public static readonly LengthMetric kMaxAngleSpan = new LengthMetric(
#if S2_LINEAR_PROJECTION
			2																			// 2.000
#elif S2_TAN_PROJECTION
			S2Constants.M_PI_2															// 1.570
		...
		public static readonly LengthMetric kAvgAngleSpan = new LengthMetric(S2Constants.M_PI_2);
		...
		public static readonly LengthMetric kMinWidth = new LengthMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly LengthMetric kAvgWidth = new LengthMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly LengthMetric kMinEdge = new LengthMetric(
#if S2_LINEAR_PROJECTION
			2 * S2Constants.M_SQRT2 / 3													// 0.943
#elif S2_TAN_PROJECTION
			Math.PI / (2 * S2Constants.M_SQRT2)											// 1.111
		...
		public static readonly LengthMetric kAvgEdge = new LengthMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly LengthMetric kMinDiag = new LengthMetric(
#if S2_LINEAR_PROJECTION
			2 * S2Constants.M_SQRT2 / 3													// 0.943
#elif S2_TAN_PROJECTION
			Math.PI* S2Constants.M_SQRT2 / 3											// 1.481
#elif S2_QUADRATIC_PROJECTION
			8 * S2Constants.M_SQRT2 / 9													// 1.257
#else
			0
#endif
		);

		public static readonly LengthMetric kMaxDiag = new LengthMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly LengthMetric kAvgDiag = new LengthMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly AreaMetric kMinArea = new AreaMetric(
#if S2_LINEAR_PROJECTION
		...
		public static readonly AreaMetric kMaxArea = new AreaMetric(
#if S2_LINEAR_PROJECTION
			4																			// 4.000
#elif S2_TAN_PROJECTION
		...
		// This is true for all projections.
		public static readonly AreaMetric kAvgArea = new AreaMetric(S2Constants.M_4_PI / 6);     // 2.094
