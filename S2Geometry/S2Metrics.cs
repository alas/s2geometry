...
public static partial class S2
{
    public abstract class Metric
    {
        ...
        public int GetLevelForMaxValue(double value)
        {
            ...
            System.Diagnostics.Debug.Assert(level == S2.kMaxCellLevel || GetValue(level) <= value);
            System.Diagnostics.Debug.Assert(level == 0 || GetValue(level - 1) > value);
            return level;
        }
        ...
        public int GetLevelForMinValue(double value)
        {
            ...
            level = Math.Max(0, Math.Min(S2.kMaxCellLevel, level >> (Dim - 1)));
            System.Diagnostics.Debug.Assert(level == 0 || GetValue(level) >= value);
            System.Diagnostics.Debug.Assert(level == S2.kMaxCellLevel || GetValue(level + 1) < value);
            return level;
        }
    }
