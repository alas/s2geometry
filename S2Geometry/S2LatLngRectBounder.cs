...
namespace S2Geometry
{
    ...
    public class S2LatLngRectBounder
    {
        ...
        public S2LatLngRect GetBound()
        {
            ...
            S2LatLng kExpansion = S2LatLng.FromRadians(2 * S2Constants.DoubleEpsilon, 0);
        ...
        public static S2LatLngRect ExpandForSubregions(S2LatLngRect bound)
        {
            ...
            double lng_gap = Math.Max(0.0, Math.PI - bound.Lng.Length - 2.5 * S2Constants.DoubleEpsilon);
            ...
            double lat_gap1 = S2Constants.M_PI_2 + bound.Lat.Lo;
            double lat_gap2 = S2Constants.M_PI_2 - bound.Lat.Hi;
            ...
            else if (lng_gap >= S2Constants.M_PI_2)
            {
            ...
            double lat_expansion = 9 * S2Constants.DoubleEpsilon;
        ...
        public static S2LatLng MaxErrorForTests()
        {
            ...
            //    3.84 * S2Constants.DoubleEpsilon   for the RobustCrossProd calculation
            ...
            return S2LatLng.FromRadians(10 * S2Constants.DoubleEpsilon, 1 * S2Constants.DoubleEpsilon);
        }
        ...
        private void AddInternal(S2Point b, S2LatLng b_latlng)
        {
            ...
            Assert.True(S2PointUtil.ApproxEquals(b, b_latlng.ToPoint()));
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (lng_ab.Length >= Math.PI - 2 * S2Constants.DoubleEpsilon)
                    {
                    ...
                    if (m_a * m_b < 0 || Math.Abs(m_a) <= m_error || Math.Abs(m_b) <= m_error)
                    {
                        ...
                        var max_lat = Math.Min(
                            Math.Atan2(Math.Sqrt(n[0] * n[0] + n[1] * n[1]), Math.Abs(n[2])) + 3 * S2Constants.DoubleEpsilon,
                            S2Constants.M_PI_2);
                        ...
                        var lat_budget = 2 * Math.Asin(0.5 * (a_ - b).Norm * Math.Sin(max_lat));
                        var max_delta = 0.5 * (lat_budget - lat_ab.Length) + S2Constants.DoubleEpsilon;
