...
namespace S2Geometry.S2ShapeUtil
{
    public static partial class S2ShapeX
    {
        ...
        public static void BuildPolygonBoundaries(List<List<S2Shape>> components, List<List<S2Shape>> polygons)
        {
            ...
            //
            // We choose S2PointUtil.Origin for this purpose.  The loop nesting hierarchy then
            // determines the face structure.  Here are the details:
            //
            // 1. Build an S2ShapeIndex of all loops that do not contain S2PointUtil.Origin.
            ...
            for (int i = 0; i < components.Count; ++i)
            {
                ...
                foreach (var loop in component)
                {
                    if (component.Count > 1 &&
                        !loop.ContainsBruteForce(S2PointUtil.Origin))
                    {
                ...
                Assert.True(i + 1 == outer_loops.Count); // Component is not a subdivision
            }
            ...
            var children = new Dictionary<S2Shape, List<S2Shape>>(); // btree_map
            for (int i = 0; i < outer_loops.Count; ++i)
            {
                ...
                if (depth > 0)
                {
                    foreach (var candidate in ancestors[i])
                    {
                        ...
                        {
                            Assert.True(ancestor == null);
                            ancestor = candidate;
                        }
                    }
                    Assert.True(ancestor != null);
                }
                if (!children.ContainsKey(ancestor)) children.Add(ancestor, new List<S2Shape>());
                children[ancestor].Add(outer_loops[i]);
            }
