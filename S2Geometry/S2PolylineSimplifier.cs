...
namespace S2Geometry
{
    ...
    public class S2PolylineSimplifier
    {
        ...
        public bool AvoidDisc(S2Point point, S1ChordAngle radius, bool disc_on_left)
        {
            ...
            double center = GetAngle(point);
            double opposite = (center > 0) ? (center - Math.PI) : (center + Math.PI);
            S1Interval target = (disc_on_left ? new S1Interval(opposite, center) :
                                 new S1Interval(center, opposite));
        ...
        {
            const double DBL_ERR = 0.5 * S2Constants.DoubleEpsilon;
        ...
        private readonly S2Point x_dir_;
        private readonly S2Point y_dir_;
        private S1Interval window_;
    }
}
