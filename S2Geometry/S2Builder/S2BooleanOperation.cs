    ...
    private class Impl
    {
        ...
        private class CrossingProcessor
        {
            ...
            public void DoneBoundaryPair()
            {
                ...
                {
                    MyDebug.Assert(source_id_map_.ContainsKey(tmp.Item2.Item1));
            ...
            private bool ProcessEdge1(Edge a_id, S2Shape.Edge a, CrossingIterator it)
            {
                ...
                {
                    MyDebug.Assert(it.BIndex().MakeS2ContainsPointQuery().Contains(a.V1) == Inside ^ invert_b_);
                }
            ...
            private bool ProcessEdge2(Edge a_id, S2Shape.Edge a, CrossingIterator it)
            {
                ...
                if (!emit_shared)
                {
                    ...
                    if (r.matches_polygon() || r.matches_sibling())
                    {
                        ...
                        MyDebug.Assert(r.interior_crossings == 0);
            ...
            private EdgeCrossingResult ProcessEdgeCrossings(Edge a_id, S2Shape.Edge a, CrossingIterator it)
            {
                ...
                for (; !it.Done(a_id); it.Next())
                {
                    ...
                    {
                        MyDebug.Assert(2 == it.BDimension());
            ...
            {
                MyDebug.Assert(1 == it.BDimension());
                MyDebug.Assert(it.BEdge().V0 == v || it.BEdge().V1 == v);
                MyDebug.Assert(dimension == 0 || dimension == 1);
            ...
            while (next_id != kSentinel)
            {
                ...
                while (next_id.ShapeId == a_shape_id)
                {
                    ...
                    while (edge_id < chain_limit)
                    {
                        ...
                        MyDebug.Assert(cp.Inside || next_crossing.AId() == a_id);
        ...
        private bool AreRegionsIdentical()
        {
            ...
            for (int s = 0; s < num_shape_ids; ++s)
            {
                ...
                for (int c = 0; c < num_chains; ++c)
                {
                    ...
                    MyDebug.Assert(a_chain.Start == b_chain.Start);
        ...
        private bool IsFullPolygonResult(/*S2Builder.Graph g,*/ out S2Error error)
        {
            ...
            MyDebug.Assert(SnapFunction.kMaxSnapRadius.GetDegrees() <= 70);
            //
...
public class GraphEdgeClipper
{
    ...
    private static List<int> GetInputEdgeChainOrder(Graph g, List<InputEdgeId> input_ids)
    {
        ...
        MyDebug.Assert(g.Options.DuplicateEdges_ == DuplicateEdges.KEEP);
        MyDebug.Assert(g.Options.SiblingPairs_ == SiblingPairs.KEEP);
    ...
    public void Run()
    {
        ...
        for (int i = 0; i < order_.Count; ++i)
        {
            ...
            for (; next < input_crossings_.Count; ++next)
            {
                ...
                {
                    MyDebug.Assert(input_crossings_[next].Item2.InputId == S2BooleanOperation.kSetReverseA);
            ...
            MyDebug.Write($"input edge {a_input_id} (inside={inside}):");
            foreach (var id in a_vertices) MyDebug.Write(" " + id);
#endif
            ...
            for (int bi = 0; bi < b_input_edges.Count; ++bi)
            {
                ...
                {
#if s2builder_verbose
                    MyDebug.Write($"{Environment.NewLine}  b input edge {b_input_edges[bi].InputId} (l2r={left_to_right}, crossing={a_vertices[a_index]})");

                    foreach (var x in b_edges[bi])
                    {
                        var e = g_.GetEdge(x.Id);
                        MyDebug.Write($" ({e.ShapeId}, {e.EdgeId})");
                    }
#endif
    ...
    private void GatherIncidentEdges(List<int> a, int ai, List<CrossingInputEdge> b_input_edges, List<CrossingGraphEdgeVector> b_edges)
    {
        ...
        MyDebug.Assert(b_input_edges.Count == b_edges.Count);
...
#if s2builder_verbose
        MyDebug.WriteLine("Edges after clipping: ");
        for (int i = 0; i < new_edges.Count; ++i)
        {
            MyDebug.WriteLine($"  {new_input_edge_ids[i]} ({new_edges[i].ShapeId}, {new_edges[i].EdgeId})");
        }
#endif
