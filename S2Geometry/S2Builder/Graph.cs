...
public partial class S2Builder
{
    ...
    public class Graph
    {
            ...
            IsFullPolygonPredicate? is_full_polygon_predicate)
        {
            ...
            MyDebug.Assert(edges.IsSorted());
            MyDebug.Assert(edges.Count == input_edge_id_set_ids.Count);
        }
        ...
        public void MakeSiblingMap(EdgeLoop in_edge_ids)
        {
            ...
            {
                MyDebug.Assert(GetEdge(e) == Reverse(GetEdge(in_edge_ids[e])));
            }
            ...
            for (var e = 0; e < NumEdges; ++e)
            {
                ...
                {
                    MyDebug.Assert(e + 1 < NumEdges);
                    MyDebug.Assert(GetEdge(e + 1).ShapeId == v);
                    MyDebug.Assert(GetEdge(e + 1).EdgeId == v);
                    MyDebug.Assert(in_edge_ids[e] == e);
                    MyDebug.Assert(in_edge_ids[e + 1] == e + 1);
                    in_edge_ids[e] = e + 1;
                    in_edge_ids[e + 1] = e;
                    ++e;
                }
        ...
        {
            MyDebug.Assert(Options.DegenerateEdges_ == DegenerateEdges.DISCARD ||
                   Options.DegenerateEdges_ == DegenerateEdges.DISCARD_EXCESS);
            MyDebug.Assert(Options.EdgeType_ == EdgeType.DIRECTED);
        ...
        public bool GetUndirectedComponents(LoopType loop_type, List<UndirectedComponent> components, out S2Error error)
        {
            MyDebug.Assert(Options.DegenerateEdges_ == DegenerateEdges.DISCARD ||
                   Options.DegenerateEdges_ == DegenerateEdges.DISCARD_EXCESS);
            MyDebug.Assert(Options.EdgeType_ == EdgeType.UNDIRECTED);
        ...
        {
            MyDebug.Assert(Options.SiblingPairs_ == SiblingPairs.DISCARD ||
        ...
        private class EdgeProcessor
        {
            ...
            public void Run(out S2Error error)
            {
                ...
                for (; ; )
                {
                    ...
                    {
                        MyDebug.Assert(options_.SiblingPairs_ == SiblingPairs.REQUIRE ||
                               options_.SiblingPairs_ == SiblingPairs.CREATE);
        ...
        private class PolylineBuilder
        {
            ...
            private EdgePolyline BuildPath(int e)
            {
                ...
                for (; ; )
                {
                    ...
                    if (directed_)
                    {
                        MyDebug.Assert(1 == out_.Degree(v));
                        e = out_.EdgeIds(v).First();
                    }
                    else
                    {
                        MyDebug.Assert(2 == out_.Degree(v));
