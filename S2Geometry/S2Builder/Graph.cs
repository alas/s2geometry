...
namespace S2Geometry
{
    ...
    public partial class S2Builder
    {
        ...
        public class Graph
        {
            ...
            //     returned value (a int) in the "label_set_lexicon".
            // "label_set_lexicon":
            //   - a class that maps a int to a set of S2Builder.Labels.
            ...
            public static Edge Reverse(Edge e) => new Edge(e.Item2, e.Item1);
            ...
            public void MakeSiblingMap(EdgeLoop in_edge_ids)
            {
                ...
                for (var e = 0; e < NumEdges; ++e)
                {
                    var v = GetEdge(e).Item1;
                    if (GetEdge(e).Item2 == v)
                    {
                        Assert.True(e + 1 < NumEdges);
                        Assert.True(GetEdge(e + 1).Item1 == v);
                        Assert.True(GetEdge(e + 1).Item2 == v);
            ...
            //   for (var label : g.labels(input_edge_id)) { ... }
            public LabelSet Labels(int e) => LabelSetLexicon.IdSet_(LabelSetIds[e]);
            ...
            public int LabelSetId(int e) => LabelSetIds[e];
            ...
            public bool GetLeftTurnMap(EdgeLoop in_edge_ids, out EdgeLoop left_turn_map, out S2Error error)
            {
                ...
                while (min_edge != sentinel)
                {
                    ...
                    var v0 = min_edge.Item1;
                    for (; min_edge.Item1 == v0; min_edge = new[] { out_edge, Reverse(in_edge) }.Min())
                    {
                        var v1 = min_edge.Item2;
            ...
            public static void CanonicalizeVectorOrder(InputEdgeLoop min_input_ids, List<EdgeLoop> chains) => 
                chains.Sort((EdgeLoop a, EdgeLoop b) => min_input_ids[a[0]].CompareTo(min_input_ids[b[0]]));
            ...
            public bool GetDirectedLoops(LoopType loop_type, List<EdgeLoop> loops, out S2Error error)
            {
                ...
                for (var start = 0; start < NumEdges; ++start)
                {
                    ...
                    for (var e = start; left_turn_map[e] >= 0; e = next)
                    {
                        ...
                        if (loop_type == LoopType.SIMPLE)
                        {
                            path_index[GetEdge(e).Item1] = path.Count - 1;
                            var loop_start = path_index[GetEdge(e).Item2];
                            ...
                            foreach (var e2 in loop) path_index[GetEdge(e2).Item1] = -1;
            ...
            public static void ProcessEdges(GraphOptions options, List<Edge> edges, List<int> input_ids, IdSetLexicon id_set_lexicon, out S2Error error)
            {
            ...
            public static List<S2Point> FilterVertices(List<S2Point> vertices, List<Edge> edges, List<int> tmp)
            {
                ...
                {
                    var e = edges[i];
                    edges[i] = new Edge(vmap[e.Item1], vmap[e.Item2]);
                }
                return new_vertices;
            }
            ...
            public override string ToString()
            {
                ...
                {
                    S2Point[] vertices = { Vertex(edge.Item1),
                        Vertex(edge.Item2) };
            ...
            public class VertexOutMap
            {
                ...
                public void Init(Graph g)
                {
                    ...
                    {
                        while (e < g.NumEdges&& g.GetEdge(e).Item1 < v) ++e;
                        edge_begins_.Add(e);
                    }
                }
                ...
                private readonly EdgeLoop edge_begins_ = new EdgeLoop();
            }
            ...
            public class VertexInMap
            {
                ...
                public void Init(Graph g)
                {
                    ...
                    for (var v = 0; v <= g.NumVertices; ++v)
                    {
                        while (e < g.NumEdges&& g.GetEdge(InEdgeIds[e]).Item2 < v) ++e;
                ...
                private readonly EdgeLoop in_edge_begins_ = new EdgeLoop();
            }
            ...
            private class EdgeProcessor
            {
                    ...
                    for (; ; )
                    {
                        ...
                        if (edge.Item1 == edge.Item2)
                        {
                            ...
                            if (options_.DegenerateEdges_== DegenerateEdges.DISCARD_EXCESS &&
                                ((out_begin > 0 &&
                                  edges_[out_edges_[out_begin - 1]].Item1 == edge.Item1) ||
                                 (out_ < num_edges && edges_[out_edges_[out_]].Item1 == edge.Item1) ||
                                 (in_begin > 0 &&
                                  edges_[in_edges_[in_begin - 1]].Item2 == edge.Item1) ||
                                 (in_ < num_edges && edges_[in_edges_[in_]].Item2 == edge.Item1)))
                            {
                                 ...
                                 options_.SiblingPairs_== SiblingPairs.CREATE))
                            {
                                ...
                                AddEdges(options_.DuplicateEdges_== DuplicateEdges.MERGE ?
                                         1 : (n_out / 2), edge, MergeInputIds(out_begin, out_));
                            }
                            else if (options_.DuplicateEdges_== DuplicateEdges.MERGE)
                            {
                ...
                private readonly InputEdgeLoop tmp_ids_ = new InputEdgeLoop();
            }
            ...
            private class PolylineBuilder
            {
                ...
                public List<EdgePolyline> BuildPaths()
                {
                    ...
                    foreach (var e in edges)
                    {
                        if (!used_[e] && !IsInterior(g_.GetEdge(e).Item1))
                        {
                ...
                public List<EdgePolyline> BuildWalks()
                {
                    ...
                    foreach (var e in edges)
                    {
                        ...
                        --excess_used_[g_.GetEdge(polylines.Last().Last()).Item2];
                    }
                    ...
                    for (var i = 0; i < edges.Count && edges_left_ > 0; ++i)
                    {
                        ...
                        for (var j = i; j < edges.Count && min_input_ids_[edges[j]] == id; ++j)
                        {
                            ...
                            if (g_.GetEdge(e2).Item1 == v) ++excess;
                            if (g_.GetEdge(e2).Item2 == v) --excess;
                        }
                        // It is also acceptable to start a polyline from any degenerate edge.
                        if (excess == 1 || g_.GetEdge(e).Item2 == v)
                        {
                ...
                private void MaximizeWalk(EdgePolyline polyline)
                {
                    ...
                    {
                        var v = i == 0 ? g_.GetEdge(polyline[i]).Item1
                                      : g_.GetEdge(polyline[i - 1]).Item2;
                        foreach (var e in out_.EdgeIds(v))
                        {
                            if (!used_[e])
                            {
                                var loop = BuildWalk(v);
                                Assert.True(v == g_.GetEdge(loop.Last()).Item2);
                ...
                private readonly Dictionary<int, int> excess_used_ = new Dictionary<int, int>(); // gtl.btree_map
            }
        }
    }
}
